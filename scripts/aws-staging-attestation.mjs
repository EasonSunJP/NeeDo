...
const bootstrapScript = [
  ...
  "swapfile=/var/lib/needo/swapfile",
  "swap_size_mib=2048",
  "swap_size_bytes=$((swap_size_mib * 1024 * 1024))",
  "command -v mkswap >/dev/null",
  "command -v swapon >/dev/null",
  "if test -e \"$swapfile\"; then",
  "  test -f \"$swapfile\"",
  "  test \"$(stat -c %s \"$swapfile\")\" -eq \"$swap_size_bytes\"",
  "else",
  ...
  "fi",
  "chmod 0600 \"$swapfile\"",
  "test \"$(blkid -p -s TYPE -o value \"$swapfile\")\" = \"swap\"",
  "grep -Fqx '/var/lib/needo/swapfile none swap defaults,nofail 0 0' /etc/fstab || \\",
  "  printf '%s\\n' '/var/lib/needo/swapfile none swap defaults,nofail 0 0' >> /etc/fstab",
  "if ! swapon --show=NAME --noheadings --raw | grep -Fxq \"$swapfile\"; then",
  "  swapon \"$swapfile\"",
  "fi",
  "swapon --show=NAME --noheadings --raw | grep -Fxq \"$swapfile\"",
  "",
...
const verificationScript = [
  ...
  "done",
  "test \"$(stat -c '%u:%g %a' /srv/needo/media/customer-avatars)\" = \"1000:1000 750\"",
  "test \"$(stat -c '%u:%g %a' /srv/needo/media/identity-applications)\" = \"1000:1000 750\"",
  "test \"$(stat -c '%u:%g %a' /srv/needo/media/im-media)\" = \"1000:1000 750\"",
  "test \"$(stat -c '%u:%g %a' /srv/needo/media/content-media)\" = \"1000:1000 750\"",
  "swapfile=/var/lib/needo/swapfile",
  "test -f \"$swapfile\"",
