...
export function requireAcceptedEnvironment(value) {
  ...
  const commonAccepted = value.accountId === APPROVED_ACCOUNT
    && value.region === APPROVED_REGION
    && value.hostname === APPROVED_HOSTNAME;
  const stackAccepted = value.ssm?.online === true
    && value.stack?.name === "needo-staging-infrastructure"
    && ["CREATE_COMPLETE", "UPDATE_COMPLETE"].includes(value.stack?.status);
  const initialAccepted = value.applicationDeployed === false
    && value.migrationRun === false
    && value.seedRun === false
    && value.dnsModified === false
    && value.businessDataMutation === false
    && value.secretVersionCount === 0;
  const activeAccepted = value.gate === "aws-staging-application-baseline"
    && value.status === "passed"
    && value.applicationDeployed === true
    && value.migrationRun === true
    && value.seedRun === false
    && value.dnsModified === false
    && value.businessDataMutation === true
    && value.secretVersionCount === 1
    && value.readiness?.backend === true
    && value.readiness?.operations === true
    && value.readiness?.merchant === true
    && FULL_REVISION.test(value.sourceRevision ?? "")
    && value.activeRelease?.sourceRevision === value.sourceRevision
    && SHA256.test(value.activeRelease?.archiveSha256 ?? "")
    && typeof value.timestamp === "string"
    && new Date(value.timestamp).toISOString() === value.timestamp;
  if (!commonAccepted || !stackAccepted || (!initialAccepted && !activeAccepted)) {
    throw new Error("environment acceptance evidence is not approved for application deployment");
  }
  ...
  return Object.freeze({
    ...
    instanceId,
    applicationDeployed: value.applicationDeployed,
