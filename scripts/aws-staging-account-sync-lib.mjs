...
export const parseImportSummary = (stdout) => {
  ...
  if (!exactKeys(value, summaryKeys) || value.gate !== "staging-selective-account-import" || value.status !== "passed" || value.userCount !== 262 || value.nonTestUserCount !== 0 || value.administratorCount !== 1 || !exactKeys(value.tableCounts, TABLES) || !exactKeys(value.verificationDigests, TABLES)) fail("ACCOUNT_SYNC_SSM_SUMMARY_REJECTED");
...
export const buildHostCommand = ({ region, transferBucket, transferKey, transferVersionId, backupBucket, backupKey, sourceRevision, bundleSha256 }) => {
  ...
  return [
    ...
    "docker compose --env-file /srv/needo/config/staging.env --project-name needo-staging --file \"$current_release/deploy/staging/docker-compose.yml\" exec -T mysql sh -c 'MYSQL_PWD=\"$MYSQL_PASSWORD\" mysqldump --single-transaction --routines --triggers --no-tablespaces -u\"$MYSQL_USER\" \"$MYSQL_DATABASE\"' | gzip -c > \"$backup_path\"",
