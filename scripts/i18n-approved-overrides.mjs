...
export const approvedI18nTranslationOverrides = [
  ...
  // Manual localization lock: user-center Japanese uses familiar consumer-app
  // wording and must never reintroduce generated ellipses or literal Chinese.
  ["个人中心", { ja: "マイページ" }],
  ["个人中心说明", { ja: "マイページの説明" }],
  ["账号资料、订单入口与服务权益都统一收在这里。", { ja: "アカウント情報、予約、各種特典をここでまとめて確認できます。" }],
  ["账号与服务", { ja: "アカウント・各種設定" }],
  ["账号设置", { ja: "アカウント設定" }],
  ["手机号、邮箱、登录密码", { ja: "電話番号・メールアドレス・パスワード" }],
  ["支付方式", { ja: "支払い方法" }],
  ["现金、NDP 与外部渠道状态", { ja: "現金・NDP・外部決済の状況" }],
  ["银行卡、PayPay、现金", { ja: "クレジットカード・PayPay・現金" }],
  ["发票记录", { ja: "請求書履歴" }],
  ["发票功能暂未开放", { ja: "請求書機能は現在利用できません" }],
  ["企业抬头与历史发票", { ja: "請求先情報・過去の請求書" }],
  ["通知设置", { ja: "通知設定" }],
  ["订单、营销、客服提醒", { ja: "予約・キャンペーン・サポートからのお知らせ" }],
  ["隐私与安全", { ja: "プライバシーとセキュリティ" }],
  ["登录设备、数据授权", { ja: "ログイン端末・データ利用設定" }],
  ["联系客服", { ja: "サポートに問い合わせる" }],
  ["退款、改期、投诉风控", { ja: "返金・予約変更・トラブルの相談" }],
  ["我的收藏", { ja: "お気に入り" }],
  // Manual terminology lock: every consumer-facing favorite noun, action,
  // state, and metric uses お気に入り instead of collection/save wording.
  ["收藏", { ja: "お気に入り" }],
  ["取消收藏", { ja: "お気に入りから削除" }],
  ["已收藏", { ja: "お気に入り登録済み" }],
  ["收藏门店", { ja: "店舗をお気に入りに追加" }],
  ["收藏人数", { ja: "お気に入り登録者数" }],
  ["收藏数", { ja: "お気に入り数" }],
  ["查看收藏详细数字", { ja: "お気に入り登録数の詳細を表示" }],
  ["收藏失败，请稍后重试", { ja: "お気に入りに追加できませんでした。しばらくしてからもう一度お試しください。" }],
  ["家已收藏", { ja: "店舗をお気に入りに登録済み" }],
  ["惠比寿居酒屋收藏榜", { ja: "恵比寿の居酒屋 お気に入りランキング" }],
  ["按收入、收藏人气和订单量查看技师排名。", { ja: "売上、お気に入り登録数、注文数別にスタッフランキングを確認できます。" }],
  ["回复、点赞、转发、引用和收藏需要对应正式数据合同、权限和审计记录，当前不会写入本地假数据。", { ja: "返信、いいね、再投稿、引用、お気に入り登録には正式なデータ契約、権限、監査記録が必要です。現在はローカルのサンプルデータに書き込みません。" }],
  ["服务前提醒很有帮助，已经收藏了。", { ja: "サービス前のリマインダーがとても役立ったので、お気に入りに追加しました。" }],
  ["可线上预约、可收藏、可查看担当者、支持平台内聊天沟通。", { ja: "オンライン予約、お気に入りへの追加、担当スタッフの確認、アプリ内チャットが利用できます。" }],
  ["上次你发的那家店我也收藏了。", { ja: "前回送ってくれたお店もお気に入りに追加しておきました。" }],
  ["我看你上次收藏的那家店这周末还有双人档，要不要一起去？", { ja: "前にお気に入りに追加したお店は、今週末も2名で予約できるよ。一緒に行かない？" }],
  ["我收藏的服务卡，方便朋友直接了解价格、时长和可约时间。", { ja: "お気に入りに追加したサービスカードなら、友だちも料金・所要時間・予約可能時間をすぐ確認できます。" }],
  ["限时动态 -> Swipe link -> App 注册 -> 店铺收藏", { ja: "ストーリーズ → スワイプリンク → アプリ登録 → 店舗をお気に入りに追加" }],
  ["像朋友圈一样记录服务体验、收藏店铺和生活片段", { ja: "SNSのように、サービス体験やお気に入りの店舗、日常の出来事を記録できます。" }],
  ["长短视频测评 -> 简介链接 -> 店铺页收藏 -> 首单支付", { ja: "動画レビュー → プロフィールリンク → 店舗をお気に入りに追加 → 初回注文の支払い" }],
  ["这家我也收藏了，照片很有参考价值。", { ja: "このお店もお気に入りに追加しました。写真がとても参考になります。" }],
  ["最近收藏了夜间护理", { ja: "最近、夜間ケアをお気に入りに追加しました。" }],
  ["本次更新继续重做了前台三端共用的聊天底部输入模块。此前底部功能虽然已经可以发送图片、订单和位置，但整体位置偏低，输入区和加号面板也还不够像成熟的手机聊天工具。现在输入区已经改成更贴近参考图的结构：左边是独立的语音按钮，中间是主输入条，内部保留发送纸飞机图标，右边是表情和加号；整体底边也向上收了一些，更接近底部导航文字的基线位置。与此同时，加号面板从原来的 7 项改成了更完整的 8 宫格：保留照片、拍摄、视频通话、语音通话、位置、订单，并把“收藏”改成“介绍”，再新增“支付”入口，减少概念歧义。当前进入页版本号已同步更新为 ver：2604160836。", { ja: "今回の更新では、3つのユーザー向け画面で共通利用するチャット入力欄を刷新しました。左に音声、中央に入力欄と送信、右に絵文字と追加メニューを配置し、下端も調整しています。追加メニューは8項目に整理し、「お気に入り」を「紹介」に変更して「支払い」を追加しました。入口ページのバージョンは ver：2604160836 です。" }],
  ["聊天输入区这次重新按手机 IM 的习惯做了收口：左侧保留语音入口，中间是输入条，右侧是表情与加号；加号面板里把收藏改成了介绍，并新增了支付入口。", { ja: "チャット入力欄をモバイル向けに整理しました。左に音声、中央に入力欄、右に絵文字と追加メニューを配置し、追加メニューの「お気に入り」を「紹介」に変更して「支払い」を追加しました。" }],
  ["我的地址", { ja: "登録住所" }],
  ["家庭、公司、常用地址", { ja: "自宅・勤務先・よく使う住所" }],
  ["我的评价", { ja: "レビュー" }],
  ["已评价与待回复", { ja: "投稿済み・返信待ちのレビュー" }],
  ["eKYC本人确认", { ja: "本人確認（eKYC）" }],
  ["店铺会员", { ja: "店舗会員" }],
  ["查看已加入店铺与会员卡状态", { ja: "加入中の店舗と会員証を確認できます" }],
  ["我的订单", { ja: "予約" }],
  ["全部订单", { ja: "すべての予約" }],
  ["待服务", { ja: "サービス待ち" }],
  ["进行中", { ja: "サービス中" }],
  ["预约一览", { ja: "予約一覧" }],
  ["查看全部预约、订单状态和详情跳转", { ja: "予約一覧とステータス、詳細を確認できます。" }],
  ["更换头像", { ja: "プロフィール画像を変更" }],
  ["头像裁剪预览", { ja: "プロフィール画像の調整プレビュー" }],
  ["头像裁剪已套用，点击保存后生效。", { ja: "画像の調整を反映しました。保存すると変更が確定します。" }],
  ["基础信息", { ja: "基本情報" }],
  ["利用次数", { ja: "利用回数" }],
  ["未设置", { ja: "未設定" }],
  ["未设定", { ja: "未設定" }],
  ["去认证", { ja: "本人確認へ" }],
  ["查看账号设置说明", { ja: "アカウント設定の説明を表示" }],
  ["查看支付方式说明", { ja: "支払い方法の説明を表示" }],
  ["查看发票记录说明", { ja: "請求書履歴の説明を表示" }],
  ["查看通知设置说明", { ja: "通知設定の説明を表示" }],
  ["查看隐私与安全说明", { ja: "プライバシーとセキュリティの説明を表示" }],
  ["查看联系客服说明", { ja: "お問い合わせの説明を表示" }],
  ["查看预约一览说明", { ja: "予約一覧の説明を表示" }],
  ...
  [
    "仅好友可以看到该账号信息",
    { ja: "友だちのみ、このアカウント情報を確認できます。" }
  ],
  [
    "仅好友以及关联店铺和介绍关系中的关联人可见",
    { ja: "友だち、および関係する店舗・紹介関係者にのみ表示されます。" }
  ],
  [
    "开启隐私模式",
    { ja: "プライベートモードをオンにする" }
  ...
  [
    ...
    { ja: "プライベートモードをオンにすると、あなたのアカウントは検索結果に表示されなくなります。オンにしますか？" }
  ],
  [
    "开启",
    { ja: "オンにする" }
  ...
  [
    "隐私模式已关闭",
    { ja: "プライベートモードをオフにしました" }
  ],
  [
    "隐私模式保存失败，请重试",
    { ja: "プライベートモードを保存できませんでした。もう一度お試しください" }
  ],
  [
    "完全隐私模式",
    { ja: "完全プライベートモード" }
  ...
  // Manual settings-home copy lock: keep legal, identity, logout, explanatory,
  // and accessibility wording natural across workbook imports.
  ...
  ["本人", { "zh-Hant": "本人", ja: "本人", en: "Myself", ko: "내 자신" }],
  ["確認", { "zh-Hant": "確認", ja: "確認", en: "Confirm", ko: "확인하다" }],
  ["说明", { "zh-Hant": "說明", ja: "説明", en: "Illustrate", ko: "설명하다" }],
  ...
  ["语言页面说明", { "zh-Hant": "查看語言設定說明", ja: "言語設定の説明を表示", en: "View language settings information", ko: "언어 설정 설명 보기" }],
  ["查看可选语言说明", { "zh-Hant": "查看可選語言說明", ja: "選択できる言語の説明を表示", en: "View available language information", ko: "선택 가능한 언어 설명 보기" }],
  ...
  ["正在下载 Dodo 资源，完成后才能开启。", { "zh-Hant": "正在下載 Dodo 資源，完成後即可開啟。", ja: "Dodoのデータをダウンロードしています。完了後に有効にできます。", en: "Downloading Dodo assets. You can enable it when the download is complete.", ko: "Dodo 리소스를 다운로드하고 있습니다. 완료되면 활성화할 수 있습니다." }],
  ["Dodo 资源下载进度", { "zh-Hant": "Dodo 資源下載進度", ja: "Dodoのダウンロード進捗", en: "Dodo download progress", ko: "Dodo 다운로드 진행률" }],
