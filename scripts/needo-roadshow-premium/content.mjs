...
const common = {
  financing: `融資 ${yen(financing.amountJpy)}、出讓 ${pct(financing.equity)} 股權；Pre-money ${yen(financing.preMoney)}、Post-money ${yen(financing.postMoney)}。`,
...
const slideDefinitions = [
  ...
  ["light", "市場機會：先解決高摩擦服務，再擴展為生活服務基礎設施", `先從人工協調最深的店鋪與服務切入，再複製到跨城市生活服務；供給側參考包括接待飲食 ${market.receptionVenues.toLocaleString("zh-TW")}、深夜酒類 ${market.lateNightAlcoholVenues.toLocaleString("zh-TW")}、性風俗特殊營業 ${market.adultSpecialBusinesses.toLocaleString("zh-TW")}（類別可能交疊）。`, "S1、S2、S3"],
  ["light", "高需求市場必須以合規分層進入", "一般按摩、美容、餐飲與合規店鋪 SaaS 為近期核心；有資格或特殊監管的業態採條件式、獨立法務與風控，不納入核心收入承諾。", "S3、S4"],
  ["dark", "NeeDo：預約、履約、調度、透明點單與CPS的一體化平台", "產品不是單一流量入口，而是把需求、供給、場所、履約與結算放進同一個作業系統；所有模組均以權限、審計與可驗證事件為邊界。", "S5、S7"],
  ...
  ["light", "透明點單：高消費場景也能先選、再確認、全程看總額", "把含稅價格、可選項、追加確認、即時累計與明細帳單放進同一流程；正式上線前須完成支付、權限、審計與監管要求。", "S3、S7"],
  ["dark", "CPS引擎：讓達人內容直接連接可履約的服務", "內容點擊、服務選擇、預約、完單與佣金結算以同一事件鏈歸因；CPS 收入以實際完成交易為基礎，不以曝光替代履約。", "S5、S7"],
  ["light", "從點擊到結算，CPS讓每一筆增長都可歸因", "追蹤來源、服務、店鋪、訂單狀態與退款後金額，將達人增長與可履約供給連接；模型仍需真實渠道數據驗證。", "S5、S7"],
  ["light", "產品成熟度：核心體驗已可操作，後端與商業數據待本輪融資完成", "目前已有可操作原型與產品流程；本輪資金優先用於正式後端、資料庫、權限、支付、審計、商業化與真實留存／付費數據。", "S5、S7"],
  ...
  ["light", "多元收入，但同一個核心：每一次可完成的服務交易", `收入由店鋪 SaaS、預約費、成功介紹費與 CPS 結算組成；${common.unit}`, "S5、S7"],
  ["light", "單位經濟：低門檻SaaS建立關係，交易收入放大價值", `模型採用支付率 ${economics.paymentRate * 100}%、風險準備 ${economics.riskReserve} 日圓、用戶獎勵 ${economics.userReward} 日圓、雲端成本 ${economics.cloudPerOrder} 日圓/單；簡化單筆貢獻 ${economics.simplifiedOrderContribution} 日圓，須以實際數據持續校準。`, "S5"],
  ["light", "一般方案：三年形成6,000店的高密度履約網路", `一般方案店鋪 ${scenarios.general.stores.join(" → ")}；Y3 訂單 ${scenarios.general.orders[2].toLocaleString("zh-TW")}、營收 ${scenarios.general.revenueM[2]}M 日圓、淨利 ${scenarios.general.profitM[2]}M 日圓；${scenarios.general.label}。`, "S5"],
  ...
  ["light", "保守情境：較慢擴張仍保留正向現金生成能力", `保守方案店鋪 ${scenarios.conservative.stores.join(" → ")}；Y3 營收 ${scenarios.conservative.revenueM[2]}M 日圓、淨利 ${scenarios.conservative.profitM[2]}M 日圓；現金最低 ${scenarios.conservative.minCashM}M 日圓（第 ${scenarios.conservative.minCashMonth} 個月）。`, "S5"],
  ["light", "三種情境的核心假設差異", `保守／一般／激進 Y3 店鋪分別為 ${scenarios.conservative.stores[2].toLocaleString("zh-TW")}／${scenarios.general.stores[2].toLocaleString("zh-TW")}／${scenarios.aggressive.stores[2].toLocaleString("zh-TW")}；差異來自供給拓展、渠道速度、訂單密度與執行資源，不是三個同時成立的承諾。`, "S5、S6"],
  ...
  ["light", "市場數據與引用來源", `宏觀市場、業態統計與合規依據列於來源表；${sourceDetail(["S1", "S2", "S3", "S4"])}。內部模型與商談資料另標示為內部來源，不視為第三方審計。`, "S1、S2、S3、S4、S5、S6、S7"],
  ["dark", "合規與風險控制不是附加功能，而是產品的一部分", "以業態分層、資格審核、店鋪與服務者權限、透明價格、支付／退款、個資保護、審計與爭議流程作為產品基礎；成人性服務相關業態不納入核心收入情境。", "S3、S4、S5"],
