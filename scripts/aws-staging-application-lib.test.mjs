...
test("an exact verified active staging baseline can authorize a repeat release", () => {
  const baseline = {
    gate: "aws-staging-application-baseline",
    status: "passed",
    timestamp: "2026-09-09T00:00:00.000Z",
    accountId: "430611185505",
    applicationDeployed: true,
    businessDataMutation: true,
    dnsModified: false,
    elasticIp: "32.236.134.43",
    hostname: "staging.needo.life",
    migrationRun: true,
    ...
    activeRelease: { sourceRevision: revision, archiveSha256: sha256 },
    resourceIds: {
      applicationSecretArn: "REDACTED",
      backupBucketName: "needo-staging-backup-example",
      dataVolumeId: "vol-0123456789abcdef0",
    ...
    readiness: { backend: true, merchant: true, operations: true },
    secretVersionCount: 1,
    seedRun: false,
    ssm: { online: true },
    stack: { name: "needo-staging-infrastructure", status: "UPDATE_COMPLETE" }
  };
  const environment = requireAcceptedEnvironment(baseline);

  assert.equal(environment.sourceRevision, revision);
  assert.equal(environment.applicationDeployed, true);
  assert.throws(
    () => requireAcceptedEnvironment({ ...baseline, accountId: "000000000000" }),
    /not approved/
  );
  assert.throws(
    () => requireAcceptedEnvironment({ ...baseline, readiness: { ...baseline.readiness, merchant: false } }),
    /not approved/
  );
  assert.throws(
    () => requireAcceptedEnvironment({
      ...baseline,
      activeRelease: { ...baseline.activeRelease, sourceRevision: "b".repeat(40) }
    }),
    /not approved/
  );
});
...
test("application deployer is present and validates the exact staging target", () => {
  const deployer = readFileSync(
    new URL("./aws-staging-deploy-application.mjs", import.meta.url),
    "utf8"
  );

  assert.match(deployer, /Application deployment target is not approved/);
  assert.match(deployer, /source-revision/);
  assert.match(deployer, /Pre-migration snapshot/);
  assert.match(deployer, /SSM application deployment/);
  assert.doesNotMatch(deployer, /set -x|printenv/);
});
