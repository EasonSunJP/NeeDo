...
4. Checkout has a non-null selected method/time and `paymentSelectedAt <= paymentConfirmedAt`. Cash and other require exactly one non-deleted `PAYMENT_METHOD_SELECTED` event linked to the same order, service session and checkout; its actor is the booking customer, its `occurredAt` equals `paymentSelectedAt` and is no later than `paymentConfirmedAt`, and its reason is null. Cash metadata is exactly `{ method: "cash" }`; other metadata is exactly `{ method: "other", otherMethodCode, otherMethodLabel }`, with code/label equal to the checkout values. NDP permits exactly two formal shapes: either one exact preselection event under the same link/actor/time/reason rules with metadata `{ method: "ndp" }`, or zero selection events only for the direct-pay writer shape where `paymentSelectedAt === paymentConfirmedAt`. A direct-pay NDP row with an earlier selection time, more than one event, or any missing, cross-order/session/checkout, wrong-actor/time/reason, mismatched-value or extra-key selection evidence is contradictory and fails the whole request.
...
- missing/duplicate/wrong ledger, event, actor, reference, audit, receipt reason, time order or checkout arithmetic; exact `PAYMENT_METHOD_SELECTED` authority for cash/other and both NDP shapes (preselected event or zero-event direct pay with equal selection/confirmation timestamps), including duplicate/cross-order/session/checkout/wrong-customer/time/reason/metadata-extra-key cases and zero-event NDP with unequal timestamps; crossed technician/operations evidence-reference pairs; cash/NDP stale other-method fields; non-null `otherPaymentReference`; OTHER code/label mismatch; and any stale ledger/receipt field for the selected method;
