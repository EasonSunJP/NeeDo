...
- Cursor retention classification and the bounded initial replay read are one atomic Redis Lua operation, preventing exact stream trimming from racing a separate cursor check. Future, trimmed, unknown, and empty/recreated-stream cursors return pre-header `40900 error.live_dashboard.cursor_reset_required`; retained cursors and the exact current head remain valid. Rejected cursors are not recorded as successful connection audits. Replay/read failure propagates through the normal pre-header API error path without a false `connected` frame or cursor advancement, allowing the authenticated fetch client to retry the same `Last-Event-ID` and recover the gap exactly once.
- One cancellation-safe serialized gate now owns the full joining-client sequence: transport subscription readiness, atomic retained replay, the service-provided audit callback, subscriber registration, replay delivery, and `connected`. Pub/Sub wakeups queue behind that gate and drain only after establishment, closing the prior replay-to-registration delivery gap. The retained replay is the establishment read; there is no second fallible post-header drain.
...
- OpenAPI assigns browser retry ownership explicitly: use Bearer-authenticated `fetch`, parse SSE frames, persist only the last successfully delivered Redis Stream ID, and apply exponential backoff plus jitter on EOF/network/503. Native `EventSource` is not the authenticated contract. A 409 instead clears the cursor, refreshes the full snapshot, and reconnects without `Last-Event-ID`.
...
Closure joining-client RED:

```text
npm test -- --runInBand tests/live-dashboard-events.test.ts -t "fences replay|retained-read failure|no second fallible|audit failure|pending audit"
FAIL: the audit callback was never invoked under the gateway; read failure was swallowed; the obsolete second drain ended establishment; audit failure did not propagate; close could not cancel a pending in-gate audit.

npm test -- --runInBand tests/live-dashboard-api.test.ts tests/openapi.test.ts -t "Last-Event-ID|cursor reset|dependency error|connection audit fails|protected Bearer-only"
FAIL: service prepared/audited outside the gateway fence, read failure recorded a successful connect audit, and OpenAPI omitted authenticated fetch/manual retry ownership.
```
...
Closure joining-client GREEN:

```text
npm test -- --runInBand tests/live-dashboard-events.test.ts -t "fences replay|retained-read failure|no second fallible|audit failure|pending audit"
PASS: 5/5 focused adversarial establishment tests

npm test -- --runInBand tests/live-dashboard-events.test.ts tests/live-dashboard-api.test.ts tests/openapi.test.ts
PASS: 3 suites, 66/66 tests

npm test -- --runInBand <the 25 Task 7/booking/payment/expiry/OpenAPI/security suites listed above>
PASS: 25 suites, 256/256 tests
...
The final lifecycle adaptation adds a formal atomic retained-replay result to the Redis stream port. Its single-key Lua operation reads Redis server time, rejects a supplied cursor unless that exact stream ID is still inside the five-minute retained window, and returns at most 100 exclusive successors in the same atomic execution. After authentication, authorization, platform identity, and hierarchy checks, the service passes its audit operation as a `beforeConnect` callback; the gateway runs the cursor read and callback inside the same serialized establishment gate as process drains, still without owning repository/audit logic. Only then does it register the client and write replay plus `connected` before releasing queued wakeups. A rejected client is expected to discard its cursor, fetch a full snapshot, and reconnect without `Last-Event-ID`. This preserves the current standalone Redis topology; multi-key Redis Cluster support remains outside Task 7.
...
Fix-loop commits: `eddf710e fix: harden regional dashboard event stream`, `1aa39e76 fix: fence dashboard stream races`, `f837eada fix: close dashboard stream lifecycle gaps`, and the joining-client closure commit recorded in the handoff.
