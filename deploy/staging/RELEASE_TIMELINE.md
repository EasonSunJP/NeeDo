...
migration and deployment hook are now integrated locally in `aws-staging-deployment-design`;
the frontend and backend builds pass together. `dashboard-remaining-acceptance` remains the
source of the separately running local UI acceptance instance. No archive has been uploaded
and no deployment has run.
...
The application read endpoint is restricted to a platform identity with dashboard-read
permission and returns records for the server's configured DEPLOY_ENV only. There is no public
HTTP creation, update, deletion, or comment endpoint. The current hook targets this repository's
existing staging flow; production release integration has not been executed or validated.
