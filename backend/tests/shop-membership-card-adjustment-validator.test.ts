...
describe("shop membership card adjustment validators", () => {
  it("accepts exactly one target field and trims the mandatory reason", () => {
    expect(
      shopMembershipCardAdjustmentCreateBodySchema.parse({
  ...
  it("accepts only approve/reject decisions and bounded pagination", () => {
    expect(
      shopMembershipCardAdjustmentDecisionBodySchema.parse({
        decision: "approve",
    ...
    expect(
      shopMembershipCardAdjustmentListQuerySchema.parse({
        page: "2",
        pageSize: "10",
        status: "pending"
      })
