...
describe("analytics ranking OpenAPI", () => {
  ...
  it("publishes the single protected strict ranking operation", () => {
    ...
    const parameters = Object.fromEntries(
      path.get.parameters.map((parameter) => [parameter.name, parameter])
    );
    expect(parameters.kind).toMatchObject({
      in: "path",
      required: true,
      schema: { type: "string", enum: ["service", "technician", "customer"] }
    });
    expect(parameters.metric.schema).toMatchObject({
      enum: ["gmv", "completedCount"],
      default: "gmv"
    });
    expect(parameters.categoryId.schema).toMatchObject({
      type: "integer",
      minimum: 1,
      maximum: 2147483647
    });
    expect(parameters.city.schema).toMatchObject({
  ...
  it("publishes examples for all kinds and metrics plus stable errors", () => {
    ...
    expect(path.get.responses["400"].content["application/json"].example).toMatchObject({
      code: 40001
    });
    expect(path.get.responses["401"].content["application/json"].example).toMatchObject({
      code: 40105,
      data: null
    });
    expect(path.get.responses["403"].content["application/json"].example).toMatchObject({
      code: 40301,
      data: null
    });
