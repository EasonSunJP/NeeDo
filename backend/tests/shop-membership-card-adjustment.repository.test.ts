...
const card = {
  ...
  membership: {
    ...
    shop: {
      id: 71,
      publicId: "00000000-0000-4000-8000-000000000671",
      shopNo: "s000000071",
      name: "青山护理店"
    },
...
describe("ShopMembershipCardAdjustmentRepository", () => {
    ...
    expect(findMany).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        where: expect.objectContaining({
          AND: [
            { OR: [{ status: { not: "PENDING" } }, { status: "PENDING", expiresAt: { gt: now } }] }
          ]
        })
      })
    );
  ...
  it("creates a 72-hour pending request, audit and customer notification in one transaction", async () => {
    ...
    await expect(
      repository.createRequestWithAuditAndNotification(createInput)
    ).resolves.toMatchObject({
      kind: "created",
      value: { publicId: requestPublicId, status: "pending", expiresAt }
    });
    expect(transaction.shopMembershipCard.findFirst).toHaveBeenCalledWith(
      expect.objectContaining({
        where: {
          publicId: cardPublicId,
          membership: { shopId: 71, deletedAt: null },
          deletedAt: null
        }
      })
    );
    expect(transaction.shopMembershipCardAdjustmentRequest.create).toHaveBeenCalledWith(
      expect.objectContaining({
    ...
    expect(transaction.auditLog.create).toHaveBeenCalledWith({
      data: expect.objectContaining({ action: createInput.audit.action, targetId: 91 })
    });
  ...
  it("expires a due pending request under lock before creating its replacement", async () => {
    ...
    const expired = adjustment({
      status: "EXPIRED",
      pendingKey: null,
      expiresAt: now,
      updatedAt: now
    });
    const replacement = adjustment({
      id: 92,
      publicId: "00000000-0000-4000-8000-000000000603",
      expiresAt
    });
    const transaction = {
      $queryRaw: jest
  ...
  it("approves against the current snapshot and changes principal exactly once", async () => {
    ...
    const transaction = {
      ...
      userIdentity: {
        findFirst: jest.fn().mockResolvedValueOnce({ id: 109 }).mockResolvedValueOnce({ id: 141 })
      },
      auditLog: { create: jest.fn().mockResolvedValue({ id: 1 }) },
      notification: { create: jest.fn().mockResolvedValue({ id: 2 }) }
    };
    ...
    await expect(
      repository.decideRequestWithAuditAndNotification(decisionInput)
    ).resolves.toMatchObject({
      kind: "approved",
    ...
    expect(transaction.auditLog.create).toHaveBeenCalledWith({
      data: expect.objectContaining({ action: decisionInput.audit.action, targetId: 91 })
    });
  });
  ...
  it("expires at the exact deadline without touching the card", async () => {
    ...
    await expect(
      repository.decideRequestWithAuditAndNotification(decisionInput)
    ).resolves.toMatchObject({ kind: "expired", value: { status: "expired" } });
  ...
  it("expires a scoped worker batch with audit and both notifications", async () => {
    ...
    expect(client.shopMembershipCardAdjustmentRequest.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          status: "PENDING",
          expiresAt: { lte: expiresAt },
          ...
          card: expect.objectContaining({
            membership: expect.objectContaining({ customerProfile: { userId: 41 } })
          })
        }),
        take: 100
      })
  ...
  it("invalidates a stale snapshot without overwriting the newer card", async () => {
    ...
    await expect(
      repository.decideRequestWithAuditAndNotification(decisionInput)
    ).resolves.toMatchObject({ kind: "invalidated", value: { status: "invalidated" } });
    expect(transaction.shopMembershipCard.updateMany).not.toHaveBeenCalled();
    expect(transaction.shopMembershipCardAdjustmentRequest.updateMany).toHaveBeenCalledWith({
