...
const createFixture = async () => {
  ...
  const users = [
    {
      ...
      userRoles: [{ deletedAt: null, role: role("merchant_owner", ["wallet:adjustment:create", "wallet:adjustment:list"]) }]
    },
    {
      ...
      identities: [{ id: 2, userId: 2, type: "platform_admin", scopeType: "global", scopeId: null, displayName: "Operator", isDefault: true, isActive: true, deletedAt: null }],
      userRoles: [{ deletedAt: null, role: role("operator", ["backoffice:wallet-adjustment:list", "backoffice:wallet-adjustment:review"]) }]
    }
  ];
  let wallet = { id: 3, ownerType: "shop", ownerId: 11, currency: "NDP", availableBalance: 1000, frozenBalance: 0, createdAt: now, updatedAt: now };
  ...
  Object.assign(ledgerRepository, {
    runInTransaction: jest.fn(async (handler: (repository: unknown) => Promise<unknown>) => handler(ledgerRepository)),
    findWalletAdjustmentByIdempotencyKey: jest.fn(async (key: string) => adjustment?.idempotencyKey === key ? adjustment : null),
    createWalletAdjustmentRequest: jest.fn(async (input: Record<string, unknown>) => {
      adjustment = { id: 41, status: "pending", bankReference: null, note: null, reviewedById: null, reviewedAt: null, reviewNote: null, ledgerTransactionId: null, createdAt: now, updatedAt: now, ...input };
      return adjustment;
    }),
    listWalletAdjustmentRequests: jest.fn(async () => ({ list: adjustment ? [adjustment] : [], total: adjustment ? 1 : 0, page: 1, page_size: 20 })),
    lockWalletAdjustmentRequest: jest.fn(async (id: number) => adjustment?.id === id ? adjustment : null),
    approveWalletAdjustmentRequest: jest.fn(async (input: Record<string, unknown>) => {
      adjustment = { ...adjustment, status: "approved", reviewedById: input.reviewedById, reviewedAt: now, reviewNote: input.reviewNote, ledgerTransactionId: input.ledgerTransactionId };
      return adjustment;
    }),
...
describe("wallet adjustment API", () => {
  it("lets a merchant submit and list a shop wallet top-up request", async () => {
    ...
    const response = await request(fixture.app)
      ...
      .send({ type: "topup", amountNdp: 5000, idempotencyKey: "topup-api-001", bankReference: "BANK-001" })
      .expect(201);

    expect(response.body.data).toMatchObject({ ownerType: "shop", ownerId: 11, amountNdp: 5000, status: "pending" });
  ...
  it("lets platform operations list and approve a pending request", async () => {
    ...
    expect(response.body.data).toMatchObject({ id: 41, status: "approved", ledgerTransactionId: 51 });
