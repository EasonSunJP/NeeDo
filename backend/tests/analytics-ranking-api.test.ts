...
const fixture = (role = "operator", permissions = [permission]) => {
  ...
  const app = createApp(env, {
    ...
    auditLogRepository: { create: auditCreate }, analyticsRankingRepository: repository,
    analyticsRankingClock: () => now
  } as never);
  const token = new AuthTokenService(env).issueAccessToken({ id: user.id, email: user.email,
    currentIdentityId: user.identities[0].id, sessionGeneration: 0 }).token;
...
describe("formal analytics rankings API", () => {
  it.each(["admin", "operator"])("serves %s through the actual createApp chain", async (role) => {
    ...
    const response = await request(test.app)
      .get("/api/v1/backoffice/analytics/rankings/service?metric=gmv&period=last7days&page=1&pageSize=10")
      .set(bearer(test.token)).expect(200);
    expect(response.body.data).toMatchObject({ dataStatus: "ready", list: [], total: 0,
      page: 1, page_size: 10, filter: { kind: "service", metric: "gmv", period: "last7days" } });
  });
  ...
  it("enforces authentication, permission and strict path/query validation", async () => {
    ...
    await request(denied.app).get("/api/v1/backoffice/analytics/rankings/service")
      .set(bearer(denied.token)).expect(403);
  ...
  it("maps category 404 and incomplete evidence 409 without leaking evidence", async () => {
    ...
    const notFound = await request(missing.app)
      ...
      .set(bearer(missing.token)).expect(404);
    expect(notFound.body).toMatchObject({ code: ERROR_CODES.ANALYTICS_RANKING_CATEGORY_NOT_FOUND,
      message: "error.analytics_ranking.category_not_found", data: null });

    const corrupt = fixture();
    corrupt.repository.listRankings.mockRejectedValueOnce(new AnalyticsRankingIncompleteEvidenceError());
    const conflict = await request(corrupt.app)
      .get("/api/v1/backoffice/analytics/rankings/customer")
      .set(bearer(corrupt.token)).expect(409);
    expect(conflict.body).toEqual({ code: ERROR_CODES.ANALYTICS_RANKING_INCOMPLETE_EVIDENCE,
