  ...
  it("folds fixed aliases, keeps four zero-count rows, and aggregates normalized custom text", async () => {
    const groupBy = jest.fn(async () => [
      ...
      },
      {
        label: "魅力max",
        _count: { _all: 2 },
        _min: { createdAt: new Date("2026-02-01T00:00:00.000Z") }
      },
      {
        label: "Care",
        _count: { _all: 1 },
      ...
      },
      {
        label: "ｃａｒｅ",
        _count: { _all: 2 },
        _min: { createdAt: new Date("2026-01-03T00:00:00.000Z") }
      },
      {
        label: "手法細致",
        _count: { _all: 1 },
        _min: { createdAt: new Date("2026-01-04T00:00:00.000Z") }
      }
    ]);

    await expect(
      loadTechnicianReviewTagSummary({ orderReviewTag: { groupBy } } as never, 31)
    ).resolves.toEqual({
      special: [
