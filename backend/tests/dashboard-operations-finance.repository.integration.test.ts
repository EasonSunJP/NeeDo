...
export function assertOperationsFinanceIntegrationSchema(availableColumns: SchemaColumn[]): void {
        ...
        client.$transaction(
          async (tx) => {
            ...
            const [shop, otherCityShop] = await Promise.all([
              ...
              tx.shop.create({
                data: { name: `${marker}-osaka`, city: `${marker}-other`, address: "Osaka" }
              })
            ]);
            ...
            if (fixtureRateVersion > 2_147_483_647) {
              throw new Error(
                "Dashboard operations finance integration cannot allocate a fixture rate version"
              );
            }
              ...
              if (input.method === "NDP") {
                const ledger = await tx.ledgerTransaction.create({
                  data: {
                    ...
                    status: "APPLIED",
                    referenceType: input.invalidLedger
                      ? "dashboard_fixture"
            ...
            await expect(
              reader.getOperationsFinance({
                scope: { kind: "shop", shopId: shop.id },
                city: null,
                window
              })
