...
const scope = {
  servicePublicIds: [servicePublicId], categoryCodes: ["body-care"],
  excludedServicePublicIds: [], excludedCategoryCodes: [], activeFrom: null, activeTo: null
};
const draftInput = {
...
const version = {
  ...
  publishedAt: null,
  rules: draftInput.rules.map((rule, index) => ({ publicId: `rule-${index}`, ruleGroup: "base" as const, sortOrder: index, ...rule }))
};
const plan = {
  internalId: 31,
...
function repository(overrides: Partial<jest.Mocked<ShopMembershipCardPlanRepositoryPort>> = {}) {
  return {
    ...
    publishDraftWithAudit: jest.fn(async () => ({ kind: "published" as const, value: { ...plan, status: "active" as const, currentVersion: { ...version, status: "published" as const, platformFeePolicyPublicId: "fee-1", platformFeeRateBps: 1000 }, draftVersion: null } })),
    ...
    getFeePolicySummary: jest.fn(async () => ({ evaluatedAt: now, current: null, nextScheduled: null, latestVersion: 1 })),
    createFeePolicyVersionWithAudit: jest.fn(),
    ...overrides
  } as jest.Mocked<ShopMembershipCardPlanRepositoryPort>;
}

const audit = { createInput: jest.fn((input) => ({ actorId: input.actor.userId, action: input.action, targetType: input.targetType, metadata: input.metadata })) };
...
describe("ShopMembershipCardPlanService", () => {
  it("rejects non-shop identities before repository access", async () => {
    ...
    await expect(service.listPlans(owner({ currentIdentityScopeType: "global", currentIdentityScopeId: null }), { page: 1, pageSize: 20 }))
  ...
  it("rejects card-type fields that could silently add amount or uses", async () => {
    ...
    await expect(service.updateDraft(owner(), { ip: "127.0.0.1" }, planPublicId, {
      ...draftInput,
      issuance: { ...draftInput.issuance, minInitialUses: 1, maxInitialUses: 10 }
    })).rejects.toMatchObject({ message: "error.shop_membership_card_plan.invalid_card_fields", statusCode: 400 });
  ...
  it("validates service and category references inside the current shop before saving", async () => {
    const repo = repository({ validateShopRuleReferences: jest.fn(async (...args: [number, string[], string[]]) => {
      void args;
      return false;
    }) });
    const service = new ShopMembershipCardPlanService(repo, audit, () => now);

    await expect(service.updateDraft(owner(), { ip: "127.0.0.1" }, planPublicId, draftInput))
      .rejects.toMatchObject({ message: "error.shop_membership_card_plan.rule_reference_not_found", statusCode: 404 });
    expect(repo.validateShopRuleReferences).toHaveBeenCalledWith(71, [servicePublicId], ["body-care"]);
    expect(repo.updateDraftWithAudit).not.toHaveBeenCalled();
  });
  ...
  it("previews customer NDP and the extra platform fee without wallet mutation", async () => {
    ...
    await expect(service.previewPlan(owner(), planPublicId, {
    ...
    })).resolves.toMatchObject({ customerRewardNdp: 1000, platformFeeNdp: 100, totalShopDebitNdp: 1100 });
  ...
  it("publishes with the expected lock and safe audit metadata", async () => {
    ...
    await expect(service.publishPlan(owner(), { ip: "127.0.0.1" }, planPublicId, { expectedLockVersion: 2 }))
      .resolves.toMatchObject({ currentVersion: { status: "published", platformFeeRateBps: 1000 } });
    expect(repo.publishDraftWithAudit).toHaveBeenCalledWith(expect.objectContaining({
      actorId: 9,
      shopId: 71,
      planPublicId,
      expectedLockVersion: 2,
