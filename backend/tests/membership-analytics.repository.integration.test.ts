...
function requireSafeDatabaseUrl(): URL {
  ...
  return new URL(requireMembershipAnalyticsIntegrationAuthority({
...
describeIntegration("MembershipAnalyticsRepository against guarded local MySQL", () => {
  it("executes issuance, post-window void/expiry, in-window removal, provenance rejection, and rollback", async () => {
    ...
    try {
      ...
      const readBaseline = () => client.$queryRaw<Array<{
        userCount: bigint;
        shopCount: bigint;
      ...
      await expect(client.$transaction(async (tx) => {
        ...
        const createCard = async (input: {
          ...
          const initial = await tx.shopMembershipCardStatusEvent.create({
            data: {
              ...
              fromStatus: null,
              toStatus: "ACTIVE",
              source: input.authority === "migration_backfill" ? "MIGRATION_BACKFILL" : "ISSUANCE",
              occurredAt: input.issuedAt,
              reasonCode: input.authority === "migration_backfill"
        ...
        await assertCorruptionRejected(
          ...
          () => tx.shopMembershipCardStatusEvent.update({
            where: { id: issuanceId }, data: { reasonCode: "card_issued" }
          })
        );
        await assertCorruptionRejected(
          () => tx.shopMembershipCardStatusEvent.update({
            where: { id: issuanceId }, data: { actorUserId: null }
          }),
          () => tx.shopMembershipCardStatusEvent.update({
            where: { id: issuanceId }, data: { actorUserId: actor.id }
          })
        );
        await assertCorruptionRejected(
          () => tx.shopMembershipCardStatusEvent.update({
            where: { id: issuanceId }, data: { eventKey: `${marker}-wrong-issued` }
          }),
          () => tx.shopMembershipCardStatusEvent.update({
            where: { id: issuanceId },
        ...
        await assertCorruptionRejected(
          () => tx.shopMembershipCardStatusEvent.update({
            where: { id: issuanceId }, data: { source: "MIGRATION_BACKFILL" }
          }),
          () => tx.shopMembershipCardStatusEvent.update({
            where: { id: issuanceId }, data: { source: "ISSUANCE" }
          })
        );
        await assertCorruptionRejected(
        ...
        await assertCorruptionRejected(
          () => tx.shopMembershipCardStatusEvent.update({
            where: { id: migrationBackfill.initial.id }, data: { actorUserId: actor.id }
          }),
          () => tx.shopMembershipCardStatusEvent.update({
            where: { id: migrationBackfill.initial.id }, data: { actorUserId: null }
          })
        );
        await assertCorruptionRejected(
          () => tx.shopMembershipCardStatusEvent.update({
            where: { id: migrationBackfill.initial.id }, data: { reasonCode: "card_issued" }
          }),
          () => tx.shopMembershipCardStatusEvent.update({
            where: { id: migrationBackfill.initial.id }, data: { reasonCode: "historical_card_issued" }
          })
        );
        await assertCorruptionRejected(
          () => tx.shopMembershipCardStatusEvent.update({
            where: { id: migrationBackfill.initial.id }, data: { source: "ISSUANCE" }
          }),
          () => tx.shopMembershipCardStatusEvent.update({
            where: { id: migrationBackfill.initial.id }, data: { source: "MIGRATION_BACKFILL" }
          })
        );
        await assertCorruptionRejected(
          () => tx.shopMembershipCardStatusEvent.update({
            where: { id: migrationBackfill.initial.id }, data: { metadata: { issuanceSource: "offline_paid", extra: true } }
          }),
          () => tx.shopMembershipCardStatusEvent.update({
            where: { id: migrationBackfill.initial.id }, data: { metadata: { issuanceSource: "offline_paid" } }
          })
        );
        await assertCorruptionRejected(
          () => tx.shopMembershipCardStatusEvent.update({
            where: { id: migrationBackfill.initial.id }, data: { eventKey: `${marker}-wrong-backfill` }
          }),
          () => tx.shopMembershipCardStatusEvent.update({
