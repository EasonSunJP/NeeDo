...
import { MEMBERSHIP_ANALYTICS_ROUTE_PERMISSIONS } from "../src/routes/membership-analytics.routes";
...
const series: MembershipTrendSeries = [
  {
    ...
    label: "Added members",
    unit: "people",
    points: pointKeys.map((key) => ({ key, label: key.slice(5), value: 1 }))
  },
  {
    seriesKey: "removed",
    label: "Removed members",
    unit: "people",
    points: pointKeys.map((key) => ({ key, label: key.slice(5), value: 0 }))
  },
  {
    seriesKey: "net",
    label: "Net members",
...
describe("formal membership analytics API", () => {
  it("serves platform trend and paginated list through actual createApp routes", async () => {
    ...
    const list = await request(operator.app)
      .get(
        "/api/v1/backoffice/analytics/members?period=last7days&city=%E4%B8%9C%E4%BA%AC&needoId=u0000000041&nickname=%E7%BE%8E%E5%92%B2&page=1&pageSize=20"
      )
      .set(bearer(operator.token))
      .expect(200);
    ...
    expect(JSON.stringify(list.body)).not.toMatch(
      /customerProfileId|cardId|deletedAt|issuanceNote|issuanceReference/u
    );
    expect(operator.repository.listAddedMembers).toHaveBeenCalledWith(
  ...
  it("uses the merchant-account selected shop and rejects client scope overrides", async () => {
    const owner = fixture(
      makeUser({
        ...
        scopeId: 501,
        role: "merchant_owner",
        permissions: [MEMBERSHIP_ANALYTICS_ROUTE_PERMISSIONS.merchant]
      }),
      true
    );
    await request(owner.app)
      ...
      .expect(200);
    expect(owner.repository.getTrend).toHaveBeenCalledWith(
      expect.objectContaining({
    ...
    await request(owner.app)
      .get(
        "/api/v1/merchant-admin/analytics/members/trend?period=last30days&city=%E4%B8%9C%E4%BA%AC"
      )
      .set(bearer(owner.token))
      .expect(400);
    await request(owner.app)
  ...
  it("publishes incomplete authority as the stable 409 without a partial payload", async () => {
    ...
    operator.repository.getTrend.mockRejectedValueOnce(
      new MembershipAnalyticsIncompleteHistoryError()
    );
    await request(operator.app)
      .get("/api/v1/backoffice/analytics/members/trend")
      .set(bearer(operator.token))
