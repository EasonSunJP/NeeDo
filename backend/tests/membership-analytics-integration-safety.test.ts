...
describe("membership analytics MySQL integration safety", () => {
  ...
  it.each([
    [
      {
        enabled: undefined,
        envFile: "/tmp/formal.env",
        parsed: { DATABASE_URL: "mysql://x@127.0.0.1/needo_test" }
      },
      "explicit opt-in"
    ],
    [
      { enabled: "true", parsed: { DATABASE_URL: "mysql://x@127.0.0.1/needo_test" } },
      "FORMAL_BACKEND_ENV_FILE"
    ],
    [
      {
        enabled: "true",
        envFile: "/tmp/formal.env",
        parsed: {},
        runtime: { DATABASE_URL: "mysql://x@127.0.0.1/needo_test" }
      },
      "must define DATABASE_URL"
    ],
    [
      {
        enabled: "true",
        envFile: "/tmp/formal.env",
        parsed: { DATABASE_URL: "mysql://x@db.internal/needo_test" }
      },
      "loopback needo_test"
    ],
    [
      {
        enabled: "true",
        envFile: "/tmp/formal.env",
        parsed: { DATABASE_URL: "mysql://x@127.0.0.1/needo_dev" }
      },
      "loopback needo_test"
    ],
    [
      {
        enabled: "true",
        envFile: "/tmp/formal.env",
        parsed: { DATABASE_URL: "mysql://x@127.0.0.1/needo_test", NODE_ENV: "production" }
      },
      "non-production"
    ],
    [
  ...
  it("preflights every lifecycle query/fixture column and required Task2A index", () => {
    ...
    expect(() =>
      assertMembershipAnalyticsIntegrationSchema(columns, toAvailableIndexes(requiredIndexes), [
    ...
    expect(() => assertMembershipAnalyticsIntegrationSchema(columns, indexes, [])).toThrow(
      "20260901103000_membership_acquisition_sources"
    );

    expect(membershipAnalyticsIntegrationRequiredColumns).toEqual(
      expect.arrayContaining(["customer_profiles.city", "shop_membership_cards.frozen_at"])
    );
    expect(requiredIndexes).toEqual(
    ...
    for (const [label, corrupt] of [
      [
        ...
        (rows: typeof indexes) =>
          rows.map((row) =>
            row.indexName === "shop_membership_card_status_events_event_key"
              ? { ...row, tableName: "shop_membership_cards" }
              : row
          )
      ],
      [
        ...
        (rows: typeof indexes) =>
          rows.map((row) =>
            row.indexName === "shop_membership_card_status_events_card_time_idx" &&
            row.seqInIndex === 1
              ? { ...row, seqInIndex: 2 }
              : row
          )
      ...
      [
        ...
        (rows: typeof indexes) =>
          rows.map((row) =>
            row.indexName === "shop_membership_cards_status_expiry_idx" && row.seqInIndex === 2
              ? { ...row, columnName: "issued_at" }
              : row
          )
      ],
      [
      ...
      expect(() =>
        assertMembershipAnalyticsIntegrationSchema(columns, corrupt(indexes), [
          "20260901103000_membership_acquisition_sources"
        ])
      ).toThrow(label);
    }
