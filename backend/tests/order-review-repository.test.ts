...
const createHarness = (failure?: "tag" | "summary" | "shopSummary" | "audit") => {
  ...
  const order: any = {
    ...
    shop: { name: "Shop", ownerUserId: 303 },
  ...
  const withTags = (review: any) => ({
    ...review,
    tags: tags.filter((tag) => tag.orderReviewId === review.id && !tag.deletedAt)
  });
  const tx: any = {
    $queryRaw: jest.fn(async (parts: TemplateStringsArray) => {
      locks.push(
        String(parts[0]).includes("technician_profiles")
          ? "technician"
          : String(parts[0]).includes("customer_profiles")
            ? "customer"
            : String(parts[0]).includes("shops")
              ? "shop"
              : "order"
      );
      return [{ id: 1 }];
    ...
    technicianProfile: {
      findFirst: jest.fn(async ({ where }: any) =>
        where.id === order.technicianProfile.id && !order.technicianProfile.deletedAt
          ? { id: 702, userId: order.technicianProfile.userId }
          : null
      )
    },
    customerProfile: {
      findFirst: jest.fn(async ({ where }: any) =>
        (where.id === undefined || where.id === customerProfile.id) &&
        where.userId === customerProfile.userId &&
    ...
    orderCheckout: {
      findUnique: jest.fn(async ({ where }: any) =>
        where.bookingOrderId === checkout.bookingOrderId ? checkout : null
      )
    },
    ...
    orderReview: {
      findUnique: jest.fn(async ({ where, include }: any) => {
        ...
        if (where.idempotencyKey)
          found =
            reviews.find(
              (review) => collate(review.idempotencyKey) === collate(where.idempotencyKey)
            ) ?? null;
        if (where.bookingOrderId_reviewerUserId_targetType) {
          ...
          found =
            reviews.find(
              (review) =>
                review.bookingOrderId === key.bookingOrderId &&
                review.reviewerUserId === key.reviewerUserId &&
                review.targetType === key.targetType
            ) ?? null;
        }
      ...
      findFirst: jest.fn(async ({ where }: any) => {
        const found = reviews.find(
          (review) =>
            review.bookingOrderId === where.bookingOrderId &&
        ...
        reviews
          .filter(
            (review) =>
              ...
              !review.deletedAt &&
              (where.bookingOrder
                ? review.bookingOrderId === order.id &&
                  order.shopId === where.bookingOrder.shopId &&
                  !order.deletedAt
      ...
      create: jest.fn(async ({ data }: any) => {
        if (
          reviews.some((review) => collate(review.idempotencyKey) === collate(data.idempotencyKey))
        )
    ...
    orderReviewTag: {
      create: jest.fn(async ({ data }: any) => {
        ...
        const tag = { id: tags.length + 1, deletedAt: null, ...data };
        tags.push(tag);
        return tag;
      })
    },
    reviewSummary: {
      upsert: jest.fn(async ({ where, create, update }: any) => {
        ...
        if (failure === "shopSummary" && create.targetType === "shop")
          throw new Error("shop summary failed");
        const found = summaries.find(
          (summary) =>
            summary.targetType === where.targetType_targetId.targetType &&
            summary.targetId === where.targetType_targetId.targetId
        );
        if (found) Object.assign(found, update);
        else summaries.push({ id: summaries.length + 1, ...create });
    ...
    auditLog: {
      create: jest.fn(async ({ data }: any) => {
        ...
        audits.push({ id: audits.length + 1, ...data });
        return data;
      })
    }
  };
  const client: any = {
    ...
    $transaction: jest.fn(async (handler: (transaction: any) => Promise<unknown>) => {
      const snapshot = {
        ...
        summaries: summaries.map((item) => ({ ...item })),
      ...
      try {
        ...
        reviews.splice(0, reviews.length, ...snapshot.reviews);
        tags.splice(0, tags.length, ...snapshot.tags);
        summaries.splice(0, summaries.length, ...snapshot.summaries);
        audits.splice(0, audits.length, ...snapshot.audits);
        nextReviewId = snapshot.nextReviewId;
        throw error;
      }
    })
  };
...
describe("formal completed-order review repository", () => {
  it("applies one customer service review to both the technician and shop summaries in one transaction", async () => {
    ...
    await expect(h.repository.createOrderReview(h.input())).resolves.toMatchObject({
      outcome: "ok",
      applied: true,
      review: { targetType: "technician", rating: 5, tags: ["服务精神", "魅力值"] }
    });
  ...
  it("keeps the technician-to-customer direction independent from shop scoring", async () => {
    const h = createHarness();
    await expect(
      h.repository.createOrderReview(
        h.input({
          actorUserId: 202,
          actor: "technician",
          technicianProfileId: 702,
    ...
    ).resolves.toMatchObject({ outcome: "ok", applied: true, review: { targetType: "customer" } });
    expect(h.reviews).toHaveLength(1);
    expect(h.summaries).toEqual([
      expect.objectContaining({ targetType: "customer", targetId: 501, reviewCount: 1 })
    ]);
  ...
  it.each([
    ...
    "CANCELLED"
  ])("rejects ineligible state %s with zero writes", async (status) => {
    const h = createHarness();
    h.order.status = status;
    await expect(h.repository.createOrderReview(h.input())).resolves.toEqual({
      outcome: "invalid_state"
    });
    expect([h.reviews, h.tags, h.summaries, h.audits].map((items) => items.length)).toEqual([
      0, 0, 0, 0
    ]);
  });

  it("rejects malformed completed evidence, outsiders, deleted targets and self-review", async () => {
    const evidence = createHarness();
    evidence.checkout.ledgerTransactionId = null;
    await expect(evidence.repository.createOrderReview(evidence.input())).resolves.toEqual({
      outcome: "invalid_evidence"
    });
    const outsider = createHarness();
    await expect(
      outsider.repository.createOrderReview(outsider.input({ actorUserId: 999 }))
    ).resolves.toEqual({ outcome: "not_found" });
    const forgedActor = createHarness();
    await expect(
      forgedActor.repository.createOrderReview(
        forgedActor.input({
          actor: "operations",
          actorUserId: 202,
          technicianProfileId: 702,
    ...
    deleted.order.technicianProfile.deletedAt = new Date();
    await expect(deleted.repository.createOrderReview(deleted.input())).resolves.toEqual({
      outcome: "not_found"
    });
    const self = createHarness();
    self.order.technicianProfile.userId = 101;
    await expect(self.repository.createOrderReview(self.input())).resolves.toEqual({
      outcome: "not_found"
    });
  });
  ...
  it("replays only byte-exact equivalent input and rejects semantic, case and Unicode lookalikes", async () => {
    ...
    await expect(
      h.repository.createOrderReview(h.input({ requestFingerprint: "b".repeat(64) }))
    ).resolves.toEqual({ outcome: "conflict" });
    await expect(
      h.repository.createOrderReview(h.input({ idempotencyKey: "REVIEW-command-key-0001" }))
    ).resolves.toEqual({ outcome: "conflict" });
    await expect(
      h.repository.createOrderReview(h.input({ idempotencyKey: "ｒｅｖｉｅｗ-command-key-0001" }))
    ).resolves.toEqual({ outcome: "conflict" });
    expect(h.reviews).toHaveLength(1);
  ...
  it("returns already_submitted for the natural direction under a different key", async () => {
    ...
    await expect(
      h.repository.createOrderReview(
        h.input({ idempotencyKey: "review-command-key-9999", requestFingerprint: "c".repeat(64) })
      )
    ).resolves.toEqual({ outcome: "already_submitted" });
  });

  it.each(["tag", "summary", "shopSummary", "audit"] as const)(
    "rolls back review, tags, both summaries and audit when %s persistence fails",
    async (failure) => {
      const h = createHarness(failure);
      await expect(h.repository.createOrderReview(h.input())).rejects.toThrow(
        failure === "shopSummary" ? "shop summary failed" : `${failure} failed`
      );
      expect([h.reviews, h.tags, h.summaries, h.audits].map((items) => items.length)).toEqual([
        0, 0, 0, 0
      ]);
  ...
  it("GET mine returns only the actor's safe direction and excludes deleted rows", async () => {
    ...
    await h.repository.createOrderReview(h.input());
    await expect(
      h.repository.findOwnOrderReview({
        orderId: 41,
        actorUserId: 101,
        actor: "customer",
        technicianProfileId: null,
    ...
    ).resolves.toMatchObject({ outcome: "ok", review: { targetType: "technician", rating: 5 } });
    ...
    ).resolves.toEqual({ outcome: "ok", review: null });
  });
});
