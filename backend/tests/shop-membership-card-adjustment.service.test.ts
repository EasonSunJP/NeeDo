...
describe("ShopMembershipCardAdjustmentService", () => {
  it("submits a stored-value target without mutating or accepting a client before value", async () => {
    ...
    const result = await service.create(actor(), { ip: "127.0.0.1", userAgent: "jest" }, cardPublicId, {
      targetPrincipalBalanceJpy: 12_000,
      targetRemainingUses: null,
      reason: "  线下账目核对后修正  ",
    ...
    expect(repo.createRequestWithAuditAndNotification).toHaveBeenCalledWith(expect.objectContaining({
      actorId: 9,
      shopId: 71,
      cardPublicId,
      reason: "线下账目核对后修正",
      ...
      beforeRemainingUses: null,
      targetRemainingUses: null,
      audit: expect.objectContaining({ action: "merchant.shop_membership_card.adjustment.request" })
    }));
  });
  ...
  it("submits a count-card remaining target and rejects mixed or unchanged values", async () => {
    ...
    const repo = repository({ getMerchantCardContext: jest.fn(async () => ({ kind: "ready" as const, value: countCard })) });
    const service = new ShopMembershipCardAdjustmentService(repo, audit, () => now);
    ...
    })).resolves.toMatchObject({ dimension: "remaining_uses", beforeValue: 4, targetValue: 6, difference: 2 });
    await expect(service.create(actor(), requestContext, cardPublicId, {
      targetPrincipalBalanceJpy: null,
  ...
  it("rejects benefit cards and non-shop merchant scope before mutation", async () => {
    const benefitRepo = repository({ getMerchantCardContext: jest.fn(async () => ({ kind: "ready" as const, value: { ...record().card, type: "benefit" as const } })) });
    ...
    const forbiddenService = new ShopMembershipCardAdjustmentService(forbiddenRepo, audit, () => now);
    await expect(forbiddenService.create(actor({ currentIdentityScopeType: "global", currentIdentityScopeId: null }), requestContext, cardPublicId, {
      targetPrincipalBalanceJpy: 1,
      targetRemainingUses: null,
      reason: "无店铺范围",
  ...
  it("replays only the same normalized creation request", async () => {
    ...
    const input = { targetPrincipalBalanceJpy: 12_000, targetRemainingUses: null, reason: "修正", idempotencyKey: "adjustment-replay-001" };
    ...
    const existing = record({ requestFingerprint: createdInput.requestFingerprint, reason: "修正" });
    ...
    await expect(replayService.create(actor(), requestContext, cardPublicId, input)).resolves.toMatchObject({ replayed: true, publicId: requestPublicId });
    expect(replayRepo.getMerchantCardContext).not.toHaveBeenCalled();

    const conflictRepo = repository({ findByRequestIdempotencyKey: jest.fn(async () => record({ requestFingerprint: "different" })) });
    await expect(new ShopMembershipCardAdjustmentService(conflictRepo, audit, () => now).create(actor(), requestContext, cardPublicId, input))
      .rejects.toMatchObject({ code: ERROR_CODES.SHOP_MEMBERSHIP_CARD_ADJUSTMENT_IDEMPOTENCY_CONFLICT, statusCode: 409 });
  });

  it.each(["approve", "reject"] as const)("allows only the owning customer to %s and maps terminal results", async (decision) => {
    const repo = repository();
    const service = new ShopMembershipCardAdjustmentService(repo, audit, () => now);
    const customer = actor({ currentIdentityType: "customer", currentIdentityScopeType: "customer_profile", currentIdentityScopeId: 51, userId: 41, roles: ["customer"], permissions: ["customer-profile:read"] });

    await expect(service.decide(customer, { ip: "127.0.0.1" }, requestPublicId, {
      decision,
      idempotencyKey: `decision-${decision}-001`
    })).resolves.toMatchObject({ status: decision === "approve" ? "approved" : "rejected", replayed: false });
    expect(repo.decideRequestWithAuditAndNotification).toHaveBeenCalledWith(expect.objectContaining({
      customerUserId: 41,
      requestPublicId,
      decision,
      audit: expect.objectContaining({ action: `customer.shop_membership_card.adjustment.${decision}` })
    }));
  });
  ...
  it("maps expiry and snapshot invalidation without presenting a successful approval", async () => {
    ...
    for (const kind of ["expired", "invalidated"] as const) {
      const repo = repository({ decideRequestWithAuditAndNotification: jest.fn(async () => ({
        kind,
        value: record({ status: kind, invalidatedAt: kind === "invalidated" ? now : null })
      })) });
      const service = new ShopMembershipCardAdjustmentService(repo, audit, () => now);
      await expect(service.decide(customer, requestContext, requestPublicId, { decision: "approve", idempotencyKey: `decision-${kind}-001` }))
        .rejects.toMatchObject({
          code: kind === "expired" ? ERROR_CODES.SHOP_MEMBERSHIP_CARD_ADJUSTMENT_EXPIRED : ERROR_CODES.SHOP_MEMBERSHIP_CARD_ADJUSTMENT_SNAPSHOT_CONFLICT,
          statusCode: 409
        });
    }
  });
  ...
  it("lets the scoped shop cancel a pending request", async () => {
    const repo = repository();
    const service = new ShopMembershipCardAdjustmentService(repo, audit, () => now);
    await expect(service.cancel(actor(), { ip: "127.0.0.1" }, requestPublicId))
      .resolves.toMatchObject({ status: "cancelled", replayed: false });
    expect(repo.cancelRequestWithAuditAndNotification).toHaveBeenCalledWith(expect.objectContaining({
      shopId: 71,
      actorId: 9,
      requestPublicId,
