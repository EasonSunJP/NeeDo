...
const createFixture = () => {
  ...
  const repository = {
    ...
    createOrderReview: jest.fn(
      async (input: { actorUserId: number; targetType: string; idempotencyKey: string }) => {
        if (input.actorUserId === 999) return { outcome: "not_found" } as const;
        if (input.idempotencyKey === "review-conflict-key-01")
          return { outcome: "conflict" } as const;
        return {
          outcome: "ok",
          applied: true,
          review: { ...review, targetType: input.targetType }
        } as const;
      }
    ),
...
describe("formal completed-order review API", () => {
  ...
  it("runs both participant directions through the real router/auth/RBAC/validator/controller", async () => {
    ...
    const customer = await request(fixture.app)
      ...
      .send({
        ...
        tags: ["服务精神"],
        comment: "很好",
        idempotencyKey: "review-customer-key-001"
      })
      .expect(200);
    expect(customer.body.data).toEqual(
      expect.objectContaining({
        applied: true,
        review: expect.objectContaining({ targetType: "technician" })
      })
    );
    await request(fixture.app)
      ...
      .send({
        ...
        tags: ["礼貌友好"],
        comment: null,
        idempotencyKey: "review-technician-key-1"
      })
      .expect(200);
  });
  ...
  it("enforces strict validation, authentication, least privilege, and hidden participant mismatch", async () => {
    ...
    await request(fixture.app)
      ...
      .send({
        ...
        tags: [],
        comment: null,
        idempotencyKey: "review-invalid-key-001",
        extra: true
      })
      .expect(400);
    ...
    await request(fixture.app)
      ...
      .send({
        ...
        tags: [],
        comment: null,
        idempotencyKey: "review-forbidden-key-01"
      })
      .expect(403);
    await request(fixture.app)
      .post(path)
      ...
      .send({
        ...
        tags: [],
        comment: null,
        idempotencyKey: "review-merchant-key-001"
      })
      .expect(403);
    await request(fixture.app)
      .post(path)
      ...
      .send({
        ...
        tags: [],
        comment: null,
        idempotencyKey: "review-outsider-key-001"
      })
      .expect(404);
  });
  ...
  it("isolates GET mine and whitelists the response", async () => {
    ...
    const response = await request(fixture.app)
      .get("/api/v1/orders/41/reviews/mine")
      .set("Authorization", "Bearer customer")
      .expect(200);
    expect(response.body.data).toEqual({
      review: expect.objectContaining({ targetType: "technician", rating: 5 })
    });
    expect(JSON.stringify(response.body.data)).not.toMatch(
      /reviewer|profileId|idempotency|fingerprint|audit|bookingOrderId/i
    );
    await request(fixture.app)
      .get("/api/v1/orders/41/reviews/mine")
      .set("Authorization", "Bearer outsider")
  ...
  it("maps idempotency conflict to 409", async () => {
    ...
    await request(fixture.app)
      ...
      .send({
        ...
        tags: [],
        comment: null,
        idempotencyKey: "review-conflict-key-01"
      })
      .expect(409);
  });
