...
const migration = readFileSync(
  resolve(backendRoot, "prisma/migrations/20260831000000_social_reply_relation/migration.sql"),
  "utf8"
);
const schema = readFileSync(resolve(backendRoot, "prisma/schema.prisma"), "utf8");
const checker = readFileSync(
  resolve(backendRoot, "scripts/check-social-reply-relations.ts"),
  "utf8"
);
...
describe("Social reply relation migration", () => {
  ...
  it("ships a local-only preflight and postflight checker", () => {
    expect(packageJson.scripts["check:social-reply-relations"]).toContain(
      "check-social-reply-relations.ts"
    );
...
describe("Social reply relation checker safety", () => {
  ...
  it.each([
    [
      "an encoded production-like database name",
      { ...localRuntime, DATABASE_URL: "mysql://needo:needo@127.0.0.1:3307/needo%5Fprod" },
      "rejects production-like database names"
    ],
    [
      "malformed database-name encoding",
      { ...localRuntime, DATABASE_URL: "mysql://needo:needo@127.0.0.1:3307/needo%ZZ" },
      "DATABASE_URL database name must use valid URL encoding"
    ],
    [
      "a production NODE_ENV",
      { ...localRuntime, NODE_ENV: "production" },
      "rejects staging and production environments"
    ],
    [
      "a staging DEPLOY_ENV",
      { ...localRuntime, DEPLOY_ENV: "staging" },
      "rejects staging and production environments"
    ],
    [
      "a non-loopback database host",
      { ...localRuntime, DATABASE_URL: "mysql://needo:needo@db.example.com/needo_dev" },
      "only accepts a loopback MySQL host"
    ]
  ])(
    "rejects %s before Prisma, database access, or snapshot writes",
    (_case, environment, message) => {
