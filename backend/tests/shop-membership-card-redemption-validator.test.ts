...
describe("shop membership card redemption validators", () => {
  it("accepts only a server-scoped completed order reference and idempotency key", () => {
    expect(shopMembershipCardRedemptionCreateBodySchema.parse({
  ...
  it("validates public ids and bounded pagination", () => {
    ...
    expect(shopMembershipCardRedemptionPublicIdParamSchema.parse({ publicId })).toEqual({ publicId });
    expect(shopMembershipCardRedemptionCandidateQuerySchema.parse({ page: "2", pageSize: "50" }))
      .toEqual({ page: 2, pageSize: 50 });
    expect(shopMembershipCardRedemptionListQuerySchema.parse({ page: "1", cardPublicId: publicId }))
