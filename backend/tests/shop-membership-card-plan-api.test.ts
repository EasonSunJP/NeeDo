...
function repository(overrides: Partial<jest.Mocked<ShopMembershipCardPlanRepositoryPort>> = {}) {
  return {
    ...
    createFeePolicyVersionWithAudit: jest.fn(async () => ({
...
function fixture(
  ...
  repositoryOverrides: Partial<jest.Mocked<ShopMembershipCardPlanRepositoryPort>> = {}
) {
...
const merchantUser = (permissions: string[]) =>
  makeUser({
    id: 90,
...
const operationsUser = (permissions: string[]) =>
  makeUser({
    id: 91,
...
describe("shop membership card plan APIs", () => {
  it("requires authentication and exact merchant permissions", async () => {
    ...
    await request(readOnly.app)
      .get("/api/v1/merchant-admin/shop-membership-card-plans?page=1&pageSize=20")
  ...
  it("rejects client scope, fee, and unknown reward fields", async () => {
    ...
    await request(owner.app)
      ...
      .send({ ...draftBody, shopId: 999 })
      .expect(400);
    await request(owner.app)
      .post("/api/v1/merchant-admin/shop-membership-card-plans")
      .set("Authorization", `Bearer ${owner.token}`)
      .send({ ...draftBody, platformFeeRateBps: 0 })
      .expect(400);
    await request(owner.app)
      .post("/api/v1/merchant-admin/shop-membership-card-plans")
      .set("Authorization", `Bearer ${owner.token}`)
      .send({ ...draftBody, rules: [{ ...draftBody.rules[0], giftName: "free service" }] })
      .expect(400);
  ...
  it("returns paginated public plans and server-authoritative reward cost preview", async () => {
    ...
    expect(list.body.data).toMatchObject({
      ...
      page: 1,
      page_size: 20,
      list: [{ publicId: planPublicId }]
    });
    expect(list.body.data.list[0]).not.toHaveProperty("internalId");

    const preview = await request(owner.app)
      .post(`/api/v1/merchant-admin/shop-membership-card-plans/${planPublicId}/preview`)
      .set("Authorization", `Bearer ${owner.token}`)
      .send(previewBody)
      .expect(200);
  ...
  it("returns 404 for a plan outside the shop and 409 for a stale draft lock", async () => {
    const missing = fixture(merchantUser(["shop.member.card_plan.view"]), {
      findPlan: jest.fn(async (...args: [number, string]) => {
        void args;
        return null;
      })
    });
    await request(missing.app)
      .get(`/api/v1/merchant-admin/shop-membership-card-plans/${planPublicId}`)
      .set("Authorization", `Bearer ${missing.token}`)
      .expect(404);

    const stale = fixture(merchantUser(["shop.member.card_plan.manage"]), {
      updateDraftWithAudit: jest.fn(
        async (
          ...args: Parameters<ShopMembershipCardPlanRepositoryPort["updateDraftWithAudit"]>
        ) => {
          void args;
    ...
    await request(stale.app)
      .patch(`/api/v1/merchant-admin/shop-membership-card-plans/${planPublicId}/draft`)
      .set("Authorization", `Bearer ${stale.token}`)
      .send({ ...draftBody, expectedLockVersion: 9 })
      .expect(409);
  });
  ...
  it("publishes with its dedicated permission and returns the immutable fee snapshot", async () => {
    ...
    const response = await request(publisher.app)
      .post(`/api/v1/merchant-admin/shop-membership-card-plans/${planPublicId}/publish`)
  ...
  it("protects fee history and immutable version creation with operations permissions", async () => {
    ...
    const response = await request(reader.app)
      .get("/api/v1/backoffice/membership-reward-fee-policy?page=1&pageSize=20")
      .set("Authorization", `Bearer ${reader.token}`)
      .expect(200);
    expect(response.body.data).toMatchObject({
      summary: { latestVersion: 1 },
      history: { total: 1, page_size: 20 }
    });
    const futureEffectiveFrom = "2099-08-31T03:00:00.000Z";
    await request(reader.app)
