...
const repository = (overrides: Partial<ShopMembershipCardAdjustmentRepositoryPort> = {}) =>
  ({
    ...
    listMerchantRequests: jest.fn(async () => ({
      list: [record()],
      total: 1,
      page: 1,
      page_size: 20
    })),
    listCustomerRequests: jest.fn(async () => ({
      list: [record()],
      total: 1,
      page: 1,
      page_size: 20
...
function fixture(
  ...
  permissions: string[],
  overrides: Partial<ShopMembershipCardAdjustmentRepositoryPort> = {}
) {
...
describe("shop membership card adjustment API", () => {
  ...
  it("returns paginated merchant history and supports cancellation", async () => {
    const merchant = fixture("merchant", ["shop.member.card.adjust.request"]);
    const list = await request(merchant.app)
      .get(`${merchantListPath}?page=1&pageSize=20&status=pending`)
      .set("Authorization", `Bearer ${merchant.token}`)
      .expect(200);
    expect(list.body.data).toMatchObject({
      ...
      page: 1,
      page_size: 20,
      list: [expect.objectContaining({ publicId: requestPublicId })]
    });
    await request(merchant.app)
      .post(`${merchantListPath}/${requestPublicId}/cancel`)
      .set("Authorization", `Bearer ${merchant.token}`)
      .send({})
      .expect(200);
  });
  ...
  it("lets only the customer identity list and decide its own requests", async () => {
    ...
    const result = await request(customer.app)
      .post(decisionPath)
      .set("Authorization", `Bearer ${customer.token}`)
      .send({ decision: "approve", idempotencyKey: "adjustment-decision-api-001" })
      .expect(200);
    ...
    expect(customer.repository.decideRequestWithAuditAndNotification).toHaveBeenCalledWith(
      expect.objectContaining({ customerUserId: 41, decision: "approve" })
    );
  });
});
