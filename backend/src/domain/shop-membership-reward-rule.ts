...
const categoryCodeListSchema = z
...
export const membershipRewardScopeSchema = z
  ...
  .superRefine((scope, context) => {
    if (
      scope.activeFrom &&
      scope.activeTo &&
      new Date(scope.activeFrom) >= new Date(scope.activeTo)
    ) {
      context.addIssue({
...
const fixedPerCompletionSchema = z
  .object({
...
const percentOfEligibleAmountSchema = z
  .object({
...
const firstCardUseBonusSchema = z
  .object({
...
const serviceScopeBonusSchema = z
  .object({
...
const completionMilestoneBonusSchema = z
  .object({
...
const spendMilestoneBonusSchema = z
  .object({
...
const birthdayMonthBonusSchema = z
  .object({
...
const scheduleWindowBonusSchema = z
  .object({
...
const consecutiveMonthBonusSchema = z
  .object({
...
export const membershipRewardRuleSchema = rawMembershipRewardRuleSchema.superRefine(
  (rule, context) => {
...
const baseKinds = new Set(["fixed_per_completion", "percent_of_eligible_amount", "spend_block"]);

export const membershipRewardRuleListSchema = z
  .array(membershipRewardRuleSchema)
  .min(1)
  .max(21)
  .superRefine((rules, context) => {
    ...
    if (rules.length - baseCount > 20) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message: "at most 20 bonus rules are allowed"
      });
    }
  });

export const membershipRewardCapsSchema = z
  .object({
...
function scopeMatches(
  rule: MembershipRewardRuleInput,
  facts: MembershipRewardPreviewFacts
): boolean {
  const { scope } = rule;
  if (facts.servicePublicId && scope.excludedServicePublicIds.includes(facts.servicePublicId))
    return false;
...
function ruleReward(
  rule: MembershipRewardRuleInput,
  facts: MembershipRewardPreviewFacts
): { reward: number; basis: MembershipRewardRuleHit["basis"] } {
  switch (rule.kind) {
    ...
    case "spend_block": {
      ...
      return {
        ...
        basis: {
          eligibleAmountJpy: facts.eligibleAmountJpy,
          blockAmountJpy: rule.blockAmountJpy,
          blocks
        }
      };
    }
    ...
    case "service_scope_bonus":
      return typeof rule.rewardNdp === "number" && rule.rewardNdp > 0
        ...
        : {
            ...
            basis: {
              eligibleAmountJpy: facts.eligibleAmountJpy,
              rewardRateBps: rule.rewardRateBps ?? 0
            }
          };
    case "completion_milestone_bonus": {
      const completedAfter = facts.completedCountBefore + 1;
      const hitCount = rule.repeat
        ? completedAfter % rule.everyCompletions === 0
          ? 1
          : 0
        : completedAfter === rule.everyCompletions
          ? 1
          : 0;
      return {
    ...
    case "spend_milestone_bonus": {
      const spendAfter = checkedNumber(
        BigInt(facts.lifetimeEligibleSpendJpyBefore) + BigInt(facts.eligibleAmountJpy)
      );
      const hits = rule.repeat
        ? Math.floor(spendAfter / rule.thresholdJpy) -
          Math.floor(facts.lifetimeEligibleSpendJpyBefore / rule.thresholdJpy)
        : facts.lifetimeEligibleSpendJpyBefore < rule.thresholdJpy &&
            spendAfter >= rule.thresholdJpy
          ? 1
          : 0;
      return {
    ...
    case "birthday_month_bonus": {
      const japanMonth = Number(
        new Intl.DateTimeFormat("en-US", { timeZone: "Asia/Tokyo", month: "numeric" }).format(
          new Date(facts.occurredAt)
        )
      );
      const eligible =
        facts.customerBirthMonth === japanMonth && facts.birthdayRewardsThisYear < rule.annualLimit;
    ...
    case "schedule_window_bonus": {
      ...
      return {
        reward: eligible ? rule.rewardNdp : 0,
        basis: { localDay: local.day, localMinutes: local.minutes, eligible }
      };
    }
    case "consecutive_month_bonus": {
      const alreadyRewarded = facts.rewardedConsecutiveMonthMilestones.includes(
        rule.consecutiveMonths
      );
      const eligible =
        facts.consecutiveEligibleMonths >= rule.consecutiveMonths && !alreadyRewarded;
      return {
        reward: eligible ? rule.rewardNdp : 0,
        basis: { consecutiveMonths: facts.consecutiveEligibleMonths, alreadyRewarded }
      };
    }
  }
}
...
export function evaluateMembershipRewardRules(input: {
  ...
  const rawHits = rules.flatMap((rule, ruleIndex) => {
    ...
    return result.reward > 0
      ? [{ ruleIndex, kind: rule.kind, basis: result.basis, rewardNdp: result.reward }]
      : [];
  });
  ...
  const platformFeeNdp =
    customerRewardNdp === 0 ? 0 : multiplyCeil(customerRewardNdp, platformFeeRateBps, 10_000);
...
export function summarizeMembershipRewardRule(ruleInput: unknown): string {
  ...
  switch (rule.kind) {
    case "fixed_per_completion":
      return `${rule.rewardNdp} NDP / completion`;
    case "percent_of_eligible_amount":
      return `${rule.rewardRateBps} bps of eligible amount`;
    case "spend_block":
      return `${rule.rewardNdpPerBlock} NDP / ${rule.blockAmountJpy} JPY`;
    case "first_card_use_bonus":
      return `${rule.rewardNdp} NDP first-use bonus`;
    case "service_scope_bonus":
      return rule.rewardNdp
        ? `${rule.rewardNdp} NDP service bonus`
        : `${rule.rewardRateBps ?? 0} bps service bonus`;
    case "completion_milestone_bonus":
      return `${rule.rewardNdp} NDP every ${rule.everyCompletions} completions`;
    case "spend_milestone_bonus":
      return `${rule.rewardNdp} NDP at ${rule.thresholdJpy} JPY`;
    case "birthday_month_bonus":
      return `${rule.rewardNdp} NDP birthday-month bonus`;
    case "schedule_window_bonus":
      return `${rule.rewardNdp} NDP schedule bonus`;
    case "consecutive_month_bonus":
      return `${rule.rewardNdp} NDP at ${rule.consecutiveMonths} consecutive months`;
  }
}
