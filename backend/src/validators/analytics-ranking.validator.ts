...
const canonicalPositiveInteger = (maximum: number) =>
  z
    .union([
...
export const analyticsRankingParamsSchema = z
  .object({
    kind: z.enum(["service", "technician", "customer"])
  })
  .strict();

export const analyticsRankingQuerySchema = dashboardQueryBaseSchema
  .extend({
