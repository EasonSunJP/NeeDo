...
export class ShopMembershipCardPlanController {
  ...
  public list = this.handle(async (request, response) => {
    response.status(200).json(successResponse(await this.service.listPlans(
  ...
  public get = this.handle(async (request, response) => {
    ...
    response.status(200).json(successResponse(await this.service.getPlan(getAuthenticatedAccess(response), publicId)));
  });

  public create = this.handle(async (request, response) => {
    response.status(201).json(successResponse(await this.service.createPlan(
      getAuthenticatedAccess(response),
  ...
  public preview = this.handle(async (request, response) => {
    const { publicId } = shopMembershipCardPlanPublicIdParamSchema.parse(request.params);
    response.status(200).json(successResponse(await this.service.previewPlan(
      getAuthenticatedAccess(response),
      publicId,
  ...
  public publish = this.handle(async (request, response) => {
    const { publicId } = shopMembershipCardPlanPublicIdParamSchema.parse(request.params);
    response.status(201).json(successResponse(await this.service.publishPlan(
      getAuthenticatedAccess(response),
      getRequestContext(request),
      publicId,
  ...
  public retire = this.handle(async (request, response) => {
    ...
    const { publicId } = shopMembershipCardPlanPublicIdParamSchema.parse(request.params);
    response.status(200).json(successResponse(await this.service.retirePlan(
      getAuthenticatedAccess(response),
      getRequestContext(request),
      publicId
    )));
  ...
  public createFeePolicyVersion = this.handle(async (request, response) => {
    response.status(201).json(successResponse(await this.service.createFeePolicyVersion(
      getAuthenticatedAccess(response),
      getRequestContext(request),
