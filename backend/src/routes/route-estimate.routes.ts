...
export const createRouteEstimateRoutes = (config: AppConfig, dependencies: AppDependencies): Router => {
  ...
  const provider = dependencies.routeDistanceProvider ?? (config.TRAVEL_ROUTE_PROVIDER === "geoapify" && config.GEOAPIFY_API_KEY
    ? new GeoapifyRouteDistanceProvider({ apiBaseUrl: config.GEOAPIFY_API_BASE_URL, apiKey: config.GEOAPIFY_API_KEY, timeoutMs: config.TRAVEL_ROUTE_TIMEOUT_MS, maxRetries: config.TRAVEL_ROUTE_MAX_RETRIES })
  ...
  const service = new RouteEstimateService(
    ...
    { estimateTtlSeconds: config.TRAVEL_ESTIMATE_TTL_SECONDS, cacheTtlSeconds: config.TRAVEL_ROUTE_CACHE_TTL_SECONDS }
  );
  const controller = new RouteEstimateController(service);
  router.post("/bookings/travel-estimates", authenticate(), createAuthorizeMiddleware(TRAVEL_FARE_PERMISSIONS.estimateCreate), validateRequest({ body: routeEstimateCreateBodySchema }), controller.create);
  return router;
};
