...
type SupportedContractLanguage = "zh-CN" | "ja" | "en";

const EFFECTIVE_AT = new Date("2026-08-26T00:00:00.000Z");
const VERSIONS: Readonly<Record<ContractType, string>> = {
  merchant: "merchant-2026-08-26-v1",
  affiliate: "affiliate-2026-08-26-v1"
};

const AFFILIATE_TEXT: Readonly<Record<SupportedContractLanguage, string>> = {
...
const MERCHANT_TEXT: Readonly<Record<SupportedContractLanguage, string>> = {
...
const textCatalog: Readonly<
  Record<ContractType, Readonly<Record<SupportedContractLanguage, string>>>
> = {
...
export class NeedoContractCatalogService implements ContractCatalogPort {
  public async getCurrent(type: ContractType, language: string): Promise<ContractDefinition> {
    ...
    const text = textCatalog[type][language];
    return {
      type,
