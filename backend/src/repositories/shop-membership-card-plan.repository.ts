...
export interface ShopMembershipCardPlanRepositoryPort {
  listPlans: (shopId: number, input: PaginationInput) => Promise<PaginatedResponse<ShopMembershipCardPlanPayload>>;
  findPlan: (shopId: number, publicId: string) => Promise<ShopMembershipCardPlanPayload | null>;
  createPlanWithDraft: (input: { actorId: number; shopId: number; draft: ShopMembershipCardPlanDraftPersistenceInput; audit: AuditLogCreateInput }) => Promise<ShopMembershipCardPlanPayload>;
  updateDraftWithAudit: (input: PlanWriteInput & { draft: ShopMembershipCardPlanDraftPersistenceInput }) => Promise<PlanMutationResult>;
  publishDraftWithAudit: (input: PlanWriteInput & { expectedLockVersion: number }) => Promise<PlanMutationResult>;
  retirePlanWithAudit: (input: PlanWriteInput) => Promise<PlanMutationResult>;
  validateShopRuleReferences: (shopId: number, servicePublicIds: string[], categoryCodes: string[]) => Promise<boolean>;
  getEffectiveFeePolicy: (at: Date) => Promise<MembershipRewardFeePolicyPayload | null>;
  listFeePolicies: (input: PaginationInput) => Promise<PaginatedResponse<MembershipRewardFeePolicyPayload>>;
  getFeePolicySummary: (at: Date) => Promise<MembershipRewardFeePolicySummaryPayload>;
  createFeePolicyVersionWithAudit: (input: { actorId: number; feeRateBps: number; expectedVersion: number; effectiveFrom: Date; reason: string; audit: AuditLogCreateInput }) => Promise<FeePolicyMutationResult>;
}
...
const versionSelect = Prisma.validator<Prisma.ShopMembershipCardPlanVersionSelect>()({
  ...
  rules: { where: { deletedAt: null }, orderBy: [{ sortOrder: "asc" }, { id: "asc" }], select: ruleSelect }
});
...
type VersionRecord = Prisma.ShopMembershipCardPlanVersionGetPayload<{ select: typeof versionSelect }>;
...
type FeePolicyRecord = Prisma.MembershipRewardFeePolicyVersionGetPayload<{ select: typeof feePolicySelect }>;
...
export class ShopMembershipCardPlanRepository implements ShopMembershipCardPlanRepositoryPort {
  ...
  public async listPlans(shopId: number, input: PaginationInput): Promise<PaginatedResponse<ShopMembershipCardPlanPayload>> {
    const pagination = toPrismaPagination(input);
    const where: Prisma.ShopMembershipCardPlanWhereInput = { shopId, deletedAt: null };
    const [records, total] = await Promise.all([
      this.client.shopMembershipCardPlan.findMany({ where, orderBy: [{ updatedAt: "desc" }, { id: "desc" }], skip: pagination.skip, take: pagination.take, select: planSelect }),
      this.client.shopMembershipCardPlan.count({ where })
    ]);
    return buildPaginatedResponse(records.map((record) => this.mapPlan(record)), total, input);
  }

  public async findPlan(shopId: number, publicId: string): Promise<ShopMembershipCardPlanPayload | null> {
    const record = await this.client.shopMembershipCardPlan.findFirst({ where: { publicId, shopId, deletedAt: null }, select: planSelect });
    return record ? this.mapPlan(record) : null;
  }

  public async createPlanWithDraft(input: { actorId: number; shopId: number; draft: ShopMembershipCardPlanDraftPersistenceInput; audit: AuditLogCreateInput }): Promise<ShopMembershipCardPlanPayload> {
    const created = await this.client.$transaction(async (transaction) => {
      const plan = await transaction.shopMembershipCardPlan.create({
        data: { shopId: input.shopId, status: ShopMembershipCardPlanStatus.DRAFT, createdById: input.actorId, updatedById: input.actorId },
      ...
      await transaction.shopMembershipRewardRule.createMany({ data: this.ruleCreateManyData(version.id, input.draft.rules) });
      await transaction.auditLog.create({ data: toAuditLogCreateData({ ...input.audit, targetId: plan.id, metadata: { ...(this.metadataObject(input.audit.metadata)), planPublicId: plan.publicId, planVersionPublicId: version.publicId, version: 1 } }) });
  ...
  public async updateDraftWithAudit(input: PlanWriteInput & { draft: ShopMembershipCardPlanDraftPersistenceInput }): Promise<PlanMutationResult> {
    const result = await this.client.$transaction(async (transaction) => {
      const plan = await transaction.shopMembershipCardPlan.findFirst({ where: { publicId: input.planPublicId, shopId: input.shopId, deletedAt: null }, select: planSelect });
      if (!plan) return { kind: "not_found" as const };
      if (plan.status === ShopMembershipCardPlanStatus.RETIRED) return { kind: "invalid_state" as const };
      let draft = plan.versions[0];
      if (!draft) {
        const nextVersion = (plan.currentVersion?.version ?? 0) + 1;
        draft = await transaction.shopMembershipCardPlanVersion.create({ data: this.versionCreateData(plan.id, nextVersion, input.draft), select: versionSelect });
        await transaction.shopMembershipRewardRule.createMany({ data: this.ruleCreateManyData(draft.id, input.draft.rules) });
      } else {
        if (draft.lockVersion !== input.draft.expectedLockVersion) return { kind: "version_conflict" as const };
        const updated = await transaction.shopMembershipCardPlanVersion.updateMany({
          where: { id: draft.id, status: ShopMembershipCardPlanVersionStatus.DRAFT, lockVersion: input.draft.expectedLockVersion, deletedAt: null },
        ...
        await transaction.shopMembershipRewardRule.updateMany({ where: { planVersionId: draft.id, deletedAt: null }, data: { deletedAt } });
        await transaction.shopMembershipRewardRule.createMany({ data: this.ruleCreateManyData(draft.id, input.draft.rules) });
      }
      await transaction.shopMembershipCardPlan.update({ where: { id: plan.id }, data: { updatedById: input.actorId } });
      await transaction.auditLog.create({ data: toAuditLogCreateData({ ...input.audit, targetId: plan.id, metadata: { ...this.metadataObject(input.audit.metadata), planPublicId: plan.publicId, version: draft.version } }) });
  ...
  public async publishDraftWithAudit(input: PlanWriteInput & { expectedLockVersion: number }): Promise<PlanMutationResult> {
    const at = this.now();
    const result = await this.client.$transaction(async (transaction) => {
      const plan = await transaction.shopMembershipCardPlan.findFirst({ where: { publicId: input.planPublicId, shopId: input.shopId, deletedAt: null }, select: planSelect });
      if (!plan) return { kind: "not_found" as const };
      ...
      if (draft.lockVersion !== input.expectedLockVersion) return { kind: "version_conflict" as const };
      const feePolicy = await transaction.membershipRewardFeePolicyVersion.findFirst({
        where: { deletedAt: null, effectiveFrom: { lte: at }, OR: [{ effectiveTo: null }, { effectiveTo: { gt: at } }] },
      ...
      const published = await transaction.shopMembershipCardPlanVersion.updateMany({
        where: { id: draft.id, status: ShopMembershipCardPlanVersionStatus.DRAFT, lockVersion: input.expectedLockVersion, deletedAt: null },
        data: { status: ShopMembershipCardPlanVersionStatus.PUBLISHED, draftKey: null, platformFeePolicyId: feePolicy.id, platformFeeRateBps: feePolicy.feeRateBps, publishedById: input.actorId, publishedAt: at, lockVersion: { increment: 1 } }
      });
      ...
      const planUpdated = await transaction.shopMembershipCardPlan.updateMany({
        ...
        data: { status: ShopMembershipCardPlanStatus.ACTIVE, currentVersionId: draft.id, updatedById: input.actorId }
      });
      if (planUpdated.count !== 1) return { kind: "version_conflict" as const };
      await transaction.auditLog.create({ data: toAuditLogCreateData({ ...input.audit, targetId: plan.id, metadata: { ...this.metadataObject(input.audit.metadata), planPublicId: plan.publicId, planVersionPublicId: draft.publicId, version: draft.version, platformFeePolicyPublicId: feePolicy.publicId, platformFeeRateBps: feePolicy.feeRateBps } }) });
  ...
  public async retirePlanWithAudit(input: PlanWriteInput): Promise<PlanMutationResult> {
    const result = await this.client.$transaction(async (transaction) => {
      const plan = await transaction.shopMembershipCardPlan.findFirst({ where: { publicId: input.planPublicId, shopId: input.shopId, deletedAt: null }, select: { id: true, publicId: true, status: true, currentVersionId: true } });
      if (!plan) return { kind: "not_found" as const };
      if (plan.status === ShopMembershipCardPlanStatus.RETIRED) return { kind: "invalid_state" as const };
      await transaction.shopMembershipCardPlan.update({ where: { id: plan.id }, data: { status: ShopMembershipCardPlanStatus.RETIRED, updatedById: input.actorId } });
      if (plan.currentVersionId) await transaction.shopMembershipCardPlanVersion.update({ where: { id: plan.currentVersionId }, data: { status: ShopMembershipCardPlanVersionStatus.RETIRED } });
      await transaction.auditLog.create({ data: toAuditLogCreateData({ ...input.audit, targetId: plan.id, metadata: { ...this.metadataObject(input.audit.metadata), planPublicId: plan.publicId } }) });
  ...
  public async validateShopRuleReferences(shopId: number, servicePublicIds: string[], categoryCodes: string[]): Promise<boolean> {
    const [serviceCount, categoryCount] = await Promise.all([
      servicePublicIds.length === 0 ? 0 : this.client.service.count({ where: { shopId, publicId: { in: servicePublicIds }, deletedAt: null } }),
      categoryCodes.length === 0 ? 0 : this.client.category.count({ where: { code: { in: categoryCodes }, deletedAt: null, services: { some: { shopId, deletedAt: null } } } })
    ]);
  ...
  public async listFeePolicies(input: PaginationInput): Promise<PaginatedResponse<MembershipRewardFeePolicyPayload>> {
    const pagination = toPrismaPagination(input);
    const where = { deletedAt: null } as const;
    const [records, total] = await Promise.all([
      this.client.membershipRewardFeePolicyVersion.findMany({ where, orderBy: [{ version: "desc" }], skip: pagination.skip, take: pagination.take, select: feePolicySelect }),
      this.client.membershipRewardFeePolicyVersion.count({ where })
    ]);
    return buildPaginatedResponse(records.map((record) => this.mapFeePolicy(record)), total, input);
  }
  ...
  public async getFeePolicySummary(at: Date): Promise<MembershipRewardFeePolicySummaryPayload> {
    const [current, next, latest] = await Promise.all([
      this.client.membershipRewardFeePolicyVersion.findFirst({ where: { deletedAt: null, effectiveFrom: { lte: at }, OR: [{ effectiveTo: null }, { effectiveTo: { gt: at } }] }, orderBy: [{ effectiveFrom: "desc" }, { version: "desc" }], select: feePolicySelect }),
      this.client.membershipRewardFeePolicyVersion.findFirst({ where: { deletedAt: null, effectiveFrom: { gt: at } }, orderBy: [{ effectiveFrom: "asc" }, { version: "asc" }], select: feePolicySelect }),
      this.client.membershipRewardFeePolicyVersion.findFirst({ where: { deletedAt: null }, orderBy: { version: "desc" }, select: { version: true } })
    ]);
    return { evaluatedAt: at, current: current ? this.mapFeePolicy(current) : null, nextScheduled: next ? this.mapFeePolicy(next) : null, latestVersion: latest?.version ?? 0 };
  }

  public async createFeePolicyVersionWithAudit(input: { actorId: number; feeRateBps: number; expectedVersion: number; effectiveFrom: Date; reason: string; audit: AuditLogCreateInput }): Promise<FeePolicyMutationResult> {
    const now = this.now();
    try {
      return await this.client.$transaction(async (transaction) => {
        const latest = await transaction.membershipRewardFeePolicyVersion.findFirst({ where: { deletedAt: null }, orderBy: { version: "desc" }, select: feePolicySelect });
        if ((latest?.version ?? 0) !== input.expectedVersion) return { kind: "version_conflict" as const };
        const current = await transaction.membershipRewardFeePolicyVersion.findFirst({ where: { deletedAt: null, effectiveFrom: { lte: now }, OR: [{ effectiveTo: null }, { effectiveTo: { gt: now } }] }, orderBy: [{ effectiveFrom: "desc" }, { version: "desc" }], select: feePolicySelect });
        if (current && current.effectiveFrom >= input.effectiveFrom) return { kind: "policy_conflict" as const };
        if (current) {
          await transaction.membershipRewardFeePolicyVersion.updateMany({
            where: { id: current.id, version: current.version, deletedAt: null },
            data: { activeKey: null, effectiveTo: input.effectiveFrom, status: input.effectiveFrom <= now ? MembershipRewardFeePolicyStatus.SUPERSEDED : MembershipRewardFeePolicyStatus.ACTIVE, updatedById: input.actorId }
          });
        }
        const created = await transaction.membershipRewardFeePolicyVersion.create({
          data: { version: input.expectedVersion + 1, feeRateBps: input.feeRateBps, status: MembershipRewardFeePolicyStatus.ACTIVE, effectiveFrom: input.effectiveFrom, effectiveTo: null, activeKey: input.effectiveFrom <= now ? "membership_reward" : null, reason: input.reason, createdById: input.actorId, updatedById: input.actorId },
          select: feePolicySelect
        });
        await transaction.auditLog.create({ data: toAuditLogCreateData({ ...input.audit, targetId: created.id, metadata: { ...this.metadataObject(input.audit.metadata), previous: current ? { feeRateBps: current.feeRateBps, version: current.version } : null, next: { feeRateBps: created.feeRateBps, version: created.version }, effectiveFrom: created.effectiveFrom.toISOString(), reason: created.reason } }) });
        return { kind: "created" as const, value: this.mapFeePolicy(created) };
      });
    } catch (error) {
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") return { kind: "version_conflict" };
      throw error;
    }
  }

  private versionCreateData(planId: number, version: number, draft: ShopMembershipCardPlanDraftPersistenceInput): Prisma.ShopMembershipCardPlanVersionUncheckedCreateInput {
    return { planId, version, status: ShopMembershipCardPlanVersionStatus.DRAFT, draftKey: `plan:${planId}`, ...this.versionMutableData(draft) };
  }
  ...
  private ruleCreateManyData(planVersionId: number, rules: MembershipRewardRuleInput[]): Prisma.ShopMembershipRewardRuleCreateManyInput[] {
    return rules.map((rule, sortOrder) => ({
      planVersionId,
      kind: ruleKindToDb[rule.kind],
      ruleGroup: baseKinds.has(rule.kind) ? ShopMembershipRewardRuleGroup.BASE : ShopMembershipRewardRuleGroup.BONUS,
  ...
  private mapVersion(record: VersionRecord): ShopMembershipCardPlanVersionPayload {
    const validity: ShopMembershipCardPlanValidityPayload = record.validityMode === ShopMembershipCardPlanValidityMode.NEVER
      ? { mode: "never" }
  ...
  private mapFeePolicy(record: FeePolicyRecord): MembershipRewardFeePolicyPayload {
    return { publicId: record.publicId, version: record.version, feeRateBps: record.feeRateBps, effectiveFrom: record.effectiveFrom, effectiveTo: record.effectiveTo, reason: record.reason, createdByNeedoId: record.createdBy?.needoId ?? null, createdAt: record.createdAt };
  }
