...
type AdjustmentRecord = Prisma.ShopMembershipCardAdjustmentRequestGetPayload<{ select: typeof adjustmentSelect }>;
...
export class ShopMembershipCardAdjustmentRepository implements ShopMembershipCardAdjustmentRepositoryPort {
  ...
  public async findByRequestIdempotencyKey(shopId: number, idempotencyKey: string): Promise<ShopMembershipCardAdjustmentRecord | null> {
  ...
  public async findByDecisionIdempotencyKey(customerUserId: number, idempotencyKey: string): Promise<ShopMembershipCardAdjustmentRecord | null> {
    const record = await this.client.shopMembershipCardAdjustmentRequest.findFirst({
      where: {
        decisionIdempotencyKey: idempotencyKey,
        card: { membership: { customerProfile: { userId: customerUserId }, deletedAt: null }, deletedAt: null },
        deletedAt: null
      },
      select: adjustmentSelect
    });
  ...
  public async getMerchantCardContext(shopId: number, cardPublicId: string): Promise<ShopMembershipCardAdjustmentContextResult> {
    ...
    try {
      return await runWithTransactionConflictRetry(() => this.client.$transaction(async (transaction) => {
        const existing = await transaction.shopMembershipCardAdjustmentRequest.findFirst({
          where: { shopId: input.shopId, requestIdempotencyKey: input.requestIdempotencyKey, deletedAt: null },
          select: adjustmentSelect
        });
        if (existing) {
        ...
        const recipientIdentityId = await resolveCanonicalPersonalIdentityId(transaction, card.membership.customerProfile.user.id);
      ...
      if (targets.some((target) => target.includes("request_idempotency_key"))) {
        const existing = await this.findByRequestIdempotencyKey(input.shopId, input.requestIdempotencyKey);
        if (!existing) throw error;
        return existing.requestFingerprint === input.requestFingerprint
          ? { kind: "replayed", value: existing }
          : { kind: "idempotency_conflict" };
      }
      if (targets.some((target) => target.includes("pending_key"))) return { kind: "pending_conflict" };
      throw error;
    }
  }

  public async decideRequestWithAuditAndNotification(input: DecideShopMembershipCardAdjustmentRepositoryInput): Promise<ShopMembershipCardAdjustmentDecisionResult> {
    try {
      return await runWithTransactionConflictRetry(() => this.client.$transaction(async (transaction) => {
        const replay = await transaction.shopMembershipCardAdjustmentRequest.findFirst({
          where: {
            ...
            card: { membership: { customerProfile: { userId: input.customerUserId }, deletedAt: null }, deletedAt: null },
            deletedAt: null
          },
          select: adjustmentSelect
        });
        ...
        if (!this.matchesDecisionSnapshot(request, databaseNow)) {
          const invalidated = await this.invalidateLockedRequest(transaction, request, input, databaseNow);
        ...
        await transaction.auditLog.create({
          data: {
            ...toAuditLogCreateData({ ...input.audit, targetId: request.id, metadata: { ...this.metadata(input.audit.metadata), requestPublicId: request.publicId } }),
            createdAt: databaseNow
          }
        });
        ...
        return { kind: input.decision === "approve" ? "approved" as const : "rejected" as const, value: this.mapAdjustment(updated) };
      }));
    } catch (error) {
      if (!this.isUniqueConflict(error)) throw error;
      const existing = await this.findByDecisionIdempotencyKey(input.customerUserId, input.decisionIdempotencyKey);
      if (!existing) throw error;
  ...
  public async cancelRequestWithAuditAndNotification(input: CancelShopMembershipCardAdjustmentRepositoryInput): Promise<ShopMembershipCardAdjustmentCancelResult> {
    return runWithTransactionConflictRetry(() => this.client.$transaction(async (transaction) => {
      const locked = await this.lockRequest(transaction, input.requestPublicId);
      if (!locked) return { kind: "not_found" as const };
      const databaseNow = await this.getDatabaseNow(transaction);
      const request = await transaction.shopMembershipCardAdjustmentRequest.findFirst({
        where: { id: locked, publicId: input.requestPublicId, shopId: input.shopId, deletedAt: null },
        select: adjustmentSelect
      });
      if (!request) return { kind: "not_found" as const };
      if (request.status === ShopMembershipCardAdjustmentStatus.CANCELLED) return { kind: "replayed" as const, value: this.mapAdjustment(request) };
      if (request.status !== ShopMembershipCardAdjustmentStatus.PENDING) return { kind: "invalid_state" as const };
      if (request.expiresAt <= databaseNow) {
        const expired = await this.expireLockedRequest(transaction, request, databaseNow);
        void expired;
      ...
      const transitioned = await transaction.shopMembershipCardAdjustmentRequest.updateMany({
        where: { id: request.id, status: ShopMembershipCardAdjustmentStatus.PENDING, pendingKey: request.pendingKey, expiresAt: { gt: databaseNow }, deletedAt: null },
        data: { status: ShopMembershipCardAdjustmentStatus.CANCELLED, pendingKey: null, cancelledAt: databaseNow, cancelledById: input.actorId }
      });
      if (transitioned.count !== 1) throw this.transactionStateConflict();
      await transaction.auditLog.create({ data: { ...toAuditLogCreateData({ ...input.audit, targetId: request.id }), createdAt: databaseNow } });
      await this.notifyOne(transaction, {
      ...
      return { kind: "cancelled" as const, value: this.mapAdjustment(await this.findAdjustmentById(transaction, request.id)) };
    }));
  }

  private async loadMerchantCard(client: AdjustmentClient, shopId: number, cardPublicId: string): Promise<CardRecord | null> {
  ...
  private async lockRequest(client: Prisma.TransactionClient, publicId: string): Promise<number | null> {
  ...
  private async applyCardTarget(client: Prisma.TransactionClient, request: AdjustmentRecord): Promise<boolean> {
    if (request.beforePrincipalBalanceJpy !== null && request.targetPrincipalBalanceJpy !== null) {
      const update = await client.shopMembershipCard.updateMany({
        ...
        data: { principalBalanceJpy: request.targetPrincipalBalanceJpy, lockVersion: { increment: 1 } }
      });
      return update.count === 1;
    }
    if (request.beforeRemainingUses === null || request.targetRemainingUses === null || request.card.totalUses === null) return false;
    const targetTotalUses = request.card.totalUses + request.targetRemainingUses - request.beforeRemainingUses;
    if (!Number.isSafeInteger(targetTotalUses) || targetTotalUses < 0 || targetTotalUses > 2_147_483_647) return false;
    const update = await client.shopMembershipCard.updateMany({
      where: {
        id: request.card.id,
        lockVersion: request.cardLockVersionBefore,
      ...
      data: { remainingUses: request.targetRemainingUses, totalUses: targetTotalUses, lockVersion: { increment: 1 } }
    });
    return update.count === 1;
  }
  ...
  private async expireLockedRequest(client: Prisma.TransactionClient, request: AdjustmentRecord, databaseNow: Date): Promise<ShopMembershipCardAdjustmentRecord> {
    ...
    await client.auditLog.create({
      data: {
        ...
        targetId: request.id,
        metadata: { requestPublicId: request.publicId, cardPublicId: request.card.publicId, expiresAt: request.expiresAt.toISOString(), expiredAt: databaseNow.toISOString() },
        createdAt: databaseNow
      }
    });
  ...
  private async invalidateLockedRequest(
    ...
    const update = await client.shopMembershipCardAdjustmentRequest.updateMany({
      where: { id: request.id, status: ShopMembershipCardAdjustmentStatus.PENDING, pendingKey: request.pendingKey, deletedAt: null },
      data: {
    ...
    await client.auditLog.create({
      data: {
        ...
        targetType: "ShopMembershipCardAdjustmentRequest",
        targetId: request.id,
        metadata: { requestPublicId: request.publicId, cardPublicId: request.card.publicId, expectedLockVersion: request.cardLockVersionBefore, actualLockVersion: request.card.lockVersion },
        createdAt: databaseNow
      }
    });
  ...
  private async notifyBothSystem(client: Prisma.TransactionClient, request: AdjustmentRecord, state: "expired" | "invalidated", createdAt: Date): Promise<void> {
  ...
  private async notifyOne(client: Prisma.TransactionClient, input: {
    ...
    const recipientIdentityId = await resolveCanonicalPersonalIdentityId(client, input.recipientUserId);
    const actorIdentityId = input.actorUserId === null ? null : await resolveCanonicalPersonalIdentityId(client, input.actorUserId);
    if (!recipientIdentityId || (input.actorUserId !== null && !actorIdentityId)) this.throwIdentityNotFound();
  ...
  private async findAdjustmentById(client: AdjustmentClient, id: number): Promise<AdjustmentRecord> {
    const record = await client.shopMembershipCardAdjustmentRequest.findUnique({ where: { id }, select: adjustmentSelect });
  ...
  private matchesCreateSnapshot(card: CardRecord, input: CreateShopMembershipCardAdjustmentRepositoryInput): boolean {
    if (card.lockVersion !== input.cardLockVersionBefore) return false;
    if (card.type === ShopMembershipCardType.STORED_VALUE) {
      return card.principalBalanceJpy === input.beforePrincipalBalanceJpy && input.targetPrincipalBalanceJpy !== null &&
        input.beforeRemainingUses === null && input.targetRemainingUses === null;
    }
    if (card.type === ShopMembershipCardType.COUNT) {
      return card.remainingUses === input.beforeRemainingUses && input.targetRemainingUses !== null &&
        input.beforePrincipalBalanceJpy === null && input.targetPrincipalBalanceJpy === null;
    }
    return false;
  }

  private matchesDecisionSnapshot(request: AdjustmentRecord, databaseNow: Date): boolean {
    if (!this.isCardAdjustable(request.card, databaseNow) || request.card.lockVersion !== request.cardLockVersionBefore) return false;
    if (request.beforePrincipalBalanceJpy !== null) return request.card.principalBalanceJpy === request.beforePrincipalBalanceJpy;
    return request.beforeRemainingUses !== null && request.card.remainingUses === request.beforeRemainingUses;
  }
  ...
  private mapCard(record: CardRecord): ShopMembershipCardAdjustmentCardContext {
    return {
      ...
      name: record.name,
      type: record.type === ShopMembershipCardType.STORED_VALUE ? "stored_value" : record.type === ShopMembershipCardType.COUNT ? "count" : "benefit",
      status: record.status === ShopMembershipCardStatus.ACTIVE ? "active" : record.status === ShopMembershipCardStatus.FROZEN ? "frozen" : record.status === ShopMembershipCardStatus.EXPIRED ? "expired" : "void",
  ...
  private requireIdentityPair(recipientIdentityId: number | null, actorIdentityId: number | null): void {
