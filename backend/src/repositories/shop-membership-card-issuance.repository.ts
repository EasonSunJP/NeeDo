...
type ContextClient =
  | Pick<PrismaClient, "shopCustomerMembership" | "shopMembershipCardPlan">
  | Pick<Prisma.TransactionClient, "shopCustomerMembership" | "shopMembershipCardPlan">;
...
export class ShopMembershipCardIssuanceRepository implements ShopMembershipCardIssuanceRepositoryPort {
  ...
  ): Promise<IssuedMembershipCardRecord | null> {
  ...
  public async issueCardWithAuditAndNotification(
    ...
    try {
      return await this.client.$transaction(
        async (transaction) => {
          ...
          if (existing) {
            return existing.membership.shopId === input.shopId &&
              existing.issuanceFingerprint === input.issuanceFingerprint
        ...
        `);
          if (
            ...
            lockedUsers[0]?.id !== context.membership.customerUserId
          ) {
            return { kind: "invalid_state" as const };
          }
          ...
          if (postLockExisting) {
            return postLockExisting.membership.shopId === input.shopId &&
              postLockExisting.issuanceFingerprint === input.issuanceFingerprint
          ...
          const isFirstPaidSource =
            input.issuanceSource === "offline_paid" || input.issuanceSource === "online_paid";
          if (
            (isFirstPaidSource && priorPaidCard) ||
            (input.issuanceSource === "renewal" && !priorPaidCard)
          ) {
            return { kind: "invalid_state" as const };
          }
          ...
          const recipientIdentityId = await resolveCanonicalPersonalIdentityId(
            transaction,
            context.membership.customerUserId
          );
      ...
      if (targets.some((target) => target.includes("card_no")))
        return { kind: "card_number_conflict" };
      throw error;
    }
  }
  ...
  private async loadIssuanceContext(
    ...
    const validity = this.validityFromDb(
      version.validityMode,
      version.validityDays,
      version.fixedExpiryAt
    );
    ...
    return {
      ...
      value: {
        ...
        shop: {
          internalId: membership.shop.id,
          shopNo: membership.shop.shopNo,
  ...
  private mapCard(record: IssuedCardRecord): IssuedMembershipCardRecord {
    if (
      !record.plan ||
      !record.planVersion ||
      !record.issuanceSource ||
      record.platformFeeRateBpsSnapshot === null ||
      !record.issuanceFingerprint
    ) {
      throw new AppError({
  ...
  private cardTypeFromDb(type: ShopMembershipCardType): ShopMembershipCardIssuanceTypePayload {
    return type === ShopMembershipCardType.STORED_VALUE
      ? "stored_value"
      : type === ShopMembershipCardType.COUNT
        ? "count"
        : "benefit";
  }

  private sourceToDb(
    source: ShopMembershipCardIssuanceSourcePayload
  ): ShopMembershipCardIssuanceSource {
    const values: Record<
      ShopMembershipCardIssuanceSourcePayload,
      ShopMembershipCardIssuanceSource
  ...
  private sourceFromDb(
    source: ShopMembershipCardIssuanceSource
  ): ShopMembershipCardIssuanceSourcePayload {
    const values: Record<
      ShopMembershipCardIssuanceSource,
      ShopMembershipCardIssuanceSourcePayload
  ...
  private cardStatusFromDb(
    status: ShopMembershipCardStatus
  ): "active" | "frozen" | "expired" | "void" {
    return status === ShopMembershipCardStatus.ACTIVE
      ? "active"
      : status === ShopMembershipCardStatus.FROZEN
        ? "frozen"
        : status === ShopMembershipCardStatus.EXPIRED
          ? "expired"
          : "void";
  }

  private validityFromDb(
    mode: ShopMembershipCardPlanValidityMode,
    days: number | null,
    fixedExpiryAt: Date | null
  ): ShopMembershipCardIssuanceContext["version"]["validity"] | null {
    if (mode === ShopMembershipCardPlanValidityMode.NEVER)
      return days === null && fixedExpiryAt === null ? { mode: "never" } : null;
    if (mode === ShopMembershipCardPlanValidityMode.FIXED_DAYS)
      return days && days > 0 && fixedExpiryAt === null ? { mode: "fixed_days", days } : null;
