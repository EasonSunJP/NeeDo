...
export function assertFormalDatabaseSchema(evidence: FormalDatabaseSchemaEvidence): void {
  ...
  if (evidence.columnCollations["order_review_tags.label"] !== "utf8mb4_bin") {
    throw new Error(
      "Formal order schema preflight failed: order_review_tags.label must use utf8mb4_bin"
    );
  }
}
...
export function assertDeepSnapshotEqual(before: unknown, after: unknown, label: string): void {
  if (serializeEvidence(before) !== serializeEvidence(after)) {
...
async function createCheckerFixture(
  ...
  const customer = await tx.user.create({
    data: {
      ...
      username: "Formal flow customer",
      isTestAccount: true
    }
  });
  const technician = await tx.user.create({
    data: {
      needoId: `u${technicianAccountNo}`,
      accountNo: technicianAccountNo,
      primaryIdentityType: "U",
      email: `${marker}-technician@example.invalid`,
      username: "Formal flow technician",
      isTestAccount: true
    }
  });
  const customerProfile = await tx.customerProfile.create({
    data: {
      userId: customer.id,
      displayName: "Formal flow customer",
      city: "Tokyo"
    }
  });
  const shop = await tx.shop.create({
    data: {
      name: `Formal flow shop ${marker}`,
      city: "Tokyo",
  ...
  const technicianProfile = await tx.technicianProfile.create({
    data: {
      ...
      shopId: shop.id,
      displayName: "Formal flow technician",
      city: "Tokyo"
    }
  });
  ...
  const addOn = await tx.service.create({
    data: {
      categoryId: category.id,
      shopId: shop.id,
      technicianProfileId: technicianProfile.id,
  ...
  const rate = await tx.ndpExchangeRateRule.create({
    data: {
      version: (latestRate._max.version ?? 0) + 1,
      ndpUnits: 1,
      jpyUnits: 1,
      status: "ACTIVE",
      ...
      reason: "Rollback-only formal flow check"
    }
  });
  if (previousRates.some((row) => row.effectiveFrom >= now)) {
    throw new Error(
      "Formal order checker requires no future active rate inside its isolated transaction"
    );
  }
  return {
    marker,
...
async function createConfirmedOrder(
  ...
  const order = await tx.bookingOrder.create({
    data: {
      orderNo: `${fixture.marker}-${sequence}`,
  ...
  if (walletBalance > 0) {
    await tx.wallet.upsert({
      where: {
        ownerType_ownerId_currency: { ownerType: "USER", ownerId: fixture.customer.id, currency }
      },
      create: {
        ownerType: "USER",
...
export async function captureOrderMutationState(tx: Prisma.TransactionClient, orderId: number) {
  const [
    order,
    session,
    addOns,
    ...
    tx.affiliateRewardTransaction.findMany({
      where: { reward: { bookingOrderId: orderId } },
      orderBy: { id: "asc" }
    }),
...
async function createFormalFlowHarness(tx: Prisma.TransactionClient, now: Date) {
  ...
  const service = new bookingModule.BookingService(
    new bookingRepositoryModule.BookingRepository(facade),
    ledger,
    undefined,
    audit,
    undefined,
  ...
  const customer: AuthenticatedAccessContext = {
    ...
    accessTokenExpiresAt: Math.floor(now.getTime() / 1000) + 600,
    roles: ["customer"],
    permissions: [
      "order:detail:read",
      "order:service:start",
  ...
  const technician: AuthenticatedAccessContext = {
    ...
    email: fixture.technician.email,
...
async function runFormalFlow(tx: Prisma.TransactionClient): Promise<void> {
  ...
  const proposalInput = {
    ...
    idempotencyKey: `${fixture.marker}-ndp-addon`
  };
  ...
  const endInput = {
    reason: "completed",
    idempotencyKey: `${fixture.marker}-ndp-end`
  };
  ...
  assert(
    paid.status === "completed" &&
      paid.paymentEvidence === "ndp_ledger" &&
      paidReplay.id === paid.id,
    "NDP settlement evidence/replay mismatch"
  );
  assert(
    !validatorModule.orderReviewCreateBodySchema.safeParse({
    ...
    "review validator accepted a Unicode full-fold collision"
  );
  assert(
    !validatorModule.orderReviewCreateBodySchema.safeParse({
      targetType: "technician",
    ...
    "review validator accepted a final-sigma collision"
  );
  ...
  const customerReview = await service.createOrderReview(
    customer,
    ndpOrder.id,
    customerReviewInput,
    context
  );
  const customerReviewReplay = await service.createOrderReview(
    customer,
    ndpOrder.id,
    customerReviewInput,
    context
  ...
  assert(
    customerReview.applied && !customerReviewReplay.applied,
  ...
  await service.createOrderReview(
    technician,
    ndpOrder.id,
    {
      targetType: "customer",
  ...
  await assertRejectedWithMessage(
    () =>
      service.createOrderReview(
        ...
        {
          ...customerReviewInput,
          idempotencyKey: `${fixture.marker}-customer-review-second`
        },
        context
      ),
  ...
  assert(
    ndpEvidence?.status === "COMPLETED" && ndpEvidence.statusHistory.length === 4,
    "NDP order history is not coherent"
  );
  assert(
    ndpEvidence.serviceSession?.events.length === 7 &&
  ...
  assert(
    rateSnapshot.ruleId === fixture.rateId &&
      rateSnapshot.version === fixture.rateVersion &&
      rateSnapshot.ndpUnits === 1 &&
      rateSnapshot.jpyUnits === 1 &&
  ...
  assertFormalFulfillmentChain(
    ...
    },
    {
      ...
      session: {
        ...
        startedByUserId: fixture.customer.id,
        startedAt: ndpStartedEvent.occurredAt,
        expectedEndsAt: new Date(ndpStartedEvent.occurredAt.getTime() + 90 * 60_000),
        endedByUserId: fixture.customer.id,
        endedAt: ndpEndedEvent.occurredAt,
        deletedAt: null
      },
      checkout: {
        id: persistedCheckout.id,
        bookingOrderId: ndpOrder.id,
        paymentMethod: "NDP",
        createdAt: ndpCheckoutEvent.occurredAt,
        paymentSelectedAt: ndpSelectionEvent.occurredAt,
        ledgerTransactionId: paymentTransaction.id,
      ...
      addOn: {
        id: ndpAddOn.id,
        bookingOrderId: ndpOrder.id,
        serviceSessionId: ndpSession.id,
        serviceId: fixture.addOnServiceId,
        status: "ACCEPTED",
        ...
        proposedByUserId: fixture.customer.id,
        proposedAt: ndpProposedEvent.occurredAt,
        acceptedByUserId: fixture.technician.id,
        acceptedAt: ndpAcceptedEvent.occurredAt,
      ...
      histories: [
        ...
        },
        {
          ...
          createdAt: ndpStartedEvent.occurredAt
        },
        {
          ...
          actorUserId: fixture.customer.id,
          reason: "completed",
          createdAt: ndpEndedEvent.occurredAt
        },
        {
          fromStatus: "AWAITING_CHECKOUT",
          toStatus: "COMPLETED",
          actorUserId: fixture.customer.id,
      ...
      events: [
        {
          ...
          actorUserId: fixture.customer.id,
          idempotencyKey: ndpStart.idempotencyKey,
          reason: null,
          metadata: {
            actor: "customer",
        ...
        },
        {
          ...
          actorUserId: fixture.technician.id,
          idempotencyKey: acceptInput.idempotencyKey,
          reason: null,
          metadata: {
            actor: "technician",
            requestIp: context.ip,
        ...
        },
        {
          ...
          actorUserId: fixture.customer.id,
          idempotencyKey: endInput.idempotencyKey,
          reason: "completed",
          metadata: {
            actor: "customer",
            requestIp: context.ip,
        ...
        },
        {
          ...
          serviceSessionId: ndpSession.id,
          orderAddOnId: null,
          orderCheckoutId: persistedCheckout.id,
  ...
  const paymentAudit = await tx.auditLog.findFirst({
    where: {
      ...
      targetId: paymentTransaction.id,
      actorId: fixture.customer.id,
      deletedAt: null
    }
  });
  ...
  assert(
    technicianTargetReview?.reviewerUserId === fixture.customer.id &&
      technicianTargetReview.technicianProfileId === fixture.technicianProfileId &&
      technicianTargetReview.customerProfileId === null &&
      technicianTargetReview.rating === 5 &&
  ...
  assert(
    customerTargetReview?.reviewerUserId === fixture.technician.id &&
      customerTargetReview.customerProfileId === fixture.customerProfileId &&
      customerTargetReview.technicianProfileId === null &&
      customerTargetReview.rating === 5 &&
  ...
  const cashEndInput = {
    reason: "completed",
    idempotencyKey: `${fixture.marker}-cash-end`
  };
  ...
  const cashSelectionInput = {
    method: "cash",
    idempotencyKey: `${fixture.marker}-cash-select`
  } as const;
  ...
  assert(
    awaitingPaymentConfirmation.status === "awaitingPaymentConfirmation",
    "cash selection did not enter payment confirmation"
  );
  const receiptInput = {
    reason: "cash received",
    idempotencyKey: `${fixture.marker}-cash-receipt`
  };
  ...
  assert(
    cashCompleted.status === "completed" &&
      cashCompleted.paymentEvidence === "technician_receipt_confirmation" &&
      cashReplay.id === cashCompleted.id,
    "cash receipt evidence/replay mismatch"
  );
  await assertRejectedWithMessage(
    () =>
  ...
  assert(
    cashEvidence?.status === "COMPLETED" &&
      cashEvidence.statusHistory.length === 5 &&
  ...
  assert(
    cashEvidence.checkout?.paymentMethod === "CASH" &&
      cashEvidence.checkout.ledgerTransactionId === null &&
      cashEvidence.checkout.receiptConfirmedById === fixture.technician.id &&
      cashEvidence.financial?.settlementStatus === "settled" &&
      cashEvidence.reviews.length === 2,
    "cash receipt/settlement/review chain is not coherent"
  );
  const cashFinancial = cashEvidence.financial!;
  const cashTimeline = Array.isArray(cashFinancial.moneyTimelineJson)
    ? cashFinancial.moneyTimelineJson.map(asRecord)
    : [];
  ...
  assert(
    cashFinancial.serviceAmountJpy === 11_000 &&
      ...
      cashFinancial.serviceIncomeReportedById === fixture.technician.id &&
      cashFinancial.serviceIncomeConfirmedById === fixture.technician.id &&
      cashFinancial.serviceIncomeReportedAt?.getTime() ===
  ...
  assertFormalFulfillmentChain(
    ...
    },
    {
      ...
      session: {
        id: cashSession.id,
        bookingOrderId: cashOrder.id,
        startedByUserId: fixture.technician.id,
        startedAt: cashStartedEvent.occurredAt,
        expectedEndsAt: new Date(cashStartedEvent.occurredAt.getTime() + 90 * 60_000),
        endedByUserId: fixture.technician.id,
        endedAt: cashEndedEvent.occurredAt,
        deletedAt: null
      },
      checkout: {
        id: cashCheckout.id,
        ...
        paymentSelectedAt: cashSelectionEvent.occurredAt,
        ledgerTransactionId: null,
        receiptConfirmedById: fixture.technician.id,
        receiptConfirmedAt: cashReceiptEvent.occurredAt,
        receiptConfirmationReason: receiptInput.reason,
        deletedAt: null
      },
      addOn: {
        id: cashAddOn.id,
        bookingOrderId: cashOrder.id,
        serviceSessionId: cashSession.id,
        serviceId: fixture.addOnServiceId,
        status: "ACCEPTED",
        serviceNameSnapshot: "Formal add-on",
        priceAmountJpy: 2_200,
        ...
        proposedByUserId: fixture.technician.id,
        proposedAt: cashProposedEvent.occurredAt,
        acceptedByUserId: fixture.customer.id,
        acceptedAt: cashAcceptedEvent.occurredAt,
        rejectedByUserId: null,
        rejectedAt: null,
        resolutionReason: null,
        deletedAt: null
      ...
      histories: [
        ...
        },
        {
          ...
          actorUserId: fixture.technician.id,
          reason: "service_started",
          createdAt: cashStartedEvent.occurredAt
        },
        {
          fromStatus: "IN_SERVICE",
          toStatus: "AWAITING_CHECKOUT",
        ...
        {
          fromStatus: "AWAITING_CHECKOUT",
          toStatus: "AWAITING_PAYMENT_CONFIRMATION",
          actorUserId: fixture.customer.id,
          reason: "checkout_payment_method_selected",
          createdAt: cashSelectionEvent.occurredAt
        },
        {
          fromStatus: "AWAITING_PAYMENT_CONFIRMATION",
          toStatus: "COMPLETED",
          actorUserId: fixture.technician.id,
      ...
      events: [
        {
          ...
          eventType: "SERVICE_STARTED",
          actorUserId: fixture.technician.id,
          idempotencyKey: cashStartInput.idempotencyKey,
          reason: null,
          metadata: {
            actor: "technician",
            requestIp: context.ip,
        ...
        },
        {
          bookingOrderId: cashOrder.id,
          serviceSessionId: cashSession.id,
          orderAddOnId: cashAddOn.id,
          orderCheckoutId: null,
          eventType: "ADD_ON_PROPOSED",
          actorUserId: fixture.technician.id,
          idempotencyKey: cashProposalInput.idempotencyKey,
          reason: null,
          metadata: {
            actor: "technician",
            requestIp: context.ip,
        ...
        },
        {
          ...
          eventType: "ADD_ON_ACCEPTED",
          actorUserId: fixture.customer.id,
          idempotencyKey: cashAcceptInput.idempotencyKey,
          reason: null,
          metadata: {
            actor: "customer",
            requestIp: context.ip,
        ...
        },
        {
          ...
          eventType: "SERVICE_ENDED",
          actorUserId: fixture.technician.id,
          idempotencyKey: cashEndInput.idempotencyKey,
          reason: "completed",
          metadata: {
            actor: "technician",
            requestIp: context.ip,
        ...
        },
        {
          ...
          serviceSessionId: cashSession.id,
          orderAddOnId: null,
          orderCheckoutId: cashCheckout.id,
          eventType: "CHECKOUT_CREATED",
          actorUserId: fixture.customer.id,
  ...
  assert(
    summaryRows.length === 2 &&
      technicianSummary?.targetType === "technician" &&
      technicianSummary.targetId === fixture.technicianProfileId &&
      technicianSummary.reviewCount === 2 &&
      technicianSummary.ratingAverage.toString() === "4.5" &&
      Array.isArray(technicianSummary.highlights) &&
      JSON.stringify(technicianSummary.highlights) ===
        JSON.stringify(expectedTechnicianHighlights) &&
      customerSummary?.targetType === "customer" &&
      customerSummary.targetId === fixture.customerProfileId &&
      customerSummary.reviewCount === 2 &&
  ...
  await service.startService(
    ...
    {
      actor: "customer",
      idempotencyKey: `${fixture.marker}-pending-start`
    },
    context
  );
  await service.createOrderAddOn(
    ...
    {
      serviceId: fixture.addOnServiceId,
      idempotencyKey: `${fixture.marker}-pending-addon`
    },
    context
  );
    ...
    assertRejectedWithMessage(
      () =>
        service.endService(
          ...
          {
            reason: "completed",
            idempotencyKey: `${fixture.marker}-pending-end`
          },
          context
        ),
...
async function runInsufficientBalanceRollbackFlow(tx: Prisma.TransactionClient): Promise<void> {
  ...
  await service.startService(
    ...
    {
      actor: "customer",
      idempotencyKey: `${fixture.marker}-insufficient-start`
    },
    context
  );
  await withServiceSessionReadyToEnd(tx, order.id, now, () =>
    service.endService(
      ...
      {
        reason: "completed",
        idempotencyKey: `${fixture.marker}-insufficient-end`
      },
      context
    )
  );
  ...
  await service.selectCheckoutPaymentMethod(
    ...
    {
      method: "ndp",
      idempotencyKey: `${fixture.marker}-insufficient-select`
    },
    context
  );
...
export async function runOrderFulfillmentCheckoutCheck(): Promise<void> {
  ...
  try {
    ...
    await runRollbackOnlyTransaction(prisma, () => captureExternalBaseline(prisma), runFormalFlow);
