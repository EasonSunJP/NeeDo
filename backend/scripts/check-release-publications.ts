...
async function main() {
  ...
  try {
    ...
    const { stdout: manifestText } = await execute(process.execPath, [
      "--input-type=module", "-e",
    ...
    const receipt = {
      deploymentId: deploymentIds[2], environment: "test",
      sourceRevision: current.sourceRevision, previousRevision: null,
      publishedAt: new Date().toISOString(), current, previous: null
    };
    const invokeWriter = async (value: unknown) => {
      const pending = execute(process.execPath, [path.join(root, "backend/dist/cli/record-release.js")], {
    ...
    assert.equal(await prisma.auditLog.count({ where: {
      action: "release.published", targetType: "ReleasePublication", targetId: written.publicationId
    } }), 1);
    process.stdout.write(
      "release database checks passed: concurrent idempotency, one audit, conflict rejection, publication ordering, pagination, environment isolation, real Git manifest to compiled CLI, original receipt replay\n"
    );
  } finally {
...
void main()
  .catch(() => {
    process.stderr.write("release database verification failed\n");
    process.exitCode = 1;
  })
