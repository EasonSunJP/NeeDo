...
运营实时大盘 microstep A 只包含数据地基：`/api/v1/backoffice/dashboard/live-snapshot` 与 `/api/v1/backoffice/dashboard/live-events` 以 `country=JP`、可选的逐级 `admin1`/`admin2`、`period=today|last7days|last30days` 查询；快照名称按 `Accept-Language`（`zh-CN`、`zh-TW`、`ja`、`en`、`ko`，默认日语）本地化。地区归属只读取 Booking 创建时保存的不可变服务发生地，未解析旧订单只进入全国覆盖率，不进入东京/新宿下钻。缓存 TTL 为 300 秒，区域事件以 generation fence 逐级失效；SSE 发送 5 秒 retry、30 秒 heartbeat，支持 `Last-Event-ID`，过期/缺失游标返回 409，客户端需重新取快照再连接，且事件白名单不暴露客户资料、地址、备注或内部数字 ID。完整合同见 `docs/backoffice-real-data.md`。
