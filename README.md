...
Shop membership is a separate, shop-scoped relationship from the NeeDo platform-level `CustomerProfile.membershipLevel`. Merchant membership routes live under `/api/v1/merchant-admin/shop-memberships`, `/api/v1/merchant-admin/shop-membership-candidates`, `/api/v1/merchant-admin/shop-membership-cards`, `/api/v1/merchant-admin/shop-membership-activities`, and `/api/v1/merchant-admin/shop-membership-analytics`; every query derives the shop from the active authenticated `shop` identity. Customer self-service reads use `/api/v1/customer-profile/me/shop-memberships` and never accept a user or profile ID. `merchant_owner` receives view, create, analytics, operation-log, card-plan, issuance, card-adjustment, and stored-value top-up permissions, while `merchant_staff` remains read-only by default. Creating a shop membership requires a persisted booking relationship and writes audit evidence transactionally. Card plans, formal issuance, post-issuance customer-approved corrections, and immediate offline-paid stored-value top-ups are database-backed. An adjustment takes effect only after the owning customer explicitly approves it within 72 hours, and expiry never auto-approves; a top-up instead credits only the exact paid principal immediately and atomically writes immutable top-up evidence, audit, and customer notification. The `TEST` badge marks feature maturity only. Neither correction nor top-up creates NDP or wallet/ledger rows; redemption, refund, and actual reward settlement remain separate microsteps.
