...
One-to-one contact information now includes a default-off “聊天内容自动翻译” preference. The value belongs to the authenticated active identity's `ConversationParticipant` for that conversation, is updated through the formal preferences API, and changes the rendered text only after the server-confirmed conversation payload returns. Enabling it displays supported user-authored message text in the current App language; disabling it immediately renders the unchanged raw message again. Group information does not expose this switch, and translated display text is never written into the IM store, message payload, or database.
