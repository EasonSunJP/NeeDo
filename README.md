...
Operations staff with `backoffice:exchange:read` can inspect the same persisted aggregate through paginated `GET /api/v1/backoffice/exchange/posts` and `GET /api/v1/backoffice/exchange/posts/:id`. The projection includes existing post states, demand/intelligence scope, claims, matching, Request-fee hold/capture/release evidence, and a combined audit timeline. Publisher and provider identifiers are masked, embedded phone/email text is redacted, and private address lines are omitted. This slice is intentionally read-only: it does not invent draft/rejected states, review actions, forced withdrawal, or a parallel Exchange model.
...
This checker uses the same loopback and non-production guards, then persists and reuses marker `qa-technician-order-automation-20260909`. It restores the marker-owned active technician to the formal `on_duty` state, creates Availability-backed slots, and proves Booking auto-accept plus Request auto-apply for both merchant-priced shop services and technician-priced technician services. It also covers manual fallback with recorded reasons for non-matching rules, repeated-trigger idempotency, and background blocking for a technician whose current shop count is zero. Request automation creates a formal claim only and preserves user selection; it never auto-completes matching.
...
“我的地址” routes to `/me/addresses` and uses database-backed CRUD under `/api/v1/customer-profile/me/addresses`; it never opens a fabricated checkout service. Address ownership is derived from the active customer identity, every mutation is audited, the first saved address becomes the default, and deleting the default promotes one remaining address atomically. The page and formal home-service checkout reuse the same structured Japanese address contract and administrative-region codes without browser-storage persistence.
...
- 运营财务结算列表通过正式 `/api/v1/backoffice/finance/settlements` 按店铺或正式商户账户名、完整订单号、结算单号、真实结算状态、东京时区本周或本月、持久化城市筛选，并使用相同过滤条件返回服务端总数；清空搜索恢复全量分页。
...
“数据管理中心”已改为正式数据只读入口，通过后端分页和关键词过滤读取订单、客户、技师、店铺、服务、排班与结算。字段详情按业务名称展示，不再把数据库主键、外键或 JSON 对象直接转储给运营人员；订单详情统一进入订单中心，避免并行维护第二套订单 UI。库存、评价及历史全屏图表在正式表结构、RBAC、审计和分页合同完成前保持禁用，不再回退到浏览器 mock 或本地资料覆盖层。

独立“评价中心”现已接入受 `backoffice:users:read` 保护的正式完成订单评价列表与详情：固定 20 条服务端分页，支持关键词、最新有效评分、原始/已修订/系统生成、评价对象和东京日期筛选，并展示订单、评价人、客户、店铺、技师及不可变修订历史。页面不生成模拟评分、回复状态或风控预警；现有追加式修订仍通过带理由、乐观版本和审计事务的独立接口完成。
...
商户后台“订单中心”已接入按当前店铺强制隔离的服务端分页与详情、正式订单状态机、线下收款确认和退款接口。详情展示真实订单时间线，用户、服务、门店和技师均可打开叠层详情；绩效内部备注由服务端在商户响应中清除。取消订单、确认收款和确认退款采用二次点击确认；冲突、越权、网络失败和空数据都有明确状态，不再跳转到 mock 消息或调度流程。
...
运营后台“订单中心”现在使用全平台正式分页、订单状态机及受保护的运营收款/退款接口。订单状态、绩效修订和加钟事件统一使用后台时间线组件，显示真实操作人姓名与头像；订单里的用户、服务、门店和技师可继续打开叠层详情。改期、派单、改价、打印和虚构聊天在正式合同上线前不再作为可用按钮展示，避免运营人员误以为操作已写入数据库。
...
The technician schedule entry keeps the existing formal “我的排班” calendar and adds separate Test-labelled “接单设置” and “抢单设置” tabs. Both settings are versioned, audited, scoped to the active technician identity and disabled by default. Rules use fail-safe AND evaluation: non-matching bookings remain pending, while matching Requests create a formal claim without automatic matching. Apply migration `20260909090000_technician_order_automation` before enabling the feature. The implementation and local verification plan is documented in [technician booking and Request automation](docs/superpowers/plans/2026-09-09-technician-booking-request-automation.md). Request candidate selection now evaluates all eligible slots under the current shop pricing mode before reserving a technician decision; failed Request actions can retry with the same decision key. The `onlyOnline` rule is authoritative: when disabled, an offline or unsynchronised work state does not block automatic application; when enabled, the decision remains unmatched with an auditable online reason. The technician NeeDo entry opens the Request feed by default, while private publisher identity remains an anonymous projection rather than hiding the post. See the [local regression evidence](docs/verification/2026-09-20-request-auto-claim.md).
...
### Operations anytime service testing

Operations System Settings provides the audited `随时服务测试` switch. It is persisted in the immutable platform-settings version and defaults off. While off, service can start no earlier than 30 minutes before the booked `startsAt` and can end only at or after the persisted service-session `expectedEndsAt`; while on, only those two time checks are bypassed so QA can complete the formal flow. Apply migration `20260911100000_anytime_service_test`. See the [design and verification contract](docs/superpowers/specs/2026-09-11-anytime-service-test-design.md).

Operations System Settings also provides the audited `过期预约未处理门禁` switch, defaulting off in the same immutable settings version. When enabled, start-service locks and rejects the earliest unresolved overdue BookingOrder sharing the customer or assigned technician; Booking and Request-converted orders use the same rule. Participants resolve the overdue order through `POST /api/v1/orders/:id/overdue-resolution`. Resolution always records the chosen history and notifies the counterparty. While the switch is enabled, no-show outcomes additionally create one immutable system-authored zero rating, and a confirmed prepaid outcome is accepted only with the frozen 500 NDP platform-fee bearer snapshot and compensation basis before reusing formal ledger settlement and payroll readiness. While disabled, resolution causes no automatic rating or settlement; unpaid outcomes never create a settlement transfer. Run `npm --prefix backend run check:overdue-appointment-gate` for the guarded source/migration contract. Apply migration `20260911160000_overdue_appointment_gate`. See the [design contract](docs/superpowers/specs/2026-09-11-overdue-appointment-gate-design.md).

Authenticated accounts with merchant or technician access now preload their server-resolved formal schedule scopes after login. The UI displays encrypted schedule cache immediately and revalidates in the background with a non-blocking centered indicator; explicit logout clears the account's persistent resource cache, while closing the page or installed PWA does not. See [schedule preload design](docs/superpowers/specs/2026-09-13-schedule-login-preload-design.md).

Shop privacy persists on `Shop.visibility` and is enforced by one shared backend policy across discovery, direct detail, shop-owned services, favorites/shares, booking navigation, availability, and booking creation. The shop-display privacy switch reads and writes `GET/PUT /api/v1/merchant-admin/shops/:shopId/visibility` under the signed shop identity, RBAC, strict validation, and transactional audit; the merchant personal information card retains its independent profile privacy. Anonymous users see only public shops; owner, reciprocal-friend, and selected formal-network access follows the saved mode. Merchant-account relationships retain their original identity scope alongside the separately selected shop. Authenticated visibility-aware reads are never shared-cacheable. See [the Step 08 visibility contract](docs/08_CORE_READ_API_AND_API_CONTRACTS.md#店铺可见性补充契约2026-09-13) and [local regression verification](docs/verification/2026-09-20-shop-privacy.md).
