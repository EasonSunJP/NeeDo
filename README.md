...
Shop privacy now persists on `Shop.visibility` and is enforced by one shared backend policy across discovery, direct detail, shop-owned services, favorites/shares, booking navigation, availability, and booking creation. The merchant privacy switch reads and writes `GET/PUT /api/v1/merchant-admin/shops/:shopId/visibility` under the signed shop identity, RBAC, strict validation, and transactional audit. Anonymous users see only public shops; owner, reciprocal-friend, and selected formal-network access follows the saved mode. Authenticated visibility-aware reads are never shared-cacheable. See [the Step 08 visibility contract](docs/08_CORE_READ_API_AND_API_CONTRACTS.md#店铺可见性补充契约2026-09-13).
