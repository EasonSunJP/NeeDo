...
- `排班 > 自动`
  ...
  - 按「模式选择 / 规则设定 / 最终确认」三步承载模板、规则、优先级、通知和自动确认逻辑
  - 两种模式统一为：`技师自主排班`、`商户直接排班`
  - 原商户反馈收集/确认模式已废弃；历史浏览器草稿在读取时归一为技师自主排班，不能重新进入旧流程
  - 新建周期默认从当天开始，结束日期默认是一个日历月后的同日；规则页不再要求技师反馈截止时间
  - `保存草稿` 允许保存尚未完成的规则；`取消编辑` 会丢弃本次新建草稿及其派生数据并返回排班首页
  - 同一店铺的有效周期不得重叠；更换排班模式前必须先终止原周期
  - `商户直接排班` 保存发起后直接生成正式排班和 confirmed slots
- `排班 > 智能`
...
- `draft`：周期草稿
- `rule_setting`：规则设定中
- `collecting_feedback`：仅用于读取并迁移历史浏览器状态，当前流程不再生成
