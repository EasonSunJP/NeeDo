...
- Before checkout exists, every role reads one live payable projection: the persisted order payment amount plus active accepted add-on snapshots. Proposed, rejected, or deleted add-ons are excluded. Once checkout exists, its immutable total takes precedence, while settlement, commission, refund, and reconciliation continue to use the checkout and financial component snapshots.
...
- If a confirmed Booking is still unpaid and its scheduled start is in the future, cancellation by any authorized actor releases only the verifiable platform-fee hold. The order keeps its quoted price for audit, while the finance projection is cancelled with zero GMV, zero unreported service income, zero provider compensation, and no technician/shop compensation projection. Repeated transition attempts remain protected by the existing order-status compare-and-set transaction.
