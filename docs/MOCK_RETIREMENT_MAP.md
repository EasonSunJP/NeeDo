...
- Merchant admin: `MerchantAdminDashboardPage.tsx` now renders only the current shop's protected aggregate for orders, schedule, finance, shop identity, and technician profiles. Its demo customer overlay, generated shop presentation record, hard-coded financial zero, and links to unsupported design/analytics modules are removed. `MerchantAdminAnalyticsPage.tsx` now uses that same authenticated shop aggregate for the current finance, supply, order, and technician snapshot; retention, repurchase, channel, review, employee ranking, and historical comparisons remain explicitly disabled until a shop-scoped time-series contract exists.
