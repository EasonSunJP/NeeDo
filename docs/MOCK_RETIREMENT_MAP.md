...
| `src/state/userOrderStore.ts` | Legacy nonnumeric checkout/detail compatibility only; the formal customer order list and numeric order details no longer read or mutate it | `needo.user-created-orders.v1` | Retire the remaining compatibility lane when formal checkout no longer needs legacy routes | Step 10 |
...
- User booking/order/account surfaces: `src/pages/user/UserOrdersPage.tsx` now reads only the paginated Booking API and no longer hides, deletes, reviews, or merges orders in browser storage. Numeric routes in `UserOrderDetailPage.tsx` load the formal order and status history from the API and use the authenticated customer cancellation endpoint; nonnumeric demo detail links remain isolated compatibility. `CheckoutPage.tsx` and `UserCenterPage.tsx` still require their owning Step 10 slices.
