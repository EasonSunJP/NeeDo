...
- Admin surfaces: `DashboardPage.tsx` now renders only the protected backoffice aggregate for metrics, orders, schedule, finance, shops, and technicians; unsupported city trends, field jobs, risk scores, and merchant health are explicitly disabled instead of populated by demo metrics. `OrdersAdminPage.tsx`, `FinancePage.tsx`, `MerchantsPage.tsx`, and `TechniciansPage.tsx` already use the formal adapter. `DataCenterPage.tsx`, `ReviewsPage.tsx`, `RolesPage.tsx`, `MarketingPage.tsx`, `InventoryPage.tsx`, `AnalyticsPage.tsx`, and `NeedoExchangeAdminPage.tsx` remain for later owning slices.
