...
## 11. 2026-09-07 统一前台登录入口

- 登录页不展示用户、技师、店铺身份标签或切换选项；身份切换保留在用户端设置。
- 邮箱、无前缀十位数字账号 ID、`u` + 十位数字 ID 默认登录用户端。
- `s` + 十位数字 ID 登录技师端，`b` + 十位数字 ID 登录店铺端；前缀只决定请求身份，密码和身份权限仍由正式 Auth 链路校验。
- 以 s/b 开头的邮箱仍进入用户端；Google 登录和注册也统一进入用户端。
- 旧客户端 HTML 登录入口先跳转到 `user.html`；登录成功后仍在该 HTML 内进入对应身份路由，保留同一会话存储范围。后台专用登录入口继续走 AdminLoginPage。
- 欢迎文案同步简中、繁中、日文、英文、韩文。无 API、数据库或 migration 变更。
- 回归覆盖旧入口、邮箱、三种 ID 前缀、纯数字 ID（含前导零）、后端拒绝登录，以及既有 Google/注册/身份切换流程。
- 合并适配：保留 main 的平台登录设置、动态 Logo 和密码登录邮箱 OTP 流程；首次密码登录、重发验证码和验证码验证都按同一 ID 规则选择身份。
- 验证：独立 main 工作区登录页 42 项与相关前端回归 18 文件 / 246 项通过；`npm run build -- --mode formal` 通过（含 TypeScript 检查）；审查无阻塞发现。此前工作区已完成桌面 / 390px 登录页布局检查，真实账号与安装版 PWA 验收尚未完成。
