...
## 10. 用户首页轮播运营工作台紧凑改版（2026-09-09）

`pf-admin.html#/admin/carousel` 的用户首页轮播设定页改为单列发布工作流：顶部直接复用用户主页的 `FeatureCarousel`，点击某张轮播后选中该项并暂停自动轮播；五语言快捷按钮位于预览下方；当前发布内容与新草稿内容以“缩略图和文字同容器 → 向下替换箭头 → 新内容容器”的顺序显示。当前内容容器读取正式 Release 的 `activatedAt` 作为上次发布时间，当前与草稿缩略图均可打开大图。

保存草稿、立即发布和定时发布仍作用于完整 Release，继续执行五语言完整性、媒体、目标、乐观锁、RBAC 和审计校验；“复制当前语言到其他语言”仍是显式同步动作，其他语言同步后仍可分别修改。历史版本、停用和回滚没有删除，集中在页面底部默认折叠的“历史版本与回滚”区域。

本次只调整 `USER_HOME` 运营界面，不改变 `AFFILIATE_HOME_NOTICE` 编辑器、Prisma schema、migration、正式 API 或公共轮播读取契约。自动化、类型检查、构建和本地浏览器证据应在本节后续验收完成后单独记录；不能用本地组件通过推断 staging、部署或设备验收已完成。
