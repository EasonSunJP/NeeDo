...
## 2026-09-08 地名、订单热度、触控缩放与趋势宽度

- 全国初始视图按主要城市优先级显示少量地名，标签只在行政锚点内部显示；空间不足时隐藏低优先级地名，不再把名称排到地图外围，也不再绘制交叉引导线。放大时按当前可见空间逐步增加名称，选中或悬停地区始终优先。
- 地图区域统一采用绿色、黄色、橙色到红色的订单热度色阶。绿色端点取前一个完整 JST 自然日同级地区最少订单数，红色端点取最多订单数；正式快照为每个 child region 分别返回所选周期、今日及昨日订单数。颜色基准不随拖动、缩放或当前筛选视野变化。
- 加减按钮已替换为范围滑条；0% 对应 1 倍完整地图，100% 对应 8 倍，即旧 4 倍上限的两倍。保留还原操作，桌面支持滚轮和拖动，手机支持单指平移及双指围绕中心捏合缩放。
- 拖动、滚轮、滑条或双指缩放期间地名隐藏，最后一次动作结束 250ms 后重新进行碰撞计算并淡入；鼠标悬停或键盘聚焦会显示地区名、今日订单、昨日订单和支付信息。
- 手机纵向布局保留地图图形与触控控件；不再因窄屏隐藏地图。订单与支付趋势把折线几何铺满卡片可用宽度，日期标签改由独立等宽轴排列，因此文字和数字不参与 SVG 横向缩放。
- 本批次没有新增 migration、mock、静态订单数或远程服务依赖。

## 2026-09-10 地区点击与 20 倍缩放

- 地图不再在普通指针按下时立即由外层 SVG 捕获指针；只有确认发生单指平移或进入双指缩放时才启用 Pointer Capture，因此行政区域路径能够收到原生 click 并执行既有都道府县/市区町村下钻。
- 拖拽后的合成 click 仍被抑制，键盘 Enter/Space 选择与既有行政区选择器合同保持不变。
- 共享地图缩放上限由 8 倍调整为 20 倍；滑条仍以 0% 表示 1 倍完整地图、100% 表示 20 倍，滚轮与双指缩放统一受同一上限约束。
- 本批次只修改前端地图手势、缩放常量和回归测试；没有新增 API、数据库字段、migration、mock 或远程依赖。
