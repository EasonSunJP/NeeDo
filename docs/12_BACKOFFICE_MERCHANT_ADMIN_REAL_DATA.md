...
- 编辑状态在首页、环境、菜单和地图间切换时跟随当前页；动态和情报页不显示店铺五语言编辑栏，情报由 Request 发布入口维护。
...
- `POST /api/v1/merchant-admin/shop/presentation/locales/:locale/sync` 在通用红框确认后，以当前语言页面内草稿的图片和文字原子覆盖五个语言记录；请求必须携带五个版本的 `expectedLockVersions`，同步后各语言仍保持独立记录和后续编辑能力。
- `POST /api/v1/merchant-admin/shop/presentation/media` 接收 JPEG、PNG 或 WebP 原始字节，创建绑定当前店铺和商户身份的 `MediaAsset`；已上传的首页轮播图最多 5 张，没有正式媒体时允许单独保存文字草稿。
- 店铺还没有正式轮播媒体时，页面可继续显示只读兜底图；首次“新增轮播图”会用新上传的正式媒体建立轮播，不会把无法保存的兜底 URL 混入语言草稿。
- 服务套餐只本地化名称、说明、适用对象、标签、亮点和图片；价格、币种、时长及服务归属继续来自正式 `Service`。
- 公开 `GET /api/v1/shops/:id?locale=...` 按请求语言投影已保存的店铺、轮播图、环境图说明、地图信息和服务套餐展示文案，没有该语言记录时回退正式基础资料。
...
- 合作技师本人发布且标记为 `technician_shops` 的可排班时段可向其有效从属店铺公开；遇到跨店硬锁时，后端先扣除锁定区间。
