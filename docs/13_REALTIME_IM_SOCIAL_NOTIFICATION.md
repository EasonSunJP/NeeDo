...
- 各端既有“声音与提醒方式”开关即时生效；200ms 内关联事件合并为一次提示音，SSE event id 以 128 条上限做有界去重。
...
### 跨身份名称实时同步补强（2026-09-13，本地）

- 客户或技师修改正式名称时，现有持久化逻辑会同步同一账号下的全部个人身份；实时通知现在按同一范围查找会话，不再只通知当前编辑身份参与的会话。
- 商户身份名称仍严格按当前商户身份广播，避免把同一账号下不同商户组织的独立名称错误串联。
- `profile.updated` 继续只向与受影响身份共享有效会话的身份发送，事件负载不暴露额外身份关联；接收端沿用正式 IM bootstrap 刷新通讯录与会话标题。
- 回归测试覆盖个人身份跨会话扇出、商户身份隔离，以及客户、技师、商户三种资料保存入口的通知范围。

### 历史聊天与聊天记录媒体引用修复（2026-09-12，本地）

- 普通 IM 媒体继续从 Git 主检出的共享 `backend/runtime/im-media` 读取；聊天记录的受保护副本不再落入当前进程/worktree 相对目录，而是固定使用共享 IM 媒体目录旁的 `im-chat-record-media`。linked worktree 清理不再删除仍被聊天记录引用的副本。
- 受保护副本缺失时，只允许从已配置的共享 IM/Content 根目录中恢复“文件名 checksum、实际 SHA-256、MIME 魔数与大小上限”全部一致的真实字节，并立即写回共享受保护目录；不改写数据库引用，也不使用占位文件或相似图片。
- 新建转发/收藏仍须在持久化事务前成功克隆原媒体；原字节不存在、类型不符或大小不符时整条命令失败且不写 bundle、item、favorite/delivery 或 MediaAsset。
- 聊天记录媒体端点的 `40401 error.im.chat_record_media_unavailable` 在前端进入稳定“媒体不可用”终态，不再显示无限重试。普通 IM immutable URL 在原生媒体失败后以同源 `HEAD` 分类：404/410/不可解码的客户端状态为不可用，鉴权、超时、限流、5xx 与网络异常仍为可重试交付失败；404 不伪装成自然过期。
- 本地正式记录核对：会话 2571 的 10799、10802 语音和 10803 图片文件存在且静态 `HEAD` 为 200；10800 图片文件不存在且没有可证明原字节的 MediaAsset/checksum，因此保持不可用终态。聊天记录 `805b566c-c6c8-4471-9a8d-c23fe83c4e2b` 的 `6f02cf…` 图片已从 checksum 完全一致的 114297 字节 JPEG 无损恢复；真实 RBAC API 返回 200、`image/jpeg`、正确长度/ETag/immutable 缓存头和相同 SHA-256，未授权请求仍返回 401。
- 自动化覆盖存储恢复与路径稳定性、新建记录零部分写入、HTTP 40401 契约、前端永久/暂时错误分类和图片/语音终态渲染。本批次验证与数据恢复仅限本地，未推送或部署到远程环境。

### 通讯录可打开关系与详情 403 修复（2026-09-12，本地）

- `GET /api/v1/im/contacts` 只返回当前身份可以安全打开的联系人：双方均由正式好友申请建立的有效 Contact，或当前身份已有未隐藏、未删除的一对一直聊参与关系。目标账号与目标身份都必须仍启用且未删除；排班、组织或其他业务 seed 单独建立、但没有可访问会话的 Contact 不再混入普通通讯录。
- 新建好友直聊再次校验双方 Contact 的 `source = friend_request`，保留好友关系门禁；既有 `BUSINESS_CONTEXT` 会话继续按其参与关系读取，不被伪装成好友关系。
- 联系人详情对非好友来源不再调用创建会话接口，而是进入现有的服务端目录资料关系页；好友详情创建/恢复直聊的 effect 只依赖稳定方法，失败重渲染不会连续重复 `POST /api/v1/im/conversations`。目录接口仍执行现有隐私裁剪，不返回邮箱、电话、token 或其他越权字段。
- 本地真实 `needo_dev` 只读诊断确认联系人 `7546` 是双向 `admin_6m_20260906_v1` 业务关系、双方身份有效但没有直接会话；修复后的 Repository 查询不再返回该联系人。没有修改、删除或补写数据库记录，也没有新增 migration。
- 回归先验证旧实现出现 2 个后端失败和 2 个前端失败，再完成最小修复。聚焦 Repository 3 suites / 25 tests、Realtime Service 53 tests、Realtime API 22 tests，以及前端 IM 5 files / 229 tests 均通过；完整 lint、build 与 main/5180 验收在本批次集成记录中单独确认。
### 语音消息播放器气泡宽度（2026-09-10，本地）

- 三端共享 `MessageBubble` 的语音消息改为使用头像旁可用宽度，并在宽屏封顶 320px；原生音频播放器随气泡全宽，不再受 220px 上限限制。
- 带引用或表情回应的语音消息沿用相同全宽内容容器，不会因额外包装层让播放器缩回最小宽度。
- 文字、图片、视频、文件和卡片消息继续使用原宽度规则；320px 窄屏保持可收缩且不应产生横向页面溢出。
- 本地自动化与浏览器验收分别记录；远程推送、部署、已安装 PWA 和实体设备验收不在本微步骤内。

### 添加标签输入框一致性（2026-09-10，本地）

- 标签管理的添加弹窗复用 `ImChatComposer`，保留表情入口，隐藏语音、更多和内置发送按钮。弹窗直接使用聊天玻璃展示模式承载输入区与下方的取消、添加按钮，不再套用外部不透明底板或嵌套玻璃卡片。
- 嵌入模式不叠加聊天页底部安全区留白；标签仍按原有规范化与添加流程处理，表情判断标签转换为文本保存。未新增 API 或修改数据库。
- 聊天输入组件、IM 页面及 Social 回复组件定向回归 4 文件 / 158 测试通过；真机和部署验收不在本次本地验证范围内。
...
- 添加好友搜索要求目标账号至少拥有一个仍启用的个人身份，同时匹配账号名、NeeDoID、当前客户/技师资料名和仍启用的个人身份名；响应只选择个人身份并返回规范个人资料名，前端信任服务端数据库排序规则的搜索结果，不再用 JavaScript 重复过滤。纯店铺或运营账号即使账号名或 NeeDoID 命中也不会进入好友目录，创建好友关系时服务端再次校验目标个人身份；已存在的当前身份联系人仍按原规则排除，防止重复添加。
...
## 6.34 动态系统通知与相对时间本地化（2026-09-13，本地）

- 新生成的预约状态和好友申请通知使用稳定事件码与结构化参数持久化；预约状态通知携带服务名、原状态和目标状态，客户端按当前界面语言生成业务可读文案，不再展示后端拼接的英文句子。
- 已存在的预约状态历史通知继续通过 `type` 和既有 `fromStatus` / `toStatus` 参数兼容显示；缺少服务名时使用预约通用文案。未知系统事件只显示安全的通用通知，不回显未知正文或 payload。运营正式公告继续使用已选定的公告语言正文，用户发布的动态正文不进入系统通知翻译流程。
...
- 会话输入区及回复预览现通过 `ImStandaloneShell.viewportOverlay` 渲染为外层 `.client-shell` 的直接子节点，与已在镜像中验证位置正确的底部导航处于相同层级；`.im-conversation-composer-dock` 独立使用 fixed 底边和已有键盘 bottom 偏移，不再位于聊天房间内部，也不会被房间高度或 overflow 裁剪，并以当前可视视口高度限制自身最大高度，避免键盘打开且回复、附件或长文本同时存在时溢出屏幕。消息滚动区继续由 `ResizeObserver` 读取整个 dock（含回复预览）的真实高度并预留底部空间，回归覆盖高度变化与卸载清理。
...
### PWA 会话视口、资料实时刷新与语音首播修复（2026-09-10，本地）

- 已安装移动 PWA 不再只靠一个固定高度差推断键盘关闭。键盘帧出现后会记录本次最小可见高度；当视口明显扩展且稳定 180ms 时，即使 WebKit 最终高度仍低于 layout viewport，也释放陈旧键盘帧并把聊天房间重新锚定到底部，避免输入区下方长期暴露空白。
- standalone 模式下输入胶囊改用当前主题的不透明背景，并关闭该元素的 `backdrop-filter` 与外投影组合，绕开 WebKit 圆角玻璃层在固定表面上的黑角合成问题；普通浏览器和其他玻璃表面保持原样。
- 可滚动消息列表到达顶部或底部后继续拖动时使用限幅位移，并在松手后 180ms 回弹。页面滚动仍锁定，手势不会串到聊天外层。
- customer、technician、merchant 资料的显示名称或头像提交成功后，通过正式会话参与者关系发现接收身份并发送 `profile.updated` SSE。前端收到后补拉正式 bootstrap；一对一会话标题优先使用当前对方参与者名称，不再让历史持久化标题盖住新名称。该事件不新增轮询或浏览器假数据。
- 聊天内语音元素改为首播前积极缓冲，并在用户触发播放时显式恢复 `defaultMuted=false`、`muted=false`、`volume=1` 与 inline 播放，修复 iOS/PWA 第一次播放进入了播放状态但无声、第二次才有声的问题。
- 本切片没有新增数据库表、字段或 migration，没有改变 REST 请求/响应结构。物理 iPhone/Android 已安装 PWA 与双账号正式数据验收仍需在发布后完成，本地自动化和浏览器尺寸验证不能替代真机验收。
