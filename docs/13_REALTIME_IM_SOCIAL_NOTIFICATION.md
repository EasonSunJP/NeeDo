...
## 6.29 移动 Safari 对话视口与判断表情长按保护（2026-09-05）
...
- 输入编辑器与占位文案固定为 iOS 不触发聚焦缩放的 16px；全屏框架同时继承 `VisualViewport.width` 和 `offsetLeft`，即使系统视口正在过渡或保留已有缩放，也会按当前可见宽度重新约束，不让消息、头像或输入栏从右侧越出屏幕。
...
## 6.30 会员权益驱动的无痕撤回（2026-09-06，本地代码验收）
