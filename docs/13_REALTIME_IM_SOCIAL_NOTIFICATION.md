...
## 6.12 连续快捷表情的并发一致性（2026-08-30）

- 同一人可对同一条消息添加多个不同表情；同一“消息、用户、表情”仍只保留一条有效记录，再次点击同一表情表示撤销。
- reaction mutation 事务先按会话成员资格锁定权威消息行，再写入或软删除 reaction，并递增消息的 `reactionVersion` 后返回完整聚合，避免两个不同表情并发时各自返回不完整的事务快照。
...
## 6.16 好友验证、双向解除与身份资料卡（2026-08-30）
