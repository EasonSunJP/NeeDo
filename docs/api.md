...
Intelligence linked to a shop uses that shop's current visibility for feed pagination, totals, direct post reads, and interaction-read checks. Shop information cards open the store homepage directly. An older `sourcePostId` query remains accepted for link compatibility but never expands shop visibility; a denied shop read keeps the normal not-found response.
