...
- `check:booking-platform-fee-debt-flow` 在真实本地 MySQL 中创建并清理 7 个标记用户、10 个店铺和 11 个预约，已通过关闭收费、店铺/技师承担、首请求整体回滚、连续负余额欠费守恒、正常取消返还、历史无 hold 取消完整赔付及负余额审计、即时返点、审核充值后延迟返点、过期不补发和幂等重放。验收用数据库屏障分别强制“完单先锁”和“充值先锁”：结算按钱包→OrderFinancial 顺序加锁，欠费/到期处理用单条 `SELECT ... FOR UPDATE` 读取当前行，返点截止判断统一使用 MySQL `CURRENT_TIMESTAMP(3)`；清理后全库聚合基线完全一致。
...
### 4.1.1 历史 confirmed 取消恢复规则（2026-09-13）

- 缺失平台费 hold、hold 与钱包冻结余额不一致、商户可用 NDP 不足均不再阻断合法的 `confirmed → cancelled`。解冻只按真实冻结余额执行，不补造 NDP；预期额、实际额、差额写入 `OrderFinancial` 时间线与 hold metadata。
- 服务方取消仍按已发布 penalty 规则完整赔付客户。商户余额不足时复用单一钱包的负余额债务口径，保存取消前余额、取消后余额与本次新增债务；禁止缩减客户赔付或绕过账本直改余额。
- 订单、支付 `confirmed → refundPending`、时段容量、状态历史、技师绩效、钱包、hold、账本、财务时间线和审计继续处于同一个 Prisma 事务。`NDP` 与 `TEST_NDP` 严格隔离，测试币不进入正式对账。
- 前端四类主体的取消入口统一按稳定错误键展示精确提示；Exchange 双方取消、支付冲突、排班冲突、接单暂停及账本异常不得被笼统伪装成“订单状态变化”。
