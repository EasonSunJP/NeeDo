...
- Add authenticated merchant endpoints to read all five locale projections, update one locale, and upload presentation images. Shop scope comes only from the active identity or selected merchant-shop token context.
...
- Tests cover Zod validation, service scope/version behavior, route/RBAC/audit wiring, upload association, frontend API calls, inline editing, language switching, carousel append/replace, save/cancel, and localized public projection.
