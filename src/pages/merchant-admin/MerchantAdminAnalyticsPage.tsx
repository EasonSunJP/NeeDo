...
function MerchantAdminAnalyticsContent({ resource }: { resource: MerchantAdminDashboardResource }) {
  const { dashboard, error, status: loadStatus } = resource;
  const reload = resource.reload;
  const loadError = error ? describeMerchantAnalyticsError(error) : "";
      ...
      >
        ...
        {loadStatus === "error" ? (
          <section className="rounded-lg border border-coral/30 bg-coral/5 px-5 py-8 text-center shadow-panel" role="alert">
            ...
            <Button className="mt-4" onClick={reload}>重新加载本店分析</Button>
          </section>
        ) : null}
...
export function MerchantAdminAnalyticsPage() {
  return (
    <MerchantAdminLayout>
      {(resource) => <MerchantAdminAnalyticsContent resource={resource} />}
    </MerchantAdminLayout>
  );
}
