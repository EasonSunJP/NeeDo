...
import {
  ...
  type MerchantStaffEmploymentType,
...
function getMerchantEmploymentTopTag(employmentType: MerchantStaffEmploymentType) {
  return {
    label: getMerchantStaffEmploymentLabel(employmentType),
    tone: employmentType === "fullTime" ? "purple" as const : employmentType === "partTime" ? "yellow" as const : "neutral" as const
  };
}
...
export function MerchantPortalContent({
  ...
  useEffect(() => {
    ...
    if (activeView !== "me" || activeMeTab !== "service" || !storeApiId || !activeMerchantIdentityId) {
  ...
  }, [activeMeTab, activeMerchantIdentityId, activeView, merchantPrivacyScopeKey, storeApiId]);
  ...
  const reconcileStorePrivacyVisibility = async (requestedScopeKey: string, requestedIdentityId: number) => {
    ...
    try {
      ...
      if (
        ...
        if (merchantPrivacyScopeKeyRef.current === requestedScopeKey) {
          void reconcileStorePrivacyVisibility(requestedScopeKey, activeMerchantIdentityId);
        }
        return;
      }
  ...
  const updateStorePrivacyEnabled = (enabled: boolean) => {
    ...
    setStorePrivacyMenuOpen(false);
    setStorePrivacyConfirmOpen(false);
    void saveStorePrivacyVisibility("public");
  };
