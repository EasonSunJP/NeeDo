...
describe("needoPetAssets", () => {
  ...
  it("keeps Xiaobai clip timing aligned with the 6 fps assets", async () => {
    ...
    const { petSpriteSrc, xiaobaiIdleClips, xiaobaiOneShotClips, xiaobaiPetAssetManifest, xiaobaiRunningClips, xiaobaiPetAssetVersion } = await import(
      "./needoPetAssets"
    );

    expect(xiaobaiPetAssetVersion).toBe("20260921d");
    expect([
    ...
    ].every((src) => src.includes("-atlas.png?v=20260921d"))).toBe(true);
    ...
    expect(Object.keys(petSpriteSrc).sort()).toEqual(["grave", "happy", "idle", "running"]);
    expect(
      xiaobaiPetAssetManifest.some((src) => /xiao-bai-(angry|failed|hungry|jumping|notice|phone|sleeping|waiting|waving)\.png/u.test(src))
    ).toBe(false);
