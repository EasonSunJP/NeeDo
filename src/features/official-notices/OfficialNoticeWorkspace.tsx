...
export function OfficialNoticeManagement({ scope, composePath }: { scope: OfficialNoticeScope; composePath: string }) {
  ...
  const mutate = async (action: "cancel" | "archive" | "retry") => {
    ...
    setError("");
    try {
      const command = { expectedLockVersion: selected.lockVersion, reason: reason.trim(), idempotencyKey: makeKey(action) };
  ...
  return (
    <ModuleShell
      ...
      actions={<Button to={composePath}>创建通知</Button>}
    >
      ...
      <Drawer open={Boolean(selected)} onClose={() => setSelected(null)} title="通知详情">
        {selected ? <div className="space-y-5">
          ...
          <div className="rounded-lg border border-line bg-paper p-4 text-sm leading-7">{selected.translations[selected.sourceLocale]?.blocks.map((block) => block.type === "divider" ? <hr className="my-3 border-line" key={block.id} /> : <p className="whitespace-pre-wrap" key={block.id}>{block.content}</p>)}</div>
          ...
          <div className="flex flex-wrap gap-2">
            {!["sent", "cancelled", "archived"].includes(selected.status) ? <Button disabled={mutating || !reason.trim()} onClick={() => void mutate("cancel")} variant="danger">取消发送</Button> : null}
            {["sent", "cancelled"].includes(selected.status) ? <Button disabled={mutating || !reason.trim()} onClick={() => void mutate("archive")} variant="secondary">归档</Button> : null}
            {selected.delivery.failed > 0 ? <Button disabled={mutating || !reason.trim()} onClick={() => void mutate("retry")} variant="secondary">重试失败投递</Button> : null}
          </div>
        </div> : null}
      </Drawer>
    </ModuleShell>
...
export function OfficialNoticeInbox() {
  ...
  return <ModuleShell title="通知收件箱" description="这里只展示当前登录身份实际收到的通知及其阅读状态。" actions={<label className="flex items-center gap-2 text-sm font-black"><input checked={unreadOnly} onChange={(event) => { setPage(1); setUnreadOnly(event.target.checked); }} type="checkbox" />只看未读</label>}>
    {error ? <p className="rounded-lg bg-coral/10 p-3 text-sm font-bold text-coral">{error}</p> : null}
    <section className="overflow-hidden rounded-lg border border-line bg-white shadow-panel">{loading ? <p className="p-8 text-center text-sm font-bold text-ink/55">正在读取收件箱…</p> : items.length === 0 ? <p className="p-8 text-center text-sm font-bold text-ink/55">暂无通知</p> : <div className="divide-y divide-line">{items.map((item) => <article className="p-5" key={item.publicId}><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex gap-2"><Badge tone={item.readAt ? "neutral" : "blue"}>{item.readAt ? "已读" : "未读"}</Badge><Badge tone={item.level === "urgent" ? "red" : item.level === "important" ? "yellow" : "neutral"}>{levelLabels[item.level]}</Badge></div><h3 className="mt-3 text-lg font-black">{item.title}</h3><p className="mt-1 text-sm font-bold text-ink/55">{item.summary}</p></div><Button disabled={Boolean(item.readAt)} onClick={() => void markRead(item)} size="sm" variant="secondary">标记已读</Button></div><div className="mt-4 rounded-lg bg-paper p-4 text-sm leading-7">{item.blocks.map((block) => block.type === "divider" ? <hr className="my-3 border-line" key={block.id} /> : <p className="whitespace-pre-wrap" key={block.id}>{block.content}</p>)}</div><p className="mt-3 text-xs font-bold text-ink/45">{item.targetSummary} · {formatDate(item.sentAt)}</p></article>)}</div>}<div className="flex items-center justify-between border-t border-line p-4 text-xs font-bold text-ink/55"><span>共 {total} 条</span><div className="flex gap-2"><Button disabled={page <= 1} onClick={() => setPage((value) => value - 1)} size="sm" variant="secondary">上一页</Button><Button disabled={page * pageSize >= total} onClick={() => setPage((value) => value + 1)} size="sm" variant="secondary">下一页</Button></div></div></section>
  </ModuleShell>;
}
