...
export function UnifiedUserDirectory({ scope, onSelect }: { scope: UserDirectoryScope; onSelect: (userId: number) => void }) {
  ...
  // Drawer-only URL changes must not invalidate the directory query or unmount its table.
  ...
  const filterValue = useMemo(() => ({
    keyword: query.keyword, tier: query.tiers?.[0], identityType: query.identityTypes?.[0],
    state: query.states?.[0], ekyc: query.ekycStates?.[0]
  }), [query]);
