...
export function UserGroupMembersDrawer({ group, onClose, onSaved }: { group: UserGroup | null; onClose: () => void; onSaved: () => void }) {
  ...
  return <><Drawer headerActions={canCreate ? <Button size="sm" disabled={creating} onClick={() => { setCreating(true); setCreatedId(""); }}>{copy.add}</Button> : undefined} open={group !== null} title={group ? `${group.name} · 成员` : "分组成员"} onClose={onClose}>
    ...
    {data ? <div className="divide-y divide-line rounded-xl border border-line bg-white">{data.list.length ? data.list.map((member) => <div className="flex items-center gap-3 p-3" key={member.needoId}>{member.avatarUrl ? <img alt="" className="h-9 w-9 rounded-full object-cover" src={member.avatarUrl} /> : <div aria-label={member.username} className="flex h-9 w-9 items-center justify-center rounded-full bg-moss/10 text-xs font-black text-moss">{member.username.trim().slice(0, 1).toUpperCase() || "?"}</div>}<div className="min-w-0 flex-1 break-words"><p className="text-sm font-bold">{member.username}</p><p className="text-xs text-ink/45">{member.needoId} · {member.email}</p></div><Button className="shrink-0" size="sm" variant="secondary" onClick={() => setSelectedUserId(member.id)}>{copy.details}</Button></div>) : <p className="p-6 text-center text-sm text-ink/45">暂无成员</p>}</div> : null}
