...
import { platformBenefitCodes, platformTierCodes, type PlatformBenefitAdministration, type PlatformTierAdministration, type PlatformTierCode } from "./types";
...
export function MembershipTiersPage() {
  ...
  useEffect(() => { let active = true; setLoading(true); setError(null); Promise.all([platformUserManagementApi.listTiers(), platformUserManagementApi.listBenefits()]).then(([nextTiers, nextBenefits]) => { if (active) { setTiers(nextTiers); setBenefits(nextBenefits); } }).catch(() => active && setError("会员等级读取失败")).finally(() => active && setLoading(false)); return () => { active = false; }; }, [reloadToken]);
  const malformed = (tiers.length > 0 || benefits.length > 0) && (tiers.length !== platformTierCodes.length || platformTierCodes.some((code, index) => tiers[index]?.tierCode !== code) || benefits.length !== platformBenefitCodes.length || platformBenefitCodes.some((code) => !benefits.some((benefit) => benefit.code === code)));
  const activeTier = tiers.find((tier) => tier.tierCode === selected) ?? null;
  return <AdminLayout><ModuleShell title="会员等级设置" description="四种固定会员类型使用版本化草稿与发布；卡片预览复用用户端和聊天中的实际组件。">
