...
export function PlatformPartnerRangeEditor({ userId, canWrite = true }: { userId: number; canWrite?: boolean }) {
  ...
  const [history, setHistory] = useState<{ loading: boolean; error: string; list: PlatformPartnerProfile[] }>({ loading: true, error: "", list: [] });

  useEffect(() => {
    let active = true;
    setHistory((current) => ({ ...current, loading: true, error: "" }));
    platformPartnersApi.listUserPartnerProfiles(userId, { page: 1, pageSize: 20 })
      .then((page) => active && setHistory({ loading: false, error: "", list: page.list }))
      .catch(() => active && setHistory({ loading: false, error: labels.historyFailed, list: [] }));
    return () => { active = false; };
  }, [labels.historyFailed, reloadHistory, userId]);
    ...
    <section className="rounded-[18px] border border-line bg-white p-4 shadow-panel sm:p-5">
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-3">
        {partnerTypes.map((type) => {
          const draft = drafts[type];
          const typeHistory = history.list.filter((item) => item.partnerType === type);
          return (
            <article className="rounded-xl border border-line bg-paper p-4" key={type}>
              ...
              <details className="mt-3 border-t border-line pt-3">
                <summary className="cursor-pointer text-xs font-black text-ink/60">{labels.history} ({typeHistory.length})</summary>
                {history.loading ? <p className="mt-2 text-xs text-ink/45">{labels.historyLoading}</p> : null}
                {history.error ? <p className="mt-2 text-xs font-bold text-coral">{history.error}</p> : null}
                {!history.loading && !history.error && typeHistory.length === 0 ? <p className="mt-2 text-xs text-ink/45">{labels.noHistory}</p> : null}
                <div className="mt-2 grid gap-2">
                  {typeHistory.map((item) => <div className="rounded-lg border border-line bg-white p-2 text-xs text-ink/60" key={item.publicId}><p className="font-black text-ink">{new Date(item.startsAt).toLocaleString(language)} — {item.endsAt ? new Date(item.endsAt).toLocaleString(language) : labels.permanent}</p><p className="mt-1">{item.reason}</p></div>)}
                </div>
              </details>
            </article>
          );
