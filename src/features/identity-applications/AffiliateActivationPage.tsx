...
export function AffiliateActivationPage() {
  ...
  const activate = async () => {
    ...
    try {
      ...
      window.location.assign("/afirieito/me");
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : String(caught));
      setBusy(false);
    }
    ...
    <ApplicationShell info="阅读联盟营销规则及与 NeeDo 的完整合同，确认后即时开启身份。" title="开启联盟营销身份">
      ...
      {alreadyActive ? (
        <ApplicationCard className="space-y-4 text-center">
          ...
          <ApplicationButton className="w-full" onClick={() => void refreshSession("business").then((result) => result.ok && window.location.assign("/afirieito/me"))}>{t("进入联盟营销")}</ApplicationButton>
        </ApplicationCard>
      ) : (
