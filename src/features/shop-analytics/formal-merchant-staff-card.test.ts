...
describe("formal merchant staff cards", () => {
  const staff = { id: "tech-api-28", name: "佐藤", nickname: "佐藤 美咲", avatar: "/avatar.jpg", bio: "施術歴10年", serviceAreas: ["東京都"], rating: 4.6, reviewCount: 8 };
  it("uses persisted profile and review fields without generated verification or social claims", () => {
    expect(buildFormalMerchantStaffCard(staff)).toMatchObject({ id: staff.id, displayName: staff.nickname, avatar: staff.avatar, headline: staff.bio, regionLabel: "東京都", scoreValue: "4.6", kycVerified: false, levelLabel: "", followerCount: 0, followingCount: 0 });
  });
  ...
  it("uses explicit formal data and hides unsupported level and social fields in home, staff lists and staff summary", () => {
    expect(source.match(/data=\{buildFormalMerchantStaffCard\(technician\)\}/g)).toHaveLength(3);
