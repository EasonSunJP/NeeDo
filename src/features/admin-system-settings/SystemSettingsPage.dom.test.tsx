...
describe("SystemSettingsPage interactions", () => {
  ...
  it("renders service switches as the same single-layer cards as the site switches", async () => {
    await act(async () => {
      root.render(createElement(MemoryRouter, { initialEntries: ["/admin/settings/system?tab=basic"] }, createElement(SystemSettingsPage)));
    });
    ...
    const overdueCard = cardFor("过期预约未处理门禁");
    ...
    expect(overdueCard?.className).toBe(siteCard?.className);
    expect(overdueCard?.parentElement?.classList.contains("rounded-2xl")).toBe(false);
  });
});
