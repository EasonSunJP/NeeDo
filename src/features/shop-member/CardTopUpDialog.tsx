...
export function CardTopUpDialog({ card, onClose, onCompleted }: Props) {
  ...
  return <div aria-modal="true" className="fixed inset-0 z-[145] grid place-items-end bg-black/64 p-2 backdrop-blur-sm sm:place-items-center sm:p-4" role="dialog">
    <section className="max-h-[94dvh] w-full max-w-[620px] overflow-y-auto rounded-[32px] border border-[color:var(--client-line)] bg-[color:var(--client-bg)] p-4 text-[color:var(--client-text)] shadow-[0_30px_100px_rgba(0,0,0,0.58)] sm:p-5">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="block"><span className="mb-1.5 block text-xs font-black text-[color:var(--client-muted)]">实际收款金额（JPY）</span><input aria-label="实际收款金额" className={field} inputMode="numeric" onChange={(event) => setEditor((current) => ({ ...current, amountJpy: event.target.value }))} placeholder="例如 5000" value={editor.amountJpy} /></label><label className="block"><span className="mb-1.5 block text-xs font-black text-[color:var(--client-muted)]">收款方式</span><select aria-label="收款方式" className={field} onChange={(event) => setEditor((current) => ({ ...current, paymentMethod: event.target.value as typeof current.paymentMethod }))} value={editor.paymentMethod}><option value="cash">现金</option><option value="card">银行卡</option><option value="paypay">PayPay</option><option value="other">其他</option></select></label></div>
      <div className="mt-3 space-y-3"><label className="block"><span className="mb-1.5 block text-xs font-black text-[color:var(--client-muted)]">收款凭证</span><input aria-label="收款凭证" className={field} maxLength={160} onChange={(event) => setEditor((current) => ({ ...current, paymentReference: event.target.value }))} placeholder="收据号或 POS 交易号" value={editor.paymentReference} /></label><label className="block"><span className="mb-1.5 block text-xs font-black text-[color:var(--client-muted)]">备注</span><textarea className={cn(field, "min-h-24 resize-y py-3 leading-6")} maxLength={500} onChange={(event) => setEditor((current) => ({ ...current, note: event.target.value }))} placeholder="未填写凭证时，必须说明可核对的线下收款情况" value={editor.note} /></label></div>
