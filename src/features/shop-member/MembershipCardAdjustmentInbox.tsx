...
export function MembershipCardAdjustmentInbox({ onChanged }: Props) {
  ...
  useEffect(() => {
    let active = true;
    setLoading(true);
    customerShopMembershipApi.adjustmentRequests({ page: 1, pageSize: 50, status: "pending" })
  ...
  return <section className={cn(panel, "mb-4")} aria-label="待确认的会员卡调整">
    ...
    <div className="mt-3 space-y-3">{requests.map((item) => {
