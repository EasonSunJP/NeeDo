...
function OverviewView({ data, onEnroll, canEnroll }: { data: ShopMembershipOverview; onEnroll: () => void; canEnroll: boolean }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <section className={panelClassName}><h2 className="text-base font-black">后续独立开放</h2><p className="mt-2 text-sm font-semibold leading-6 text-[color:var(--client-muted)]">充值、核销、退款以及开卡后的金额或次数调整会分别接入独立状态机与账本；本次正式开卡不会自动发放 NDP。</p></section>
    </div>
  );
}
...
function IssuedCardsView({ canIssue, onIssue, page, onPageChange, result }: { canIssue: boolean; onIssue: () => void; page: number; onPageChange: (page: number) => void; result: PaginatedShopMemberships<MerchantShopMembershipCard> }) {
...
function CardsView({ canIssue, canManagePlans, canPublishPlans, onIssue, page, onPageChange, result }: { canIssue: boolean; canManagePlans: boolean; canPublishPlans: boolean; onIssue: () => void; page: number; onPageChange: (page: number) => void; result: PaginatedShopMemberships<MerchantShopMembershipCard> }) {
  const [tab, setTab] = useState<"issued" | "plans">("issued");
  return <div className="space-y-4"><div className={cn(insetClassName, "grid grid-cols-2 gap-1 p-1")} role="tablist" aria-label="会员卡管理"><button aria-selected={tab === "issued"} className={cn("min-h-11 rounded-[17px] px-3 text-sm font-black text-[color:var(--client-muted)]", tab === "issued" && "bg-[color:var(--client-surface)] text-[color:var(--client-text)] shadow-sm")} onClick={() => setTab("issued")} role="tab" type="button">已发会员卡</button><button aria-selected={tab === "plans"} className={cn("min-h-11 rounded-[17px] px-3 text-sm font-black text-[color:var(--client-muted)]", tab === "plans" && "bg-[color:var(--client-primary-soft)] text-[color:var(--client-primary)] shadow-sm")} onClick={() => setTab("plans")} role="tab" type="button">卡方案 <span className="ml-1 rounded-full bg-[color:var(--client-primary)] px-1.5 py-0.5 text-[9px] text-[color:var(--client-primary-contrast)]">TEST</span></button></div>{tab === "issued" ? <IssuedCardsView canIssue={canIssue} onIssue={onIssue} onPageChange={onPageChange} page={page} result={result} /> : <CardPlanWorkspace canManage={canManagePlans} canPublish={canPublishPlans} />}</div>;
}
...
function ShopMembershipSectionContent({ activeSection, canEnroll, canIssue, canManagePlans, canPublishPlans, onEnroll, onIssue, revision }: { activeSection: MemberSection; canEnroll: boolean; canIssue: boolean; canManagePlans: boolean; canPublishPlans: boolean; onEnroll: () => void; onIssue: () => void; revision: number }) {
  ...
  const content = useMemo(() => {
    ...
    if (activeSection === "cards") return <CardsView canIssue={canIssue} canManagePlans={canManagePlans} canPublishPlans={canPublishPlans} onIssue={onIssue} onPageChange={setPage} page={page} result={state.data as PaginatedShopMemberships<MerchantShopMembershipCard>} />;
  ...
  }, [activeSection, canEnroll, canIssue, canManagePlans, canPublishPlans, currentRequestKey, onEnroll, onIssue, page, state]);
...
export function ShopMemberCenterPage() {
  ...
  return <MobileShell showBottomNav={false}><MobileFullscreenHeader action={<TestFeatureBadge />} onBack={() => navigate(-1)} subtitle="店铺私域会员与会员卡状态" title="会员中心" /><main className="mx-auto w-full max-w-[1120px] px-4 pb-28 pt-4"><FeatureSegmentedTabs items={visibleSectionTabs} onChange={(next) => navigate(next === "overview" ? "/merchant/member" : `/merchant/member/${next}`)} value={activeSection} variant="header" />{issuanceSuccess ? <p className={cn(insetClassName, "mt-4 px-4 py-3 text-sm font-black text-[color:var(--client-primary)]")} role="status">{issuanceSuccess}</p> : null}<ShopMembershipSectionContent activeSection={activeSection} canEnroll={canEnroll} canIssue={canIssue} canManagePlans={canManagePlans} canPublishPlans={canPublishPlans} key={activeSection} onEnroll={() => setEnrollmentOpen(true)} onIssue={() => { setIssuanceSuccess(""); setIssuanceOpen(true); }} revision={revision} /></main>{enrollmentOpen ? <EnrollmentDialog onClose={() => setEnrollmentOpen(false)} onCreated={() => { setEnrollmentOpen(false); setRevision((value) => value + 1); }} /> : null}{issuanceOpen ? <CardIssuanceDialog onClose={() => setIssuanceOpen(false)} onIssued={(card) => { setIssuanceOpen(false); setIssuanceSuccess(`${card.customerDisplayName} 的 ${card.name} 已成功开卡`); setRevision((value) => value + 1); }} /> : null}</MobileShell>;
}
