...
function IssuedCardsView({ canAdjust, canIssue, onAdjust, onIssue, page, onPageChange, result }: { canAdjust: boolean; canIssue: boolean; onAdjust: (card: MerchantShopMembershipCard) => void; onIssue: () => void; page: number; onPageChange: (page: number) => void; result: PaginatedShopMemberships<MerchantShopMembershipCard> }) {
  return <div className="space-y-3"><section className={cn(panelClassName, "flex items-center justify-between gap-3")}><div><p className="text-xs font-bold text-[color:var(--client-muted)]">已发会员卡</p><strong className="text-xl font-black">{result.total} 张</strong></div>{canIssue ? <button className="inline-flex min-h-11 items-center gap-2 rounded-full bg-[color:var(--client-primary)] px-5 text-sm font-black text-[color:var(--client-primary-contrast)]" onClick={onIssue} type="button"><span>开卡</span><TestFeatureBadge className="border-white/35" /></button> : null}</section>{!result.list.length ? <PageStatus message={canIssue ? "选择有效会员和已发布卡方案即可正式开卡。" : "当前账号可查看会员卡，但没有开卡权限。"} title="当前店铺暂无会员卡" /> : result.list.map((card) => <article className={cn(panelClassName, "relative overflow-hidden")} key={card.publicId}><div className="absolute inset-y-0 right-0 w-20 bg-[radial-gradient(circle_at_right,color-mix(in_srgb,var(--client-primary)_20%,transparent),transparent_65%)]" /><div className="relative flex items-start justify-between gap-3"><div><p className="text-xs font-bold text-[color:var(--client-muted)]">{card.customerDisplayName} · {card.customerNeedoId}</p><h2 className="mt-1 text-lg font-black">{card.name}</h2><p className="mt-1 text-[11px] font-bold text-[color:var(--client-muted)]">{card.planVersion ? `方案 v${card.planVersion}` : "历史会员卡"}</p></div><StatusChip status={card.status} /></div><div className="relative mt-4 flex items-end justify-between gap-3"><div><p className="font-mono text-xs font-black tracking-[0.14em] text-[color:var(--client-muted)]">{card.cardNoMasked}</p><p className="mt-1 text-xs font-semibold text-[color:var(--client-muted)]">有效期至 {formatDate(card.expiresAt)}</p></div><strong className="text-lg font-black">{card.type === "stored_value" ? formatJpy((card.principalBalanceJpy ?? 0) + (card.bonusBalanceJpy ?? 0)) : card.type === "count" ? `${card.remainingUses ?? 0} 次` : "权益卡"}</strong></div>{canAdjust && card.status === "active" && card.type !== "benefit" ? <div className="relative mt-4 flex justify-end border-t border-[color:var(--client-line)] pt-3"><button className="inline-flex min-h-10 items-center gap-2 rounded-full border border-[color:var(--client-primary)] px-4 text-xs font-black text-[color:var(--client-primary)]" onClick={() => onAdjust(card)} type="button">申请调整 <TestFeatureBadge /></button></div> : null}</article>)}<Pagination onChange={onPageChange} page={page} pageSize={result.page_size} total={result.total} /></div>;
}

function CardsView({ canAdjust, canIssue, canManagePlans, canPublishPlans, onAdjust, onChanged, onIssue, page, onPageChange, result, revision }: { canAdjust: boolean; canIssue: boolean; canManagePlans: boolean; canPublishPlans: boolean; onAdjust: (card: MerchantShopMembershipCard) => void; onChanged: () => void; onIssue: () => void; page: number; onPageChange: (page: number) => void; result: PaginatedShopMemberships<MerchantShopMembershipCard>; revision: number }) {
  const [tab, setTab] = useState<"issued" | "adjustments" | "plans">("issued");
  return <div className="space-y-4"><div className={cn(insetClassName, "grid grid-cols-3 gap-1 p-1")} role="tablist" aria-label="会员卡管理"><button aria-selected={tab === "issued"} className={cn("min-h-11 rounded-[17px] px-2 text-xs font-black text-[color:var(--client-muted)] sm:text-sm", tab === "issued" && "bg-[color:var(--client-surface)] text-[color:var(--client-text)] shadow-sm")} onClick={() => setTab("issued")} role="tab" type="button">已发会员卡</button><button aria-selected={tab === "adjustments"} className={cn("min-h-11 rounded-[17px] px-2 text-xs font-black text-[color:var(--client-muted)] sm:text-sm", tab === "adjustments" && "bg-[color:var(--client-primary-soft)] text-[color:var(--client-primary)] shadow-sm")} onClick={() => setTab("adjustments")} role="tab" type="button">调整申请 <span className="ml-1 text-[9px]">TEST</span></button><button aria-selected={tab === "plans"} className={cn("min-h-11 rounded-[17px] px-2 text-xs font-black text-[color:var(--client-muted)] sm:text-sm", tab === "plans" && "bg-[color:var(--client-primary-soft)] text-[color:var(--client-primary)] shadow-sm")} onClick={() => setTab("plans")} role="tab" type="button">卡方案 <span className="ml-1 text-[9px]">TEST</span></button></div>{tab === "issued" ? <IssuedCardsView canAdjust={canAdjust} canIssue={canIssue} onAdjust={onAdjust} onIssue={onIssue} onPageChange={onPageChange} page={page} result={result} /> : tab === "adjustments" ? <CardAdjustmentRequestList onChanged={onChanged} revision={revision} /> : <CardPlanWorkspace canManage={canManagePlans} canPublish={canPublishPlans} />}</div>;
}
...
function ShopMembershipSectionContent({ activeSection, canAdjust, canEnroll, canIssue, canManagePlans, canPublishPlans, onAdjust, onChanged, onEnroll, onIssue, revision }: { activeSection: MemberSection; canAdjust: boolean; canEnroll: boolean; canIssue: boolean; canManagePlans: boolean; canPublishPlans: boolean; onAdjust: (card: MerchantShopMembershipCard) => void; onChanged: () => void; onEnroll: () => void; onIssue: () => void; revision: number }) {
  ...
  }, [activeSection, canAdjust, canEnroll, canIssue, canManagePlans, canPublishPlans, currentRequestKey, onAdjust, onChanged, onEnroll, onIssue, page, revision, state]);
