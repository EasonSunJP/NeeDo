...
function IssuedCardsView({ page, onPageChange, result }: { page: number; onPageChange: (page: number) => void; result: PaginatedShopMemberships<MerchantShopMembershipCard> }) {
...
function ShopMembershipSectionContent({ activeSection, canEnroll, canManagePlans, canPublishPlans, onEnroll, revision }: { activeSection: MemberSection; canEnroll: boolean; canManagePlans: boolean; canPublishPlans: boolean; onEnroll: () => void; revision: number }) {
  ...
  const content = useMemo(() => {
    ...
    if (activeSection === "cards") return <CardsView canManagePlans={canManagePlans} canPublishPlans={canPublishPlans} onPageChange={setPage} page={page} result={state.data as PaginatedShopMemberships<MerchantShopMembershipCard>} />;
  ...
  }, [activeSection, canEnroll, canManagePlans, canPublishPlans, currentRequestKey, onEnroll, page, state]);
...
export function ShopMemberCenterPage() {
  ...
  return <MobileShell showBottomNav={false}><MobileFullscreenHeader action={<TestFeatureBadge />} onBack={() => navigate(-1)} subtitle="店铺私域会员与会员卡状态" title="会员中心" /><main className="mx-auto w-full max-w-[1120px] px-4 pb-28 pt-4"><FeatureSegmentedTabs items={visibleSectionTabs} onChange={(next) => navigate(next === "overview" ? "/merchant/member" : `/merchant/member/${next}`)} value={activeSection} variant="header" /><ShopMembershipSectionContent activeSection={activeSection} canEnroll={canEnroll} canManagePlans={canManagePlans} canPublishPlans={canPublishPlans} key={activeSection} onEnroll={() => setEnrollmentOpen(true)} revision={revision} /></main>{enrollmentOpen ? <EnrollmentDialog onClose={() => setEnrollmentOpen(false)} onCreated={() => { setEnrollmentOpen(false); setRevision((value) => value + 1); }} /> : null}</MobileShell>;
}
