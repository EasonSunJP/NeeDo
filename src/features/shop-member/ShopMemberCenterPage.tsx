...
import { CardPlanWorkspace } from "./CardPlanWorkspace";
import { CardIssuanceDialog } from "./CardIssuanceDialog";
import { CardAdjustmentRequestDialog } from "./CardAdjustmentRequestDialog";
import { CardAdjustmentRequestList } from "./CardAdjustmentRequestList";
import { IssuedCardAdjustmentAction } from "./IssuedCardAdjustmentAction";
import { CardTopUpDialog } from "./CardTopUpDialog";
import { CardTopUpHistory } from "./CardTopUpHistory";
import { isCardTopUpEligible } from "./cardTopUpModel";
import { CardRedemptionDialog } from "./CardRedemptionDialog";
import { CardRedemptionHistory } from "./CardRedemptionHistory";
import { isCardRedemptionEligible } from "./cardRedemptionModel";
...
function OverviewView({ data, onEnroll, canEnroll }: { data: ShopMembershipOverview; onEnroll: () => void; canEnroll: boolean }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <section className={panelClassName}><h2 className="text-base font-black">完整会员卡流程</h2><p className="mt-2 text-sm font-semibold leading-6 text-[color:var(--client-muted)]">开卡、客户确认调整与充值不会产生 NDP；核销与退款已接入正式数据库、RBAC、审计、通知与 NDP 账本。订单正式退款后可恢复卡消费，并按原记录冲正客户返点和平台费。</p></section>
    </div>
  );
}
...
function IssuedCardsView({ canAdjust, canIssue, canRedeem, canTopUp, onAdjust, onIssue, onRedeem, onTopUp, onViewRequests, page, onPageChange, result }: { canAdjust: boolean; canIssue: boolean; canRedeem: boolean; canTopUp: boolean; onAdjust: (card: MerchantShopMembershipCard) => void; onIssue: () => void; onRedeem: (card: MerchantShopMembershipCard) => void; onTopUp: (card: MerchantShopMembershipCard) => void; onViewRequests: () => void; page: number; onPageChange: (page: number) => void; result: PaginatedShopMemberships<MerchantShopMembershipCard> }) {
  return <div className="space-y-3"><section className={cn(panelClassName, "flex items-center justify-between gap-3")}><div><p className="text-xs font-bold text-[color:var(--client-muted)]">已发会员卡</p><strong className="text-xl font-black">{result.total} 张</strong></div>{canIssue ? <button className="inline-flex min-h-11 items-center gap-2 rounded-full bg-[color:var(--client-primary)] px-5 text-sm font-black text-[color:var(--client-primary-contrast)]" onClick={onIssue} type="button"><span>开卡</span><TestFeatureBadge className="border-white/35" /></button> : null}</section>{!result.list.length ? <PageStatus message={canIssue ? "选择有效会员和已发布卡方案即可正式开卡。" : "当前账号可查看会员卡，但没有开卡权限。"} title="当前店铺暂无会员卡" /> : result.list.map((card) => <article className={cn(panelClassName, "relative overflow-hidden")} key={card.publicId}><div className="absolute inset-y-0 right-0 w-20 bg-[radial-gradient(circle_at_right,color-mix(in_srgb,var(--client-primary)_20%,transparent),transparent_65%)]" /><div className="relative flex items-start justify-between gap-3"><div><p className="text-xs font-bold text-[color:var(--client-muted)]">{card.customerDisplayName} · {card.customerNeedoId}</p><h2 className="mt-1 text-lg font-black">{card.name}</h2><p className="mt-1 text-[11px] font-bold text-[color:var(--client-muted)]">{card.planVersion ? `方案 v${card.planVersion}` : "历史会员卡"}</p></div><StatusChip status={card.status} /></div><div className="relative mt-4 flex items-end justify-between gap-3"><div><p className="font-mono text-xs font-black tracking-[0.14em] text-[color:var(--client-muted)]">{card.cardNoMasked}</p><p className="mt-1 text-xs font-semibold text-[color:var(--client-muted)]">有效期至 {formatDate(card.expiresAt)}</p></div><strong className="text-lg font-black">{card.type === "stored_value" ? formatJpy((card.principalBalanceJpy ?? 0) + (card.bonusBalanceJpy ?? 0)) : card.type === "count" ? `${card.remainingUses ?? 0} 次` : "权益卡"}</strong></div><div className="relative mt-4 flex flex-wrap gap-2">{canRedeem && isCardRedemptionEligible(card) ? <button aria-label="核销 TEST" className="inline-flex min-h-10 items-center gap-2 rounded-full bg-[color:var(--client-primary)] px-4 text-xs font-black text-[color:var(--client-primary-contrast)]" onClick={() => onRedeem(card)} type="button">核销 <TestFeatureBadge className="border-white/35" /></button> : null}{canTopUp && isCardTopUpEligible(card) ? <button className="inline-flex min-h-10 items-center gap-2 rounded-full border border-[color:var(--client-primary)] px-4 text-xs font-black text-[color:var(--client-primary)]" onClick={() => onTopUp(card)} type="button">充值 <TestFeatureBadge /></button> : null}</div><IssuedCardAdjustmentAction canAdjust={canAdjust} card={card} lookupStatus={Object.prototype.hasOwnProperty.call(card, "pendingAdjustment") ? "ready" : "error"} onAdjust={() => onAdjust(card)} onViewRequests={onViewRequests} pendingRequest={card.pendingAdjustment ?? null} /></article>)}<Pagination onChange={onPageChange} page={page} pageSize={result.page_size} total={result.total} /></div>;
}

function CardsView({ canAdjust, canIssue, canManagePlans, canPublishPlans, canRedeem, canRefund, canTopUp, onAdjust, onChanged, onIssue, page, onPageChange, result, revision }: { canAdjust: boolean; canIssue: boolean; canManagePlans: boolean; canPublishPlans: boolean; canRedeem: boolean; canRefund: boolean; canTopUp: boolean; onAdjust: (card: MerchantShopMembershipCard) => void; onChanged: () => void; onIssue: () => void; page: number; onPageChange: (page: number) => void; result: PaginatedShopMemberships<MerchantShopMembershipCard>; revision: number }) {
  const [tab, setTab] = useState<"issued" | "redemptions" | "topups" | "adjustments" | "plans">("issued");
  const [topUpCard, setTopUpCard] = useState<MerchantShopMembershipCard | null>(null);
  const [redemptionCard, setRedemptionCard] = useState<MerchantShopMembershipCard | null>(null);
  const tabClass = (active: boolean) => cn("min-h-11 w-[112px] shrink-0 rounded-[17px] px-2 text-[11px] font-black text-[color:var(--client-muted)] sm:w-auto sm:flex-1 sm:text-sm", active && "bg-[color:var(--client-primary-soft)] text-[color:var(--client-primary)] shadow-sm");
  return <div className="space-y-4"><div className={cn(insetClassName, "overflow-x-auto p-1")}><div className="flex min-w-max gap-1 sm:min-w-0" role="tablist" aria-label="会员卡管理"><button aria-selected={tab === "issued"} className={tabClass(tab === "issued")} onClick={() => setTab("issued")} role="tab" type="button">已发会员卡</button><button aria-selected={tab === "redemptions"} className={tabClass(tab === "redemptions")} onClick={() => setTab("redemptions")} role="tab" type="button">核销与退款 <span className="ml-1 text-[9px]">TEST</span></button><button aria-selected={tab === "topups"} className={tabClass(tab === "topups")} onClick={() => setTab("topups")} role="tab" type="button">充值记录 <span className="ml-1 text-[9px]">TEST</span></button><button aria-selected={tab === "adjustments"} className={tabClass(tab === "adjustments")} onClick={() => setTab("adjustments")} role="tab" type="button">调整申请 <span className="ml-1 text-[9px]">TEST</span></button><button aria-selected={tab === "plans"} className={tabClass(tab === "plans")} onClick={() => setTab("plans")} role="tab" type="button">卡方案 <span className="ml-1 text-[9px]">TEST</span></button></div></div>{tab === "issued" ? <IssuedCardsView canAdjust={canAdjust} canIssue={canIssue} canRedeem={canRedeem} canTopUp={canTopUp} onAdjust={onAdjust} onIssue={onIssue} onPageChange={onPageChange} onRedeem={setRedemptionCard} onTopUp={setTopUpCard} onViewRequests={() => setTab("adjustments")} page={page} result={result} /> : tab === "redemptions" ? <CardRedemptionHistory canRefund={canRefund} mode="merchant" onChanged={onChanged} revision={revision} /> : tab === "topups" ? <CardTopUpHistory mode="merchant" revision={revision} /> : tab === "adjustments" ? <CardAdjustmentRequestList onChanged={onChanged} revision={revision} /> : <CardPlanWorkspace canManage={canManagePlans} canPublish={canPublishPlans} />}{redemptionCard ? <CardRedemptionDialog card={redemptionCard} onClose={() => setRedemptionCard(null)} onCompleted={() => { setRedemptionCard(null); setTab("redemptions"); onChanged(); }} /> : null}{topUpCard ? <CardTopUpDialog card={topUpCard} onClose={() => setTopUpCard(null)} onCompleted={() => { setTopUpCard(null); setTab("topups"); onChanged(); }} /> : null}</div>;
}
...
function ShopMembershipSectionContent({ activeSection, canAdjust, canEnroll, canIssue, canManagePlans, canPublishPlans, canRedeem, canRefund, canTopUp, onAdjust, onChanged, onEnroll, onIssue, revision }: { activeSection: MemberSection; canAdjust: boolean; canEnroll: boolean; canIssue: boolean; canManagePlans: boolean; canPublishPlans: boolean; canRedeem: boolean; canRefund: boolean; canTopUp: boolean; onAdjust: (card: MerchantShopMembershipCard) => void; onChanged: () => void; onEnroll: () => void; onIssue: () => void; revision: number }) {
  ...
  const content = useMemo(() => {
    ...
    if (activeSection === "cards") return <CardsView canAdjust={canAdjust} canIssue={canIssue} canManagePlans={canManagePlans} canPublishPlans={canPublishPlans} canRedeem={canRedeem} canRefund={canRefund} canTopUp={canTopUp} onAdjust={onAdjust} onChanged={onChanged} onIssue={onIssue} onPageChange={setPage} page={page} result={state.data as PaginatedShopMemberships<MerchantShopMembershipCard>} revision={revision} />;
  ...
  }, [activeSection, canAdjust, canEnroll, canIssue, canManagePlans, canPublishPlans, canRedeem, canRefund, canTopUp, currentRequestKey, onAdjust, onChanged, onEnroll, onIssue, page, revision, state]);
...
export function ShopMemberCenterPage() {
  ...
  const canIssue = hasPermission("shop.member.card.issue");
  const canAdjust = hasPermission("shop.member.card.adjust.request");
  const canTopUp = hasPermission("shop.member.card.topup.create");
  const canRedeem = hasPermission("shop.member.card.redeem");
  const canRefund = hasPermission("shop.member.card.refund");
  const canManagePlans = hasPermission("shop.member.card_plan.manage");
  const canPublishPlans = hasPermission("shop.member.card_plan.publish");
  ...
  const [adjustmentCard, setAdjustmentCard] = useState<MerchantShopMembershipCard | null>(null);
  const [actionSuccess, setActionSuccess] = useState("");

  return <MobileShell showBottomNav={false}><MobileFullscreenHeader action={<TestFeatureBadge />} onBack={() => navigate(-1)} subtitle="店铺私域会员与会员卡状态" title="会员中心" /><main className="mx-auto w-full max-w-[1120px] px-4 pb-28 pt-4"><FeatureSegmentedTabs items={visibleSectionTabs} onChange={(next) => navigate(next === "overview" ? "/merchant/member" : `/merchant/member/${next}`)} value={activeSection} variant="header" />{actionSuccess ? <p className={cn(insetClassName, "mt-4 px-4 py-3 text-sm font-black text-[color:var(--client-primary)]")} role="status">{actionSuccess}</p> : null}<ShopMembershipSectionContent activeSection={activeSection} canAdjust={canAdjust} canEnroll={canEnroll} canIssue={canIssue} canManagePlans={canManagePlans} canPublishPlans={canPublishPlans} canRedeem={canRedeem} canRefund={canRefund} canTopUp={canTopUp} key={activeSection} onAdjust={(card) => { setActionSuccess(""); setAdjustmentCard(card); }} onChanged={() => setRevision((value) => value + 1)} onEnroll={() => setEnrollmentOpen(true)} onIssue={() => { setActionSuccess(""); setIssuanceOpen(true); }} revision={revision} /></main>{enrollmentOpen ? <EnrollmentDialog onClose={() => setEnrollmentOpen(false)} onCreated={() => { setEnrollmentOpen(false); setRevision((value) => value + 1); }} /> : null}{issuanceOpen ? <CardIssuanceDialog onClose={() => setIssuanceOpen(false)} onIssued={(card) => { setIssuanceOpen(false); setActionSuccess(`${card.customerDisplayName} 的 ${card.name} 已成功开卡`); setRevision((value) => value + 1); }} /> : null}{adjustmentCard ? <CardAdjustmentRequestDialog card={adjustmentCard} onClose={() => setAdjustmentCard(null)} onSubmitted={(request) => { setAdjustmentCard(null); setActionSuccess(`${request.customer.displayName} 的调整申请已提交，等待 72 小时内确认`); setRevision((value) => value + 1); }} /> : null}</MobileShell>;
}
