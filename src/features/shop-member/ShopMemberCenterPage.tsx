...
function OverviewView({ data, onEnroll, canEnroll }: { data: ShopMembershipOverview; onEnroll: () => void; canEnroll: boolean }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <section className={panelClassName}><h2 className="text-base font-black">完整会员卡流程</h2><p className="mt-2 text-sm font-semibold leading-6 text-[color:var(--client-muted)]">开卡、客户确认调整与充值不会产生 NDP；核销已接入正式数据库、审计、通知与 NDP 账本，并按已发布规则执行返点和平台费。退款仍作为下一独立步骤。</p></section>
    </div>
  );
}
...
function CardsView({ canAdjust, canIssue, canManagePlans, canPublishPlans, canRedeem, canTopUp, onAdjust, onChanged, onIssue, page, onPageChange, result, revision }: { canAdjust: boolean; canIssue: boolean; canManagePlans: boolean; canPublishPlans: boolean; canRedeem: boolean; canTopUp: boolean; onAdjust: (card: MerchantShopMembershipCard) => void; onChanged: () => void; onIssue: () => void; page: number; onPageChange: (page: number) => void; result: PaginatedShopMemberships<MerchantShopMembershipCard>; revision: number }) {
...
function ShopMembershipSectionContent({ activeSection, canAdjust, canEnroll, canIssue, canManagePlans, canPublishPlans, canRedeem, canTopUp, onAdjust, onChanged, onEnroll, onIssue, revision }: { activeSection: MemberSection; canAdjust: boolean; canEnroll: boolean; canIssue: boolean; canManagePlans: boolean; canPublishPlans: boolean; canRedeem: boolean; canTopUp: boolean; onAdjust: (card: MerchantShopMembershipCard) => void; onChanged: () => void; onEnroll: () => void; onIssue: () => void; revision: number }) {
  ...
  }, [activeSection, canAdjust, canEnroll, canIssue, canManagePlans, canPublishPlans, canRedeem, canTopUp, currentRequestKey, onAdjust, onChanged, onEnroll, onIssue, page, revision, state]);
