...
describe("contentPublicationApi", () => {
  ...
  it("keeps carousel draft and target-search inputs bound to their fixed scene", async () => {
    ...
    const affiliateReplacement = {
      ...
      slides: [
        {
          ...affiliateSlide,
          translations: affiliateSlide.translations.map((value) => ({
            ...value,
  ...
  it("rejects draft and search inputs from the other fixed scene at compile time", () => {
    ...
    contentPublicationApi.searchCarouselTargets("user-home", {
      // @ts-expect-error USER_HOME target search does not accept Affiliate task filters.
      type: "affiliate_task",
    });
    contentPublicationApi.searchCarouselTargets("affiliate-home-notice", {
  ...
  it("uses fixed scene paths for carousel reads, history, target search, and release reads", async () => {
    ...
    await contentPublicationApi.getCarouselHistory("affiliate-home-notice", {
      page: 2,
      pageSize: 30,
    });
    await contentPublicationApi.searchCarouselTargets("USER_HOME", {
  ...
  it("passes carousel draft, locale, copy, and lifecycle bodies without rewriting locks or idempotency", async () => {
    ...
    expect(httpClient.request).toHaveBeenNthCalledWith(
      ...
      {
        body: { idempotencyKey, expectedLockVersion: 5, reason: "approved" },
        method: "POST",
      },
    );
    expect(httpClient.request).toHaveBeenNthCalledWith(
    ...
    expect(httpClient.request).toHaveBeenNthCalledWith(
      ...
      {
        body: { idempotencyKey, expectedLockVersion: 6, reason: "expired" },
        method: "POST",
      },
    );
    expect(httpClient.request).toHaveBeenNthCalledWith(
      ...
      {
        body: { idempotencyKey, expectedCurrentVersion: 7, reason: "restore" },
        method: "POST",
      },
    );
  });
  ...
  it("covers the complete announcement draft, history, preview, and lifecycle contract", async () => {
    ...
    await contentPublicationApi.getAnnouncementHistory("notice / one", {
      page: 2,
      pageSize: 10,
    });
    ...
    expect(httpClient.request).toHaveBeenNthCalledWith(
      ...
      {
        body: { idempotencyKey, expectedLockVersion: 4, reason: "closed" },
        method: "POST",
      },
    );
    expect(httpClient.request).toHaveBeenNthCalledWith(
      ...
      {
        body: { idempotencyKey, expectedCurrentVersion: 5, reason: "restore" },
        method: "POST",
      },
    );
  });

  it("uses the protected announcement contracts for paginated task search and draft metadata", async () => {
    await contentPublicationApi.searchAnnouncementAffiliateTasks({
      page: 2,
      pageSize: 10,
      q: "Tokyo",
    });
    await contentPublicationApi.updateAnnouncementMetadata("notice / one", 8, {
      expectedLockVersion: 4,
      affiliateTaskId: 29,
    ...
    expect(httpClient.request).toHaveBeenNthCalledWith(
      ...
      { query: { page: 2, pageSize: 10, q: "Tokyo" } },
    );
    expect(httpClient.request).toHaveBeenNthCalledWith(
      2,
      "/backoffice/affiliate/announcements/notice%20%2F%20one/releases/8",
