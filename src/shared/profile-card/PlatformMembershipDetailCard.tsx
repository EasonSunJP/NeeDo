...
export function PlatformMembershipDetailCard(props: PlatformMembershipDetailCardProps) {
  ...
  return <article className="w-full max-w-[610px] rounded-[28px] border p-5 shadow-[0_24px_60px_rgba(0,0,0,0.2)]" data-platform-membership-detail-card="true" style={{ backgroundColor: theme.detailSurfaceColor, backgroundImage: resolveMembershipDetailGradient(theme), borderColor: theme.detailOuterBorderColor, color: theme.detailTextColor }}>
    <div className="flex items-start gap-4"><div className="h-28 w-28 shrink-0 overflow-hidden rounded-[28px] border-[3px] bg-black/10" style={{ borderColor: theme.detailAvatarBorderColor }}>{avatarNode}</div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><div className="flex min-w-0 flex-wrap items-center gap-2"><h1 className="truncate text-lg font-black">{props.displayName}</h1>{props.ekycVerified ? <span aria-label="eKYC verified" className="grid h-5 w-5 place-items-center rounded-full bg-[#85D20A] text-xs font-black text-[#071106]">✓</span> : null}</div>{props.actionSlot ? <div className="shrink-0">{props.actionSlot}</div> : null}</div><div className="mt-3 flex flex-wrap items-center gap-2"><span className="rounded-full px-3 py-1 text-xs font-black" style={{ backgroundColor: theme.detailAccentColor, color: theme.detailAccentTextColor }}>{props.tierLabel}</span>{showLevel ? <div><span className="font-black">Lv.{props.level}</span></div> : null}</div>{props.onNeedoIdClick ? <button aria-label="复制 NeeDo ID" className="mt-3 block max-w-full cursor-copy truncate text-left text-sm opacity-70" onClick={props.onNeedoIdClick} type="button">ID {props.needoId}</button> : <p className="mt-3 text-sm opacity-70">ID {props.needoId}</p>}</div></div>
