...
const source = {
  ...
  technicianProfile: read(
    "../technician-profile/TechnicianProfileInfoView.tsx",
  ),
...
describe("unified service-information-card usage", () => {
  it("follows the real home and search/category JSX delegation chain", () => {
    ...
    expect(recommendation).toMatch(
      /<SocialProfileMiniCard[\s\S]*data=\{buildServiceMiniCardData\(data\.service\)\}[\s\S]*detailTo=\{data\.to\}/u,
    );
    expect(serviceDelegate).toMatch(
      /return \(\s*<UnifiedServiceInfoCard[\s\S]*data=\{data\.serviceInfo\}/u,
    );
    expect(categoryCard).toContain(
      "return <UnifiedServiceInfoCard data={mapServiceItemToUnifiedData(service)}",
    );
  });

  it("uses the unified JSX body for shop-detail services", () => {
    const compactMenuCard = between(
      source.shopDetail,
    ...
    expect(source.shopDetail).toContain("merchantWorkspaceServiceIds.has(menuCard.sourceServiceId)");
  ...
  it("routes technician personal and public detail services through the shared profile view", () => {
    expect(source.technicianProfile).toMatch(
      /model\.services\.map\([\s\S]*<UnifiedServiceInfoCard[\s\S]*data=\{service\}/u,
    );
    expect(source.technicianProfile).not.toContain('variant="showcase"');
    expect(source.technicianPortal).toContain("<TechnicianProfileInfoView");
    expect(source.technicianPortal).toMatch(
      /services\.map\([\s\S]*<UnifiedServiceInfoCard[\s\S]*fromTechnicianServicePayload\(service\)/u,
    );
    expect(source.technicianPortal).not.toContain('variant="showcase"');
    expect(source.profileDetail).toContain(
      "<TechnicianProfileInfoView model={model} />",
    );
    expect(source.publicTechnicianCard).toContain(
      "<TechnicianProfileInfoView model={model} />",
    );
  });
  ...
  it("uses actual unified-card JSX for checkout, order, merchant, and technician-service consumers", () => {
    ...
    expect(source.orderMiniCard).toMatch(
      /<UnifiedServiceInfoCard[\s\S]*data=\{serviceCardData\}/u,
    );
    expect(
      countJsx(source.userOrder, "UnifiedServiceInfoCard"),
    ).toBeGreaterThanOrEqual(4);
    expect(source.userOrder).toContain(
      "data={buildBookingOrderServiceData(order, orderService)}",
    );
    expect(
      countJsx(source.merchantOrders, "UnifiedServiceInfoCard"),
    ).toBeGreaterThanOrEqual(4);
    ...
    expect(source.technicianServices).toMatch(
  ...
  it("uses the shared profile and service cards for Intelligence detail", () => {
    ...
    expect(source.exchangeDetail).toContain(
  ...
  it("keeps duplicate service fact-body signatures outside the shared card", () => {
    ...
    expect(consumers).not.toMatch(
      /<img[^>]+alt=\{(?:service|item)\.name\}[^>]*>/u,
    );
    expect(consumers).not.toMatch(
      /value=\{item\.(?:duration|name|priceLabel)\}/u,
    );
  });
});
